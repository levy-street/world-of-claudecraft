...
Break a large feature into a phased implementation plan designed for multiple Codex sessions. Every phase runs as its own fresh session and uses **Opus 4.8 at xhigh effort** (`ultracode` where the phase warrants deterministic multi-agent orchestration). The whole point is to **save context per phase**: the orchestrator delegates reading and fan-out to subagents and keeps only conclusions, so each session stays sharp.
...
**Before doing anything else: scan memory.** If you use Codex's memory feature, check your `MEMORY.md` index and project memory entries for prior decisions or feedback relevant to this feature's domain. Past incidents encoded there are cheaper than rediscovering them.
...
- `phase-XX-{slug}.md` - implementation prompt for phase XX (self-contained; a fresh-context Codex can paste it into a new session and execute without referring back to the TOC).
...
Split the feature into phases. Each implementation phase is followed by a dedicated QA phase. Both are separate Codex sessions. The numbering is: Phase 1 (implement), Phase 1 QA (verify), Phase 2 (implement), Phase 2 QA (verify), etc.
...
1. **Step 0 - Pre-flight**: Verify `git status` is clean (and that no concurrent session is mid-change in your files). Scan your Codex memory (the `MEMORY.md` index and any entries matching the phase domain), if you use it.
...
- Memory scan (if you use Codex memory): check your `MEMORY.md` index and any
...
- AGENTS.md (root) + the relevant sub-AGENTS.md files
  ({e.g. src/sim/AGENTS.md, server/AGENTS.md, src/ui/AGENTS.md, src/net/AGENTS.md})
...
- If you use Codex memory, record any surprising rules or current-state notes for
  the next session.
...
- Memory scan (if you use Codex memory): check entries from the Phase N domain.
...
- AGENTS.md (root) and relevant sub-AGENTS.md files
...
- If you use Codex memory, record any surprising rules learned during QA.
