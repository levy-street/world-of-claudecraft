...
sub `AGENTS.md` files are the source of truth; this is the checklist.
...
- When in doubt, hand this to the `cross-platform-sync` agent (`.Codex/agents/`).
...
- AGENTS.md requires tests for sim/server behavior changes. A server change with no test
  is a finding.
