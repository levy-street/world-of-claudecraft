...
behind the root AGENTS.md "Modularity" section. Apply it whenever you implement a
...
- **A multi-file subsystem:** a directory with an `index.ts` barrel that exports only
  its public surface, plus its own short `AGENTS.md` (templates:
...
2. Add a Vitest at `tests/<behavior>.test.ts` that imports the module directly and
   ...
   source (see `tests/AGENTS.md` for the idioms). For sim logic, add a determinism
...
4. If the module is the public face of a new directory, add an `index.ts` barrel and a
   local `AGENTS.md` describing only that directory's conventions.
