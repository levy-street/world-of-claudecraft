...
const links = [];
const link = (label, sig) => { const u = `https://solscan.io/tx/${sig}?cluster=devnet`; links.push({ label, sig, u }); console.log(`  ok ${label}\n    ${u}`); };
...
async function expectFail(label, expectCode, fn) {
  try { await fn(); throw new Error(`${label}: expected failure but it SUCCEEDED`); }
    ...
    console.log(`  ok ${label} rejected with ${expectCode}`);
  }
}
