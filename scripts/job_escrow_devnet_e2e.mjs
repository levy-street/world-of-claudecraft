...
const record = (label, sig) => {
  links.push({ label, url: solscan(sig) });
...
function loadDeployer() {
  ...
  const raw = line
    .slice('SOLANA_DEVNET_DEPLOYER='.length)
    .trim()
    .replace(/^["']|["']$/g, '');
...
const jobPda = (jobId) =>
  PublicKey.findProgramAddressSync([Buffer.from('job'), u64le(jobId)], PROGRAM_ID)[0];
...
console.log(
  `  helper now holds ${(await getAccount(conn, helperAta)).amount / unit} tokens (expect 100)`,
);
...
fs.writeFileSync(
  'docs/screenshots/jobs/devnet-tx-links.json',
  JSON.stringify({ program: PROGRAM_ID.toBase58(), mint: mint.toBase58(), links }, null, 2),
);
