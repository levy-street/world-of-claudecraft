...
async function enter(page, charName, cls, mode) {
  ...
  await page.evaluate(
    ...
    `mkt_${charName}`,
    PASS,
    mode === 'register' ? '#btn-register' : '#btn-login',
  );
...
async function goToMerchant(page) {
  ...
  await page.waitForFunction(() => window.__game.world.marketInfo !== null, {
    timeout: 8000,
    polling: 150,
  });
}
...
if (PHASE === 'sell') {
  ...
  // persistence phase. Listings stake a copper deposit now, so the seller first
  // earns coin at Trader Wilkes exactly the way the buyer does below.
  await seller.evaluate(() => {
    ...
    window.__game.online.cmd({ cmd: 'dev_give', item: 'militia_vest', count: 5 });
  });
  await sleep(500);
  await seller.evaluate(() => window.__game.online.cmd({ cmd: 'dev_teleport', x: -7, z: 3 })); // Trader Wilkes
  await sleep(600);
  await seller.evaluate(() => {
    for (let i = 0; i < 5; i++) window.__game.world.sellItem('militia_vest');
  });
  await sleep(600);
  const sellerPurse = await seller.evaluate(() => window.__game.world.copper);
  check(sellerPurse >= 200, `seller earned coin for the listing deposits (${sellerPurse}c)`);
  await goToMerchant(seller);
  await seller.evaluate(() => {
  ...
  await sleep(500);
  // the wire carries the deposit staked on the viewer's own listings; capture the
  // boots deposit so the owed assertion below can account for its refund on sale
  const bootsDeposit = await seller.evaluate(
    () =>
      window.__game.world.marketInfo?.listings.find((l) => l.mine && l.itemId === 'oiled_boots')
        ?.depositTotal ?? 0,
  );
  check(bootsDeposit > 0, `boots listing staked a deposit (${bootsDeposit}c)`);
  ...
  // the seller, still online, was credited: sale proceeds less the 5% cut PLUS
  // the refunded listing deposit ride the collection box together
  await sleep(500);
  const owed = await seller.evaluate(() => window.__game.world.marketInfo?.collectionCopper ?? 0);
  const expectedOwed = Math.floor(400 * 0.95) + bootsDeposit;
  check(
    owed === expectedOwed,
    `seller is owed ${owed}c (380c sale proceeds + ${bootsDeposit}c deposit refund)`,
  );
  ...
  check(
    after === before + owed,
    `seller collected ${owed}c into the purse (${before} -> ${after})`,
  );
} else {
  ...
  await seller.evaluate(() => window.__game.hud.openMarket());
  await sleep(500);
  const persisted = await seller.evaluate(
    (sellerName) =>
      window.__game.world.marketInfo.listings.some(
        (l) => l.sellerName === sellerName && l.itemId === 'tallow_candle',
      ),
    SELLER,
  );
