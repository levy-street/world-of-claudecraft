...
// grants riding plus every reins item through the public dev command, screenshots
// the bags (the collection), and uses those reins to show mounted riders in the
// world (the rigged Valorsteed, the epic griffin and gobbler, and the clipless
// hover cycle).
...
// Learn riding and collect every reins item through the same dev command players
// use while building visual proofs.
...
const swapTo = async (itemId, key, path) => {
  await page.evaluate((id) => window.__game.sim.useItem(id), itemId);
  await page.waitForFunction(
    (k) =>
...
await swapTo('reins_stormfeather_griffin', 'stormfeather_griffin', 'tmp/mounts_griffin.png');
await swapTo('reins_thunderstrut_gobbler', 'thunderstrut_gobbler', 'tmp/mounts_gobbler.png');
await swapTo('reins_aether_hover_cycle', 'aether_hover_cycle', 'tmp/mounts_hover_cycle.png');
