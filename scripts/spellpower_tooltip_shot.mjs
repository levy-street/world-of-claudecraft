...
await page.evaluate(() => {
  for (const b of document.querySelectorAll('button')) if (/skip tutorial/i.test(b.textContent || '')) b.click();
});
...
const hovered = await page.evaluate(() => {
  const rows = [...document.querySelectorAll('#spellbook [draggable="true"], #spellbook .spell-row, #spellbook li, #spellbook .ability-row')];
  const row = rows.find((r) => /frostbolt/i.test(r.textContent || ''));
  if (!row) return false;
  const rect = row.getBoundingClientRect();
  for (const type of ['mouseover', 'mouseenter', 'mousemove']) {
    row.dispatchEvent(new MouseEvent(type, { bubbles: true, clientX: rect.left + 10, clientY: rect.top + 5 }));
  }
  return true;
});
await wait(300);
const tip = await page.evaluate(() => {
  const t = [...document.querySelectorAll('div')].find((d) =>
    /Frostbolt/i.test(d.textContent || '') && /Frost damage|to \d/.test(d.textContent || '') && d.offsetParent !== null && d.className.includes('tt'),
