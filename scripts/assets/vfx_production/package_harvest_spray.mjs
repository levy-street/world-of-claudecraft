// Pack the authored wound spray, preserving straight alpha and cell gutters.
// node scripts/assets/vfx_production/package_harvest_spray.mjs tmp/harvest-spray
...
const source = path.resolve(process.argv[2] ?? 'tmp/harvest-spray');
const metadata = JSON.parse(await readFile(path.join(source, 'metadata.json'), 'utf8'));
assert.equal(metadata.source, 'Blender authored directional spray');
assert.deepEqual(metadata.pivot, [0.5, 0.5]);
assert.deepEqual(
...
assert(frames.filter((frame) => frame.bounds).length >= 60, 'Incomplete spray render');
