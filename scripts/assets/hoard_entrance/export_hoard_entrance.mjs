// Deterministic procedural Buried Hoard entrance export, optimization, and visual evidence.
//
// Usage:
//   node scripts/assets/hoard_entrance/export_hoard_entrance.mjs
//   node scripts/assets/hoard_entrance/export_hoard_entrance.mjs --no-preview
//   node scripts/assets/hoard_entrance/export_hoard_entrance.mjs --raw-only
...
import { HOARD_ENTRANCE_CONTRACT } from './model.js';
import { hoardEntranceSourceFingerprint } from './source_fingerprint.mjs';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(HERE, '..', '..', '..');
const ENTRY = path.join(HERE, 'export_entry.js');
const RAW_OUT = path.join(ROOT, 'tmp/asset_src/hoard_entrance/hoard_entrance-final.glb');
const SPEC = path.join(ROOT, 'scripts/assets/specs/hoard_entrance.json');
const BUILD_ASSETS = path.join(ROOT, 'scripts/assets/build_assets.mjs');
const SHIPPING_OUT = path.join(ROOT, 'public/models/props/hoard_entrance.glb');
const CANDIDATE_ROOT = path.join(ROOT, 'tmp/asset_optimized/hoard_entrance/deterministic');
const CANDIDATE_OUT = path.join(CANDIDATE_ROOT, 'models/props/hoard_entrance.glb');
const EVIDENCE_ROOT = path.join(ROOT, 'tmp/hoard_entrance_preview');
...
async function inspectGlb(glbPath) {
  ...
  const modelRoot = root
    .listNodes()
    .find((node) => node.getName() === HOARD_ENTRANCE_CONTRACT.rootName);
  const sockets = HOARD_ENTRANCE_CONTRACT.sockets.map((definition) => {
...
function verifyContract(stats, optimized, sourceFingerprint) {
  ...
  assertCondition(
    JSON.stringify(stats.sceneChildren) === JSON.stringify([HOARD_ENTRANCE_CONTRACT.rootName]),
    `${stats.path} scene root changed`,
  );
  assertCondition(stats.nodes === 11, `${stats.path} must contain eleven semantic nodes`);
  assertCondition(stats.meshes.length === 5, `${stats.path} must contain five meshes`);
  assertCondition(stats.primitives === 5, `${stats.path} must contain five primitives`);
  assertCondition(
    stats.meshes.every(
      (mesh) =>
  ...
  assertCondition(
    stats.triangles <= HOARD_ENTRANCE_CONTRACT.triangleTarget,
    `${stats.path} exceeds ${HOARD_ENTRANCE_CONTRACT.triangleTarget} target triangles`,
  );
  assertCondition(
    stats.triangles <= HOARD_ENTRANCE_CONTRACT.triangleCeiling,
    `${stats.path} exceeds ${HOARD_ENTRANCE_CONTRACT.triangleCeiling} hard triangles`,
  );
  assertCondition(stats.materials.length === 4, `${stats.path} must contain four materials`);
  assertCondition(
    JSON.stringify(stats.materials.map((material) => material.name).sort()) ===
      JSON.stringify(['HoardEarthStone', 'HoardIron', 'HoardRarityMetalwork', 'HoardWood']),
  ...
  if (optimized) {
    assertCondition(
      stats.bytes <= HOARD_ENTRANCE_CONTRACT.byteCeiling,
      `${stats.path} exceeds ${HOARD_ENTRANCE_CONTRACT.byteCeiling} bytes`,
    );
  }
  const tolerance = optimized ? 2e-3 : 1e-5;
  assertCondition(Math.abs(stats.bounds.min[1]) < tolerance, 'asset must sit on Y=0');
  assertCondition(stats.bounds.max[1] < 3.3, 'height ceiling exceeded');
  assertCondition(stats.bounds.max[0] - stats.bounds.min[0] < 6.2, 'width ceiling exceeded');
  assertCondition(stats.bounds.max[2] - stats.bounds.min[2] < 5.2, 'depth ceiling exceeded');
  ...
  assertCondition(
    runtime?.assetId === HOARD_ENTRANCE_CONTRACT.id,
  ...
  for (const [index, definition] of HOARD_ENTRANCE_CONTRACT.sockets.entries()) {
...
function runOptimizer(outputRoot) {
  ...
  if (result.status !== 0)
    throw new Error(`hoard entrance optimizer failed: ${result.status ?? 'unknown'}`);
}
...
const sourceFingerprint = hoardEntranceSourceFingerprint(ROOT);
...
try {
  ...
  await page.waitForFunction('window.__hoardEntranceReady !== undefined', { timeout: 20_000 });

  const result = await page.evaluate(() => window.exportHoardEntrance());
  ...
  console.log(`raw hoard entrance: ${JSON.stringify(rawStats)}`);

  if (!rawOnly) {
    let stagedBytes = null;
    if (process.argv.includes('--verify-staged')) {
      const staged = spawnSync('git', ['show', ':public/models/props/hoard_entrance.glb'], {
        cwd: ROOT,
      });
      assertCondition(staged.status === 0, 'stage the expected GLB before --verify-staged');
      stagedBytes = staged.stdout;
    }
    runOptimizer(null);
    if (stagedBytes)
      assertCondition(
        stagedBytes.equals(readFileSync(SHIPPING_OUT)),
        'shipping rebuild differs from staged bytes',
      );
    ...
    assertCondition(
      ...
      'deterministic optimized hoard entrance rebuild differs byte-for-byte',
    );
    console.log(`optimized hoard entrance: ${JSON.stringify(shippingStats)}`);
  ...
  if (!noPreview) {
    const capture = async ({ kind, b64 = null, viewName, stage = 'final', out }) => {
      await page.evaluate((options) => window.renderHoardEntrancePreview(options), {
        kind,
        b64,
        viewName,
        stage,
      ...
      if (!canvas) throw new Error('hoard entrance preview canvas was not created');
    ...
    for (const [kind, base64] of [
      ...
      await makeContactSheet(files, TURNAROUND_VIEWS, contact, `${kind} Buried Hoard entrance`);
