// Ship the Blender-authored Emberforge room kit (docs/design/forge-room/). Never
// flatten or join: the runtime instances each Kit_* node on its own, and pulses
// the ForgeMolten material apart from the painted ForgeSolid one.
...
export const ASSETS = [
  {
    source: 'docs/design/forge-room/forge_kit_components.glb',
    target: 'public/models/props/hoard_forge_kit.glb',
    nodes: [
      'ForgeKit_ROOT',
      'Kit_Anvil',
      'Kit_ChainHook',
    ...
    materials: ['ForgeMolten', 'ForgeSolid'],
  },
];

export function sourceFingerprint(asset, root = ROOT) {
  const hash = createHash('sha256');
  for (const file of [asset.source, 'scripts/assets/forge_room/build.mjs']) {
    hash
      .update(file)
      .update('\0')
