// Deterministic Weirdo Cream truck export, optimization, validation, and preview.
//
// Usage:
//   node scripts/assets/weirdo_cream_truck/export_weirdo_cream_truck.mjs --stage blockout --raw-only
//   node scripts/assets/weirdo_cream_truck/export_weirdo_cream_truck.mjs --stage final
//   node scripts/assets/weirdo_cream_truck/export_weirdo_cream_truck.mjs --stage final --no-preview
//
// Same archetype as the Terrorspark Groundshaker exporter, with one addition:
// alongside the two shared procedural map families this asset carries a signage
// atlas of its own, attached to the decal material only.
...
import { buildTankSurfaceMaps, NORMAL_SCALE } from '../terrorspark_groundshaker/surface_maps.mjs';
import { ORM_CENTER } from '../terrorspark_groundshaker/surface_shading.mjs';
import { buildDecalAtlas } from './decal_atlas.mjs';
import {
  TRUCK_CAB,
  TRUCK_CLIP_NAMES,
...
import { truckSourceFingerprint } from './source_fingerprint.mjs';

const HERE = path.dirname(fileURLToPath(import.meta.url));
const ROOT = path.resolve(HERE, '..', '..', '..');
const ENTRY = path.join(HERE, 'export_entry.js');
const SPEC = path.join(ROOT, 'scripts/assets/specs/weirdo_cream_truck.json');
const BUILD_ASSETS = path.join(ROOT, 'scripts/assets/build_assets.mjs');
const SHIPPING_OUT = path.join(ROOT, 'public/models/mounts/weirdo_cream_truck.glb');
const PREVIEW_ROOT = path.join(ROOT, 'docs/screenshots/weirdo-cream-truck/authoring');
...
if (!TRUCK_STAGES.includes(stage)) throw new Error(`unknown truck stage: ${stage}`);
const rawOnly = process.argv.includes('--raw-only');
const noPreview = process.argv.includes('--no-preview');
const rawOut = path.join(ROOT, `tmp/asset_src/weirdo_cream_truck/weirdo_cream_truck-${stage}.glb`);
const sourceFingerprint = truckSourceFingerprint(ROOT);

/** Two shared families (metal, fabric) times albedo/normal/ORM, plus the one
 *  signage atlas this asset owns. */
const TRUCK_TEXTURE_COUNT = 7;
/** Shipping ceiling. The shared map families cost what they cost on the tank
 *  (576 KiB there); the signage atlas is the only addition, so the truck is
 *  allowed one atlas more and nothing else. */
const SHIPPING_BYTE_CEILING = 704 * 1024;
/** Triangle ceiling. A van is mostly flat panels, so the budget goes on the
 *  wheels, the roof cone, and the awning rather than on body curvature. */
const TRIANGLE_CEILING = 17_000;
 ...
 * KHR_texture_transform, so `quantize()` can pack the texcoords like every other
 * attribute (see the tank exporter's note: an un-normalized texcoord accessor
 * costs more than the rest of the geometry).
 *
 * The signage quads ride along safely: their UVs are authored in [0, 1] atlas
 * space, and dividing by the shared range here then multiplying by the same
 * range in the sampler transform round-trips them exactly.
 */
...
/**
 * Attach the procedural PBR map sets plus the signage atlas.
 *
 * The five body materials share the tank's metal and fabric families, with their
 * factors divided by the map midtone so factor times sampled channel lands back
 * on the authored target. The decal material takes baseColor ONLY: its UVs
 * address the atlas rather than the world-space projection every other material
 * tiles through, so a shared normal or ORM map would be sampled through the
...
async function attachSurfaceMaps(glbPath) {
  ...
  const textureFor = (family, channel) => {
    ...
    const texture = document
      .createTexture(`truck_${key}`)
      .setMimeType('image/webp')
      .setImage(family === 'decal' ? atlas.webp : maps[family][channel]);
  ...
  for (const contract of TRUCK_MATERIAL_CONTRACT) {
    ...
    if (contract.surface === 'decal') {
      material.setBaseColorTexture(textureFor('decal', 'albedo'));
      material.setRoughnessFactor(contract.roughness);
      material.setMetallicFactor(contract.metalness);
      const info = material.getBaseColorTextureInfo();
      // Atlas UVs land exactly inside their region, so clamp rather than repeat:
      // a neighbouring region must never bleed across a decal's edge.
      info.setWrapS(TextureInfo.WrapMode.CLAMP_TO_EDGE);
      info.setWrapT(TextureInfo.WrapMode.CLAMP_TO_EDGE);
...
async function inspectGlb(glbPath) {
  ...
  return {
    ...
    bounds,
    sockets: TRUCK_SOCKET_DEFINITIONS.map((definition) => {
...
function verifyContract(stats, optimized) {
  ...
  assertCondition(
    JSON.stringify(stats.sceneChildren) === JSON.stringify(['WeirdoCreamTruck']),
    `${stats.path} scene root must be WeirdoCreamTruck`,
  );
  assertCondition(
    stats.textures.length === TRUCK_TEXTURE_COUNT,
    `${stats.path} must ship ${TRUCK_TEXTURE_COUNT} textures, found ${stats.textures.length}`,
  );
  assertCondition(
    stats.textures.every((texture) => texture.mimeType === 'image/webp' && texture.bytes > 0),
    `${stats.path} textures must all be non-empty webp`,
  );
  ...
  assertCondition(
    ...
    `${stats.path} exceeds ${TRIANGLE_CEILING} triangles (${stats.triangles})`,
  );
  assertCondition(stats.primitives <= 26, `${stats.path} exceeds 26 primitives`);
  if (stage === 'final') {
    ...
    assertCondition(
      JSON.stringify(stats.materials.map((material) => material.name).sort()) ===
        JSON.stringify(TRUCK_MATERIAL_CONTRACT.map((material) => material.name).sort()),
      `${stats.path} material names changed`,
    );
  }
  assertCondition(
    JSON.stringify(stats.animations.map((animation) => animation.name)) ===
      JSON.stringify(TRUCK_CLIP_NAMES),
  ...
  // The open-cab contract: the driver stands on the cab floor, so the shell must
  // not reach the rider's torso. If a later edit roofs the cab or raises the door
  // line, this is what turns red rather than a clipping report from a player.
  const riderSocket = stats.sockets.find((socket) => socket?.name === 'Socket_Rider');
  assertCondition(Boolean(riderSocket), `${stats.path} must carry Socket_Rider`);
  assertCondition(
    Math.abs(riderSocket.translation[2] - TRUCK_CAB.riderZ) <= 0.001,
    `${stats.path} rider socket must sit at the authored cab Z`,
  );
  assertCondition(
    riderSocket.translation[1] >= TRUCK_CAB.floorY &&
      riderSocket.translation[1] <= TRUCK_CAB.floorY + 0.1,
    `${stats.path} rider socket must rest on the cab floor`,
  );
  assertCondition(
    TRUCK_CAB.screenZ - TRUCK_CAB.riderZ > TRUCK_CAB.riderClearRadius,
    'the windscreen must stand clear of the driver body radius',
  );
  assertCondition(
    TRUCK_CAB.riderZ - TRUCK_CAB.backZ > TRUCK_CAB.riderClearRadius,
    'the freezer box front wall must stand clear of the driver body radius',
  );

  if (stage === 'final') {
    assertCondition(
      stats.bounds.max[1] >= 3.6 && stats.bounds.max[1] <= 3.78,
      `${stats.path} height must remain in the 3.6 to 3.78 yard contract`,
    );
  }
  assertCondition(
  ...
  if (optimized) {
    assertCondition(
      ...
      `${stats.path} exceeds ${SHIPPING_BYTE_CEILING / 1024} KiB (${stats.bytes})`,
    );
  }
}
...
try {
  ...
  const result = await page.evaluate(
    (selectedStage, fingerprint) => window.exportWeirdoCreamTruck(selectedStage, fingerprint),
    stage,
    sourceFingerprint,
  );
...
writeFileSync(path.join(materialEvidence, 'signage-atlas.webp'), surfaceMaps.atlas.webp);
console.log(
  `surface maps: metal ${surfaceMaps.metal.albedoSize}/${surfaceMaps.metal.reliefSize}, ` +
    ...
    `signage atlas ${surfaceMaps.atlas.size}px, uv range ${surfaceMaps.uvRange}`,
);
