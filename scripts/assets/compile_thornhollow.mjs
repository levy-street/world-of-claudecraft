...
lines.push('// getter, so importing this module parses nothing: a player who never enters');
lines.push('// the band never pays for the field at all (see emitJsonConst in the compiler).');
...
// is a memoized getter instead: the parse happens on the first read, which for
// a player who never enters the band never happens at all. The exported SHAPE is
// unchanged (a readonly array), so every consumer and pin reads exactly as
// before.
