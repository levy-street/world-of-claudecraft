...
| Perf / profiling | `profile.mjs` (scenario CLI, see its `SCENARIOS` map) over `scripts/profiler/` (own CLAUDE.md); `perf_tour.mjs` (`perf:tour`), `crowd_fps_bench.mjs` (`perf:crowd`), `prewarm_travel_bench.mjs` (`perf:prewarm`), `server_load_jitter.mjs` (`perf:load`; the crowd and jitter gates share `lib/bench_gate.mjs`, pinned by `tests/bench_gate.test.ts`), `feel_smoke.mjs` (`feel:smoke`), `asset_budget.mjs` (`asset:budget`) | dev (some + server) |
| Loot balance | `procedural_legendary_balance.ts` plus its CLI/core run the 33,000,000-event named-power contribution gate; `procedural_raid_balance.ts` plus its CLI/core run the 200,000-item live Nythraxis distribution, item-level, class-roster, UID, affix, Raid-forged, power-floor, and deterministic-horizon gate. `npm run loot:balance` enforces both sequentially; `loot:balance:report` is non-enforcing. Pure cores and CLI parsing are pinned by `tests/procedural_legendary_balance*.test.ts` and `tests/procedural_raid_balance*.test.ts`. | none |
