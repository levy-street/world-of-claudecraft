...
await page.evaluate(() => {
  ...
  if (n) { n.value = 'Smith'; n.dispatchEvent(new Event('input', { bubbles: true })); }
});
...
await page.evaluate(() => {
  ...
  const items = ['cryptbone_helm', 'cryptbone_pauldrons', 'mistveil_cord', 'mistveil_grips',
    'boundstone_helm', 'boundstone_girdle', 'gravewyrm_mantle', 'gravewyrm_gauntlets',
    'deathlords_dread_visage', 'necromancers_soulspire_mantle', 'wyrmshadow_talongrips'];
...
await page.screenshot(box ? { path: 'tmp/inv2_icons.png', clip: box } : { path: 'tmp/inv2_icons.png' });
