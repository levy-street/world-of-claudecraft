...
for (const { label, url } of links) {
  ...
  await page
    .goto(explorer, { waitUntil: 'networkidle2', timeout: 45000 })
    .catch((e) => console.log(`goto ${label}:`, e.message));
  // Wait for the explorer to render the transaction result.
  await page
    .waitForFunction(() => /Finalized|Confirmed|Success|Result/i.test(document.body.innerText), {
      timeout: 20000,
    })
    .catch(() => {});
  await sleep(1500);
