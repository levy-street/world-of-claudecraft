...
const extraIcons = [
  { abilityId: 'felblade', column: 2, row: 2, source: atlas },
];
...
for (const icon of extraIcons) {
  await sharp(icon.source)
    .extract({ left: leftPad + icon.column * cellW, top: topPad + icon.row * cellH, width: size, height: size })
    .resize(iconSize, iconSize, { fit: 'cover' })
    .webp({ quality: 88, alphaQuality: 100, smartSubsample: true, effort: 6 })
    .toFile(path.join(outDir, `${icon.abilityId}.webp`));
}
...
const mapping = {
  ...
  abilities: [
    ...
    ...extraIcons.map((icon) => ({
      abilityId: icon.abilityId,
      sourceCell: { column: icon.column, row: icon.row },
      output: `${icon.abilityId}.webp`,
      source: 'scripts/assets/demon_hunter_skill_atlas.png',
    })),
  ],
};
await writeFile(path.join(outDir, 'mapping.json'), `${JSON.stringify(mapping, null, 2)}\n`);
console.log(`generated ${abilities.length + extraIcons.length} Demon Hunter icons from ${mapping.sourceAtlas}`);