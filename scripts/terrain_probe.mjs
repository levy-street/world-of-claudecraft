...
await build({ entryPoints: ['src/sim/world.ts'], bundle: true, format: 'esm', outfile: out, platform: 'node', logLevel: 'silent' });
...
const tag = (x, z) => { const h = g(x, z); return h < DEEP ? 'DEEP🌊' : h < SWIM ? 'swim' : h < WATER_LEVEL ? 'wade' : 'DRY'; };
...
for (const [name, x0, z0, x1, z1] of legs) {
  ...
  const N = 24;
  for (let i = 0; i <= N; i++) { const x = x0 + (x1 - x0) * i / N, z = z0 + (z1 - z0) * i / N; const h = g(x, z); line += h < DEEP ? '#' : h < SWIM ? '~' : h < WATER_LEVEL ? '-' : '.'; }
  console.log(line + `   (.=dry -=wade ~=swim #=deep)`);
}

console.log('\nCamp centers:');
for (const [n, x, z] of [['widows', 70, 300], ['drowned', 90, 420], ['trolls', -80, 420], ['cult', 15, 470]]) {
...
for (const r of [20, 32]) {
  let s = `  r=${r}: `;
  for (let a = 0; a < 360; a += 45) { const x = 90 + Math.cos(a * Math.PI / 180) * r, z = 420 + Math.sin(a * Math.PI / 180) * r; s += `${a}°:${tag(x, z)} `; }
  console.log(s);
}
