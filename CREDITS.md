...
- **The CraftPix generic item icons** (`public/ui/items/*.webp`, except entries
  ...
  Loot v1 is excluded through its project-owned `entries` and `proceduralSets`
  records in the mapping; none of those 225 assets inherits the CraftPix licence.
  Remove or replace the CraftPix icons in a redistributable fork, or
...
| Procedural Loot v1 item icons (34 legacy base aliases plus 191 rarity-fallback and named-power states under `public/ui/items/procedural/v1/`, all 225 declared by `mapping.json.proceduralSets`) | World of ClaudeCraft | Project-owned, AI-assisted art generated with OpenAI image generation, chroma-key cleaned, and normalized recursively through the item-art pipeline; exact base, rarity, power, revision, and provenance records in `public/ui/items/mapping.json` | Project asset, rights reserved | **No, permission required** |
