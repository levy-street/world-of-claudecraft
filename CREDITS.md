...
| Cluckwork Mech Bird rideable mount model (`public/models/mounts/mech_bird.glb`) | World of ClaudeCraft | Project-generated Tripo API parts sculpt (owned under the Tripo paid-plan license), hand-rigged and hand-animated in Blender in-project (28-joint rig, authored Idle/Run/Jump clips); shipped through the `scripts/assets/specs/mech_bird.json` optimize spec. Its item painting is credited in the item-art consistency row above | Project asset | With the project only |
