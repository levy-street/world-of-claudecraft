...
| Rock-mount reins icons (`public/ui/items/reins_pet_rock.webp`, `reins_shiny_pet_rock.webp`) | World of ClaudeCraft | Project-generated painted art via `scripts/asset_pipeline` (Tripo text-to-image), normalized to a 1024 square opaque master and encoded to 128px WebP by `scripts/convert_item_icons_webp.mjs`; per-icon provenance in `public/ui/items/mapping.json` and the review lineage in the [item-art consistency verdict](docs/achievements/item-art-consistency-2026-08-09/) | Project asset | With the project only |
