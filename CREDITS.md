...
| Solana Seeker rideable mount model (`public/models/mounts/seeker_board.glb`) | Solana Mobile wordmark and Seeker handset design are trademarks of their owner; model supplied to the project by the repository owner | Supplied rigged (Blender glTF export, Idle/Walk/Jump authored); the Run and Death clips and the animation optimisation are project work, baked by `scripts/bake_seeker_gaits.mjs`. The deck texture carries the Seeker / Solana Mobile wordmark. **Branding permission is UNRESOLVED**: issue #3628 lists official branding as an open question, and this asset must not ship until the owner records the grant here | Third-party marks, project-supplied model | **No, permission required** |
...
| Solana Seeker engine layers (`mount_run_seeker_board.mp3`, `mount_run_seeker_board_start.mp3`, `mount_run_seeker_board_stop.mp3`) | World of ClaudeCraft | Original deterministic synthesis via `scripts/sfx/synth_seeker_board.mjs`, conformed through the shared SFX path; no sampled source | Project asset | With the project only |
