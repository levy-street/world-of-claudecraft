...
import type { ToolbeltState } from '../sim/toolbelt';
...
export interface IWorldInventory {
  ...
  bagCapacity: number;
  // The tool-only container: generic tool slots (2/3/4 by belt tier), holding
  ...
  /** Move a carried tool into the first empty belt slot (slots are generic,
   *  so no slot argument; a full belt refuses). */
  storeToolInBelt(itemId: string): void;
  /** Move the tool in one belt slot (by position) back to the inventory. */
