...
export class CharacterVisual {
  ...
  update(dt: number, s: AnimState, animate: boolean): void {
    ...
    // Bow cycle phase machine: runs AFTER the base-state block so it can
    // re-assert the aim hold over an idle fade the same frame, and outside
    // `animate` so throttled rigs still latch phase edges.
