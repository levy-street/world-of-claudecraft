...
// Tower assets predate the release-wide Tripo vocabulary expansion. Keep their
// authored clip contract separate so renderer lookups never request clips the
// shipped GLBs do not contain.
const TOWER_TRIPO_RIG: ClipMap = {
  idle: 'Idle',
  walk: 'Walk',
  run: 'Run',
  attack: ['Attack'],
...
const TOWER_TRIPO_CREATURE_RIG: ClipMap = {
  idle: 'Idle',
  walk: 'Walk',
  run: 'Run',
  attack: ['Attack'],
...
export const VISUALS: Record<string, VisualDef> = {
  ...
  // The Tripo source rigs face +X, so every entry carries the same -90 degree
  // correction into the game's +Z convention. Their authored heights get a
  // presentation-only 6-20% lift: smaller bodies get the stronger boost while
  // already-large elites stay readable around the chase camera. This changes
  ...
  mob_tower_hellhound: {
    ...
    height: 1.56,
    yaw: -Math.PI / 2,
    clips: TOWER_TRIPO_CREATURE_RIG,
    tint: 'entity',
    tintStrength: 0.06,
  },
...
const MOB_KEYS: Record<string, string> = {
  ...
  tower_gloom_bat: 'mob_tower_gloom_bat',
  tower_cinder_crawler: 'mob_tower_cinder_crawler',
  tower_bone_acolyte: 'mob_tower_bone_acolyte',
  tower_shade_dancer: 'mob_tower_shade_dancer',
  tower_flame_herald: 'mob_tower_flame_herald',
  tower_rot_hulk: 'mob_tower_rot_hulk',
  tower_blood_matron: 'mob_tower_blood_matron',
  tower_void_sentinel: 'mob_tower_void_sentinel',
  tower_boss_flesh_shaper: 'mob_tower_boss_flesh_shaper',
  tower_boss_ash_tyrant: 'mob_tower_boss_ash_tyrant',
