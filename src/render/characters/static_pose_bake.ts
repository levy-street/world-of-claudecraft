// Static idle-pose baking: collapse a posed character clone into one merged
// BufferGeometry for the far-LOD / shadow-proxy path. Extracted from assets.ts
// so the merge logic is unit-testable without the loader/preload machinery.
import * as THREE from 'three';
import { mergeGeometries } from 'three/addons/utils/BufferGeometryUtils.js';

/** True when `o` and every ancestor up to `stopAt` is visible. */
...
export function bakeStaticPose(
  ...
  root.traverse((o) => {
    ...
    geos.push(out);
    // GLTFLoader emits one Mesh per primitive, so materials are never arrays here
