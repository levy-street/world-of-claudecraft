    ...
    case 'scales': {
      ...
      for (let row = 0, y = 0; y < size + r; row++, y += r * 0.62) {
        ...
        for (let x = -r; x < size + r; x += r * 2) {
          ...
          ctx.fillStyle = rgba(secondary, 0.32);
          ctx.fill();
...
function paintFinish(
  ...
  if (finish === 'metallic') {
    ...
    ctx.fillRect(-size, size * 0.2, size * 3, size * 0.3);
    ctx.restore();
  }
}
...
export function buildSkinEmissiveCanvas(
  spec: SkinDesignSpec,
  size = ATLAS,
