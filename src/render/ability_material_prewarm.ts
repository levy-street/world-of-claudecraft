...
import { HourglassFieldVisual, hourglassFieldMaterial } from './hourglass_field_visual';
...
export const ABILITY_MATERIAL_SOURCES: readonly AbilityMaterialSource[] = [
  ...
  },
  {
    ...
    materials: () => (['protective', 'hostile', 'unknown'] as const).map(hourglassFieldMaterial),
    build: () => {
      const group = new THREE.Group();
      for (const mode of ['protective', 'hostile', 'unknown'] as const) {
