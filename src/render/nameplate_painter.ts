...
export class NameplatePainter {
  ...
  private questMarker(entity: Entity): { marker: string; tone: NameplateMarkerTone } {
    let marker = '';
    let tone: NameplateMarkerTone = 'none';
    for (const questId of entity.questIds) {
      const quest = QUESTS[questId];
      if (!quest) continue;
      const status = this.world.questState(questId);
      if (status === 'ready' && isQuestTurnInNpc(quest, entity.templateId)) {
        return { marker: '?', tone: 'quest' };
      }
      if (status === 'available' && quest.giverNpcId === entity.templateId) {
        marker = '!';
        tone = 'quest';
      } else if (
        status === 'active' &&
        isQuestTurnInNpc(quest, entity.templateId) &&
        marker === ''
      ) {
        marker = '?';
        tone = 'active';
