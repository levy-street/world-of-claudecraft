    ...
    for (const c of chars) {
      ...
      row.innerHTML = `${portraitChipHtml({ cls: c.class, skin: c.skin ?? 0, name: c.name, variant: 'sm' })}
        ...
        ${c.forceRename
          ...
          : `<span class="char-actions"><button class="btn btn-danger delete-char-btn" ${c.online ? 'disabled' : ''}>${escapeHtml(t('character.delete'))}</button>${WALLET_ENABLED && !c.online ? `<button class="btn paid-rename-btn" type="button">${escapeHtml(t('character.rename'))}</button>` : ''}<button class="btn enter-world-btn" ${c.online ? 'disabled' : ''}>${escapeHtml(t('auth.enterWorld'))}</button></span>`}`;
