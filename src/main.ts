    ...
    for (const c of chars) {
      ...
      row.innerHTML = `${portraitChipHtml({ cls: c.class, skin: c.skin ?? 0, name: c.name, variant: 'sm' })}
        ...
        ${c.forceRename
          ...
          : `<span class="char-actions"><button class="btn btn-danger delete-char-btn" ${c.online ? 'disabled' : ''}>${escapeHtml(t('character.delete'))}</button>${WALLET_ENABLED && !c.online ? `<button class="btn paid-rename-btn" type="button">${escapeHtml(t('character.rename'))}</button>` : ''}${SNS_UI_ENABLED && !c.online ? `<button class="btn mint-sol-btn" type="button">${escapeHtml(t('character.mintSolName'))}</button>` : ''}<button class="btn enter-world-btn" ${c.online ? 'disabled' : ''}>${escapeHtml(t('auth.enterWorld'))}</button></span>`}`;
...
// SNS subdomain minting UI is shown only when both the wallet is configured and
// the deploy advertises SNS support (VITE_SNS_ENABLED), mirroring the server's
// SNS_ENABLED gate so players never see a mint button the server will refuse.
const SNS_UI_ENABLED = WALLET_ENABLED && /^(1|true|yes|on)$/i.test(String(import.meta.env.VITE_SNS_ENABLED ?? '').trim());
