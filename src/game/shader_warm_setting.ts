...
// the reveal mode the measured cells ran; the probe-only `all` mode stays a
// query-string arm and is never a stored option.
...
export type ShaderWarmStoredSetting = Exclude<ShaderWarmSetting, 'all'>;
...
export function shaderWarmSettingFromValue(value: number): ShaderWarmStoredSetting {
  ...
  if (rounded === SHADER_WARM_SETTING_VALUES.on) return 'reveal';
  return 'auto';
}
