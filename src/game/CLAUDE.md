...
| `boot_graphics_defaults.ts` / `render_scale_default_core.ts` | the boot-time graphics defaults `main.ts` persists before the renderer reads the store: the first-run device preset (thin glue over `gfx.ts` `firstRunGraphicsPreset`) and the Render Quality slider's default on weak integrated GPUs at LOW (`WEAK_GPU_LOW_RENDER_SCALE`, applied only while `settings.renderScaleTouched` is false; the options slider commit stamps the flag). Decided once per boot from the static adapter class, never the governor; the core is a registered `UI_PURE_CORE`. Tests: `tests/render_scale_default.test.ts`. |
