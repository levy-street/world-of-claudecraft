...
| `targeting_actions.ts` | the keyboard and pad targeting actions `main.ts` routes (the Tab cycles, the friendly picks, Pet: Mark, and the F1..F10 party target hotkeys), split out of `main.ts` so it stays a firewall; `src/ui/party_target_hotkeys_core.ts` resolves which party frame row an F-key names. Takes injected `IWorld`-shaped bags. Tests: `tests/party_target_hotkeys_core.test.ts`. |
...
| `frame_cadence_*.ts` / `display_refresh_estimator_core.ts` / `frame_rate_cap_setting.ts` | the player's Frame Rate Limit: render on a divisor of the MEASURED display rate by skipping whole frames. Pure cores (the refresh estimator, the divisor and deadline core, the automatic "settle and hold" core with its calm and surface-class leaves, the stored-choice mapping) plus ONE thin consumer, `frame_cadence_wiring.ts`, which OWNS the frame loop's re-arm: `frame()` in `src/main.ts` starts with `armFrameAndSkip(...)` in place of a bare `requestAnimationFrame`, every callback arms exactly once (a rAF, or a timer that calls `frame()` itself where no display rhythm is readable), and a throw re-arms a plain rAF. The gate view it receives is the one filled on the PREVIOUS callback, so an exemption (hidden desktop shell, graphics rebuild, held world draw, loading cover) starts or ends one callback late. `frame_cadence_auto_memory.ts` is the only module of the cluster allowed to read the graphics preset, and only to KEY what Auto remembers and to SIGN the settings a verdict was formed on (preset and render scale: a change to either re-opens the question). Tests: `tests/frame_cadence*.test.ts`, `tests/frame_rate_cap_setting.test.ts`, `tests/desktop_presentation_threading.test.ts` (the call-site pin). |
