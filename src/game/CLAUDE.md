...
| `input_tick_sampler.ts` / `movement_wire_glue.ts` / `online_movement_frame.ts` | the online movement send path. `InputTickSampler` is the pure fixed 20 Hz per-tick sampler on an absolute deadline (never a summed frame-dt accumulator, which drifts the phase) and also exposes the sub-tick `interpolationAlpha` the display reads; `MovementWireGlue` drives that sampler into `ClientWorld.sendMovementFrame` and carries a turn-engage edge across a paused wire; `sendOnlineMovementFrame` is the single per-frame call `main.ts` makes. Tests: `tests/input_tick_sampler.test.ts`, `tests/online_movement_frame.test.ts`, `tests/movement_wire_v2.test.ts`, `tests/input_cadence_model.test.ts`. |
| `online_facing_mirror.ts` | `interpolatedOnlineSelfFacing`: pure interpolation of the self facing the movement wire exposes for display, so the model's heading comes from the reconciled authoritative value instead of the rounded snapshot field. Tests: `tests/online_facing_mirror.test.ts`. |
| `self_motion_gate.ts` | pure "may the local player's motion be predicted this frame" verdict (`selfMotionPredictionEnabled`, plus `isPlayerImmobilized`/`isMovementFrozen`): off while spectating, corpse-frozen, crowd-controlled, climbing, inside a delve, or in a rift whose floor descriptor has not arrived yet, and off under the `?nopredict` kill switch. Pure, so a headless latency harness answers it exactly as the render loop does. Tests: `tests/self_motion_gate.test.ts`. |
