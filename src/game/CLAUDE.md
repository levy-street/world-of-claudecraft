...
| `woc_market_wiring.ts` | the one-call `$WOC` Exchange attach `main.ts` invokes from its online entry: browser web plus the website-distributed desktop shell (`NATIVE_APP` / `DESKTOP_APP` from `client_origin.ts` plus the shell bridge's `wocExchangeSupported` distribution probe, all injectable for tests; Steam, Epic, Capacitor, and any shell without an explicit website verdict stay fail-closed), builds the `WocMarketClient` and the `WocMarketHooks` the HUD consumes; the hooks' two wallet signers ride the external-browser handoff on desktop (`wallet.desktopAuthorize`) and the in-renderer wallet on browser web; pinned by `tests/woc_market_wiring.test.ts`. |
