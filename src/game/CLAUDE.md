...
| `gamepad.ts` / `gamepad_map.ts` / `gamepad_bindings.ts` / `gamepad_control_hint.ts` / `death_controller_hint.ts` / `dpad_focus_nav.ts` / `controller_*_prompt_coordinator.ts` / `controller_focus_hint.ts` / `gamepad_launcher_hint_view.ts` / `gamepad_action_dispatcher.ts` | pad support: thin polling consumer + pure deterministic mapping core + separate remappable layout + truthful live flat/XHB/death/world prompt labels + R3 launcher entry and spatial focus navigation + the narrow action-dispatch composition seam. Standalone death roots focus only after movement stops, so corpse-run controls are reachable without freezing the ghost. Controller hints follow last-used input ownership and stay absent from touch, keyboard/mouse, and virtual-mouse play. Tests: `tests/gamepad.test.ts`, `tests/gamepad_map.test.ts`, `tests/gamepad_bindings.test.ts`, `tests/gamepad_control_hint.test.ts`, `tests/death_controller_hint.test.ts`, `tests/dpad_focus_nav.test.ts`, `tests/controller_*_hint.test.ts`, `tests/controller_prompt_coordinator.test.ts`, `tests/gamepad_action_dispatcher.test.ts`, `tests/launcher_hint_view.test.ts`. |
