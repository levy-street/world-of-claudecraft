...
function titleBar(): string {
  return (
    `<div class="panel-title"><span>${t('hudChrome.wocSeason.title')} ` +
...
function standingsHtml(rows: SeasonStandingView[]): string {
  const head =
    `<div class="ws-st-row ws-st-head">` +
  ...
  const body = rows
    .map(
      (r) =>
        ...
        `<span class="ws-st-reward">${esc(r.rewardWoc)}</span></div>`,
    )
    .join('');
  return (
    `<div class="ws-standings"><div class="ws-st-title">${t('hudChrome.wocSeason.standingsTitle')}</div>` +
    head +
    body +
...
export function wocSeasonPanelHtml(view: SeasonView): string {
  ...
  if (view.countdown) {
    ...
    timing =
      `<div class="ws-timing"><span class="ws-timing-label">${t('hudChrome.wocSeason.endsIn')}</span> ` +
