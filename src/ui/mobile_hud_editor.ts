...
export class MobileHudEditor {
  ...
  #renderPreview(preview: HTMLElement): void {
    ...
    for (const descriptor of this.#deps.registry.descriptors) {
      ...
      if (descriptor.class === 'movable') {
        const envelope = this.#deps.document.createElement('span');
        envelope.classList.add('mobile-hud-editor-layout-envelope');
        envelope.setAttribute('aria-hidden', 'true');
        envelope.setAttribute(
          'data-mobile-hud-envelope-kind',
          descriptor.variants?.length ? 'state-envelope' : 'interactive',
        );
        const envelopeLabel = this.#deps.document.createElement('span');
        envelopeLabel.classList.add('mobile-hud-editor-layout-envelope-label');
        envelopeLabel.textContent = translatedLabel;
        envelope.append(envelopeLabel);
        proxy.append(envelope);
      }
      proxy.append(frame);
      if (descriptor.class === 'movable') {
  ...
  #positionProxies(): void {
    ...
    for (const descriptor of this.#deps.registry.descriptors) {
      ...
      let envelopeRect: MobileHudRect | undefined;
      if (descriptor.class === 'protected') {
        ...
        // A max-state outline must never steal taps from a smaller live
        // fragment. Mixed surfaces are the other important case: their live
        // visual may contain click-through text, so only the registered primary
        // control (Yumi toggle, Delve affix, View reserve) may capture selection.
        // When content is absent, the complete registered interactive footprint
        // remains selectable.
        hitRect = descriptor.primaryFootprint
          ? resolved.interactiveRect
          : (liveRect ?? resolved.interactiveRect);
        // The painted live fragment remains exact, while this separate
        // pointer-through envelope communicates every position that can receive
        // input. Stateful surfaces use their maximum active variant so a closed
        // drawer or collapsed party cannot hide a future overlap.
        envelopeRect = descriptor.variants?.length
          ? resolved.canonicalRect
          : resolved.interactiveRect;
      }
      ...
      const envelope = proxy.querySelector<HTMLElement>('.mobile-hud-editor-layout-envelope');
      if (envelope && envelopeRect) {
        const envelopePoint = mapMobileHudVisualPointToPreview(
          { x: envelopeRect.x, y: envelopeRect.y },
          transform,
        );
        envelope.style.left = `${envelopePoint.x - hitLeft}px`;
        envelope.style.top = `${envelopePoint.y - hitTop}px`;
        envelope.style.width = `${envelopeRect.width * transform.scale}px`;
        envelope.style.height = `${envelopeRect.height * transform.scale}px`;
      }
    }
  }
  ...
  #renderInspector(): void {
    ...
    if (descriptor.capabilities.includes('scale')) {
      this.#appendInspectorControl(
        ...
        this.#inspector,
        { type: 'scale-selected', steps: -1, handedness: this.#previewHandedness() },
        '−',
      );
      this.#appendInspectorControl(
        'scale-increase',
  ...
  open(): boolean {
    ...
    handle.setAttribute(
      'aria-label',
      this.#deps.translate('hudChrome.mobileHudEditor.dragHandleLabel'),
    );
    handle.setAttribute('aria-describedby', 'mobile-hud-editor-drag-hint');
    handle.setAttribute('aria-keyshortcuts', 'ArrowUp ArrowDown ArrowLeft ArrowRight Home');
    handle.tabIndex = 0;
    const handleHint = this.#deps.document.createElement('span');
    handleHint.classList.add('visually-hidden');
    handleHint.setAttribute('id', 'mobile-hud-editor-drag-hint');
    handleHint.textContent = this.#deps.translate('hudChrome.mobileHudEditor.dragHandleHint');
    handle.append(handleHint);
