...
export interface OptionsWindowDeps {
  ...
  // Optional second action bar (Hud owns the bar DOM + per-character flag); the
  // keybinds view exposes its on/off toggle and the on-bar key-bindings entry.
...
export class OptionsWindow {
  ...
  // (Per-character localStorage on the HUD, not a global Setting, so it can't reuse
  ...
  private appendSecondBarToggle(parent: HTMLElement): void {
    ...
    toggle.className = 'btn kb-key kb-toggle';
    const sync = () => {
      const on = this.deps.isSecondBarEnabled();
      toggle.textContent = on ? t('hud.options.on') : t('hud.options.off');
      toggle.classList.toggle('off', !on);
      toggle.setAttribute('aria-pressed', on ? 'true' : 'false');
    ...
    toggle.addEventListener('click', () => {
      audio.click();
      this.deps.setSecondBarEnabled(!this.deps.isSecondBarEnabled());
      sync();
    });
    row.append(name, toggle);
    parent.appendChild(row);
