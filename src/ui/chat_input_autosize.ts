...
// consumer (src/main.ts) measures the visible text height and feeds it here to
// get a clamped pixel height plus whether a scrollbar is needed once the content
// exceeds the cap. Kept host-agnostic so a Vitest unit test can pin the clamp
...
export function chatInputVisibleHeight(
  valueScrollHeight: number,
  placeholderScrollHeight: number,
  hasValue: boolean,
  borderBlockSize = 0,
): number {
  const value = Number.isFinite(valueScrollHeight) ? valueScrollHeight : 0;
  const placeholder = Number.isFinite(placeholderScrollHeight) ? placeholderScrollHeight : 0;
  const border = Math.max(0, Number.isFinite(borderBlockSize) ? borderBlockSize : 0);
  return Math.round((hasValue ? value : Math.max(value, placeholder)) + border);
}
