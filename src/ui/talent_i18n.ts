...
type DisplayGlobalKey = Exclude<GlobalKey, 'critVsRooted' | 'hotStreak'>;
...
> = {
  ...
  fr_FR: {
    improved: 'Amélioration de',
    greater: 'Supérieur',
    mastery: 'Maîtrise de',
    unknown: 'Talent',
  },
  it_IT: {
    improved: 'Miglioramento di',
    greater: 'Superiore',
    mastery: 'Maestria di',
    unknown: 'Talento',
  },
  ...
  nl_NL: {
    improved: 'Verbetering van',
    greater: 'Groter',
    mastery: 'Meesterschap van',
    unknown: 'Talent',
  },
  pl_PL: {
    improved: 'Ulepszenie:',
...
const localeTextByBase = {
  ...
  es: {
    ...
    hotStreak:
      'Tras dos golpes críticos de hechizo consecutivos, tu siguiente hechizo con tiempo de lanzamiento es instantáneo y gratuito.',
    ...
    addRoot: (abilityName, seconds) =>
      `${abilityName} también enraíza al objetivo durante ${seconds} s.`,
  ...
  fr_FR: {
    ...
    hotStreak:
      'Après deux coups critiques de sorts consécutifs, votre prochain sort à temps d’incantation est instantané et gratuit.',
    ...
    addRoot: (abilityName, seconds) =>
      `${abilityName} enracine aussi la cible pendant ${seconds} s.`,
    addAoeRoot: (abilityName, seconds) =>
  ...
  it_IT: {
    ...
    hotStreak:
      'Dopo due colpi critici di incantesimo consecutivi, il tuo prossimo incantesimo con tempo di lancio è istantaneo e gratuito.',
    ...
    addRoot: (abilityName, seconds) =>
      `${abilityName} immobilizza anche il bersaglio per ${seconds} s.`,
    addAoeRoot: (abilityName, seconds) =>
  ...
  de_DE: {
    ...
    hotStreak:
      'Nach zwei kritischen Zaubertreffern in Folge wird Euer nächster Zauber mit Zauberzeit sofort und kostenlos gewirkt.',
    ...
    addRoot: (abilityName, seconds) =>
      `${abilityName} verwurzelt das Ziel zusätzlich für ${seconds} Sek.`,
    addAoeRoot: (abilityName, seconds) =>
  ...
  zh_CN: {
    ...
    perRank: '/每级',
    noEffect: '提供一个专精增益。',
    hotStreak: '连续两次法术暴击后，你的下一个有施法时间的法术将立即施放且不消耗法力。',
  ...
  zh_TW: {
    ...
    perRank: '/每級',
    noEffect: '提供一個專精增益。',
    hotStreak: '連續兩次法術爆擊後，你的下一個有施法時間的法術將立即施放且不消耗法力。',
  ...
  ko_KR: {
    ...
    hotStreak:
      '주문 치명타가 연속으로 두 번 발생하면 다음에 시전 시간이 있는 주문이 즉시 시전되고 자원을 소모하지 않습니다.',
    ...
    addRoot: (abilityName, seconds) =>
      `${abilityName}이 대상을 ${seconds}초 동안 이동 불가로 만듭니다.`,
    addAoeRoot: (abilityName, seconds) =>
  ...
  ja_JP: {
    ...
    hotStreak:
      '呪文クリティカルが2回連続で発生すると、次の詠唱時間のある呪文が即時発動になり、コストもかかりません。',
  ...
  pt_BR: {
    ...
    hotStreak:
      'Após dois acertos críticos de feitiço consecutivos, seu próximo feitiço com tempo de lançamento é instantâneo e gratuito.',
  ...
  ru_RU: {
    ...
    hotStreak:
      'После двух критических ударов заклинаниями подряд ваше следующее заклинание со временем произнесения применяется мгновенно и бесплатно.',
    ...
    addRoot: (abilityName, seconds) => `${abilityName} также обездвиживает цель на ${seconds} сек.`,
    addAoeRoot: (abilityName, seconds) =>
...
export function hasTalentTitleOverride(lang: SupportedLanguage, source: string): boolean {
  return (
    abilityIdByName.has(source) ||
    translateChoiceRowTitle(source, lang) !== undefined ||
...
function effectDescription(
  ...
  for (const [key, value] of Object.entries(global) as [GlobalKey, GlobalModEffect[GlobalKey]][]) {
    if (value === undefined || value === 0) continue;
    if (key === 'hotStreak') {
      parts.push(text.hotStreak);
      continue;
    }
    if (key === 'critVsRooted') {
      parts.push(
        text.increase(
          text.statLabels.spellCritVsRooted,
          formatPercent(Number(value), lang),
          perRank,
        ),
      );
      continue;
    }
    parts.push(text.increase(text.statLabels[key], formatPercent(Number(value), lang), perRank));
  }
  ...
  for (const mod of effect.ability ?? []) {
    ...
    for (const added of mod.addEffects ?? []) {
      if (added.type === 'root') {
        ...
        parts.push(
          added.leechPct
            ? text.addLeechDot(name, amount, seconds)
            : text.addDot(name, amount, seconds),
        );
