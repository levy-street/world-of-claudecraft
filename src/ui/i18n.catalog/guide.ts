...
export const guideStrings = {
  ...
  professions: {
    ...
    // Intentional gathering PR1 reworded the press clause in place under that
    // same rule (the interact key takes the loot only; the Harvest section of
    // the loot window gathers), and re-filled all 18 overlays that carry the
    // key in this change. Everything else here (families, specimens, premium
    // eligibility, first come) is unchanged.
    harvestBodyFamilies:
  ...
  profPages: {
    ...
    faq: {
      ...
      // Intentional gathering PR1 reworded the first paragraph in place: the
      // one press no longer covers both, so the answer names the two actions
      // (interact / Take Loot for the loot, Harvest for the components). Under
      // the reword-staleness rule all 18 overlays that carry this key were
      // re-filled in the same change; the first-come and Town Focus paragraph
      // is untouched.
