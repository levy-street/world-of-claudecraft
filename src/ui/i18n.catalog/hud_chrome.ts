...
export const hudChromeStrings = {
  ...
  // Item-kind label for crafting materials (ore/bar/herb). The other eight kinds
  // live in the inline-per-locale items catalog (itemUi.kind.*); 'material' is
  // added here so it stays an English-only add (catalog-domain gotcha, CLAUDE).
  itemKind: {
    material: 'Material',
  },
  // Professions (gathering + crafting). English-only here; the sim emits English
  // and the HUD's localizeErrorText/localizeSystemText arms + the event handlers
  // re-localize through these keys (so no inline-per-locale BASE_DICT entries are
  // needed). Profession/rank names are resolved by id from `names`/`ranks`.
  professions: {
    // Window chrome
    title: 'Professions',
    open: 'Professions',
    ...
    // Difficulty colours (recipe/node skill-up odds)
    color: {
    ...
    // Profession display names, resolved by ProfessionId
    names: {
      mining: 'Mining',
      herbalism: 'Herbalism',
    ...
    // Rank-tier display names, resolved by rank id
    ranks: {
      apprentice: 'Apprentice',
      journeyman: 'Journeyman',
      expert: 'Expert',
    },
    // Event toasts (professionLearned / professionSkill events)
    learned: 'You have learned {prof}!',
    skillUp: '{prof} skill increased to {skill}.',
    // Sim-emitted feedback (re-localized in localizeErrorText/localizeSystemText)
