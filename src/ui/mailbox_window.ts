...
export class MailboxWindow {
    ...
    if (attachmentsRow) {
      ...
      for (const slot of opened.items) {
        ...
        if (item) {
          ...
          chip.innerHTML = `<span class="ui-socket ui-socket--bag">${this.deps.itemIcon(item, cell.quality)}${cell.qualityBadge}</span><span style="color:${cell.color}">${esc(cell.name)}${esc(stack)}</span>`;
  ...
  private renderParcels(): void {
    ...
    for (const [chipIdx, slot] of this.attachments.entries()) {
      ...
      name.tabIndex = 0;
      name.innerHTML = `<span class="ui-socket ui-socket--bag">${this.deps.itemIcon(item, cell.quality)}${cell.qualityBadge}</span><span style="color:${cell.color}">${esc(cell.name)}</span>`;
