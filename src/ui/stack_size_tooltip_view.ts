...
// hoarding), and a CHARGE-bearing instance payload (charges mutate in place,
// so bags.ts instancedCountCap holds such a slot to ONE copy regardless of
// the def cap, and quoting the def cap there would lie). A locked-but-uncharged
// payload is NOT in that last case: locking is one flag over the whole counted
// stack (item_lock.ts setItemLocked), so a locked stack still shares the def
// cap and the line stays accurate. The line exists to answer the question a
// player with ONE potion cannot answer any other way: there is no stack badge
// at quantity 1, so the tooltip is how they find out more copies will share
// the slot.

import { stackSizeOf } from '../sim/bags';
import { isChargeBearingPayload } from '../sim/item_instance_merge';
...
export function stackSizeTooltipLine(item: ItemDef, instance?: ItemInstancePayload): string {
  ...
  if (isChargeBearingPayload(instance)) return '';
