...
import { isCharterEligible, MOUNT_LIST, MOUNTS, type MountDef } from '../sim/content/mounts';
import {
  EVENT_SKIN_TIERS,
  MECH_CHROMAS,
  SKIN_RANKS,
...
export class Hud {
  ...
  constructor(
    ...
    this.courseHudEl.querySelector('.ch-abort')?.addEventListener('click', () => {
      this.sim.abortCourse();
      audio.click();
    });
    ...
    for (const ev of events) {
      ...
      switch (ev.type) {
        ...
        case 'courseStart':
          audio.click();
          break;
        case 'courseCheckpoint':
          audio.click();
          break;
        case 'courseFinish':
          break; // handled (audio + overlay) in updateCourseHud
        case 'courseFail':
          this.showBanner(
            t(ev.reason === 'dismounted' ? 'hud.course.failed' : 'hud.course.aborted'),
          );
          break;
        ...
        case 'wagerInvite': {
          ...
          this.showPrompt(
            t(key, {
              name: `<b>${esc(ev.fromName)}</b>`,
              money: formatLocalizedMoney(ev.anteCopper),
            }),
            t('hud.wager.accept'),
            () => this.sim.wagerJoin(),
            () => this.sim.wagerDecline(),
          );
          break;
        }
        case 'wagerSettled':
          if (ev.cancelled) this.showBanner(t('hud.wager.refunded'));
            ...
            this.showBanner(
              ev.charters > 0
                ? t('hud.wager.wonCharter', {
                    money: formatLocalizedMoney(ev.copper),
                    count: formatNumber(ev.charters),
                  })
          ...
          break;
        case 'wagerExpired':
          break; // the invite prompt auto-dismisses on its own timer
        case 'duelCountdown':
  ...
  renderMounts(): void {
    ...
    for (const m of MOUNT_LIST) {
      ...
      if (isUnlocked) {
        ...
        row.setAttribute(
          'aria-label',
          t('hud.mounts.lockedAria', { name: m.name, amount: fmt0(m.threshold) }),
        );
      }
    ...
    if (!onFlyer) {
      ...
      for (const c of COURSE_LIST) {
        ...
        const lapsText =
          c.laps > 1 ? t('hud.course.laps', { count: fmt0(c.laps) }) : t('hud.course.onePass');
  ...
  private onMountAction(m: MountDef, active: boolean): void {
    ...
    if (p.dead) {
      this.showError(t('hud.mounts.cantDead'));
      return;
    }
    if (p.inCombat) {
      this.showError(t('hud.mounts.cantInCombat'));
      return;
    }
    if (this.playerLikelySwimming(p)) {
      this.showError(t('hud.mounts.cantSwimming'));
      return;
    }
    if ((p.mountTier ?? 0) < m.tier) {
      this.showError(t('hud.mounts.notEligible'));
      return;
    }
  ...
  private openWagerSetup(courseId: string): void {
    ...
    const ownedCharter =
      this.sim.inventory.find((s) => s.itemId.startsWith('charter_') && s.count > 0)?.itemId ??
      null;
    ...
    if (ownedCharter)
      this.setText(charterRow.querySelector('span') as HTMLElement, t('hud.wager.stakeCharter'));
    ...
    freshOpen.addEventListener('click', () => {
      ...
      this.sim.proposeWagerRace(
        courseId,
        ante,
        charterCb.checked && ownedCharter ? ownedCharter : null,
      );
      audio.click();
      close();
    });
  ...
  private playerLikelySwimming(p: Entity): boolean {
    return (
      terrainHeight(p.pos.x, p.pos.z, this.sim.cfg.seed) < WATER_LEVEL - 0.8 &&
  ...
  private updateCourseHud(): void {
    ...
    if (run.state === 'active') {
      ...
      if (run.startTick > 0 && !this.courseTimerRunning) {
        this.courseTimerRunning = true;
        this.courseTimerStartMs = now;
      }
      ...
      const sub =
        run.startTick <= 0
          ...
          : def.laps > 1
            ...
            : t('hud.course.gate', {
                n: formatNumber(Math.min(passed + 1, total)),
    ...
    if (run.state === 'done') {
      ...
      if (now > this.courseFinishHoldUntil) {
        el.hidden = true;
        el.classList.remove('finished');
        return;
      }
      ...
      this.setText(
        el.querySelector('.ch-name') as HTMLElement,
        `${def.name} — ${t('hud.course.finish')}`,
      );
      this.setText(el.querySelector('.ch-time') as HTMLElement, formatRunTime(seconds));
      this.setText(
        el.querySelector('.ch-sub') as HTMLElement,
        isNewBest ? `${t('hud.course.newBest')} · ${parBit}` : parBit,
      );
      return;
    }
  ...
  private updateRaceHud(): void {
    ...
    const now = performance.now();
    if (info.state === 'done') {
      if (this.lastRaceState !== 'done') {
        this.lastRaceState = 'done';
        this.raceDoneHideAt = now + 6000;
      }
      if (now > this.raceDoneHideAt) {
        el.hidden = true;
        return;
      }
    } else {
    ...
    if (info.state === 'countdown') {
      title =
        info.countdown > 0
          ? `${t('hud.course.getReady')} ${formatNumber(info.countdown)}`
          : t('hud.course.go');
      ...
      title =
        me && me.place > 0
          ? t('hud.course.placeOf', {
              place: formatNumber(me.place),
              total: formatNumber(info.participants.length),
            })
          : t('hud.course.dnf');
    }
    this.setText(el.querySelector('.rh-title') as HTMLElement, title);
    (el.querySelector('.rh-list') as HTMLElement).innerHTML = info.participants
      .map((pp) => {
        const status = pp.dnf
          ? t('hud.course.dnf')
          : pp.done
        ...
        return (
          `<div class="rh-row${pp.me ? ' me' : ''}${pp.done ? ' done' : ''}${pp.dnf ? ' dnf' : ''}">` +
  ...
  private updateWagerHud(): void {
    ...
    this.setText(
      el.querySelector('.wh-pot') as HTMLElement,
      info.potCharters > 0
        ? t('hud.wager.potCharter', {
            money: formatLocalizedMoney(info.potCopper),
            count: formatNumber(info.potCharters),
          })
        : t('hud.wager.pot', { money: formatLocalizedMoney(info.potCopper) }),
    );
    const myId = this.sim.player?.id;
    (el.querySelector('.wh-list') as HTMLElement).innerHTML = info.members
      .map(
        (m) =>
          `<div class="wh-row${m.pid === info.hostPid ? ' host' : ''}${m.pid === myId ? ' me' : ''}">` +
          `<span class="wh-dot"></span><span class="wh-name">${esc(m.name)}</span></div>`,
      )
      .join('');
  ...
  private async renderCourseBoard(
