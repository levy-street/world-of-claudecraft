...
import { ACTION_BAR_SLOTS, type Keybinds } from '../game/keybinds';
...
export class Hud {
  ...
  private readonly optionsWindow = new OptionsWindow({
    ...
    isSecondBarEnabled: () => this.isSecondBarEnabled(),
    setSecondBarEnabled: (on) => this.setSecondBarEnabled(on),
    enterBindMode: () => this.enterBindMode(),
  });
  ...
  private clearActionDropTargets(): void {
    document
      .querySelectorAll('#actionbar .drop-target, #actionbar-2 .drop-target')
      .forEach((el) => {
