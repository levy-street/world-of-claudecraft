...
export const zh_CN: EnTranslations = {
  ...
  "hudChrome": {
    ...
    "options": {
      ...
      "shaderWarmOff": "关闭",
      "shaderWarmOn": "开启",
      "shaderWarmNote": "自动：在图形后端支持时（Direct3D 11、Vulkan）由后台工作线程提前链接着色器；在无法起作用的 OpenGL 上保持关闭。开启会在所有环境中强制使用工作线程，包括会让帧率变差的环境；关闭则完全不使用。下次启动游戏时生效。",
      "gpuBackend": "图形后端（Linux）",
      "gpuBackendAuto": "自动（尝试一次 Vulkan）",
      "gpuBackendVulkan": "Vulkan",
      "gpuBackendOpenGL": "OpenGL",
      "gpuBackendNote": "自动：启动时尝试一次 Vulkan，可用则保留，此时着色器加载造成的卡顿会大幅减少。驱动不支持时回退到 OpenGL。选过 Vulkan 或 OpenGL 之后再选回“自动”，会重新尝试一次。下次启动游戏时生效。",
