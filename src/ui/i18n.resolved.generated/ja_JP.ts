...
export const ja_JP: EnTranslations = {
  ...
  "guide": {
    ...
    "interfacePage": {
      ...
      "winMoreBodyNoValeCup": "ワールドマップ（M）、PvPウィンドウ（G）、リーダーボード（K）、イベントカレンダー（I）も、すべて同じように使えます。エモートホイール（X）だけは例外で、キーを押し続けるとホイールが現れ、エモートの上でキーを離すとそのエモートが再生されます。リーダーボードは初めて開くときに少し眺めてみる価値があります。プレイヤーのタブ、ギルドのタブ、功績の書の名声でアカウント全体を順位付けするタブ、デイリーの順位のタブ、そしてゲームを作っている人々のための「開発者」タブがあり、開発者タブは「開発者バッジを表示」をオフにしない限り表示されます。\n\nほかのプレイヤーをターゲットしてターゲットフレームを右クリックするか（タッチではダブルタップか長押し）、チャットでその名前を右クリックすると、「プレイヤー情報」がその人物のカードを開きます。身につけている装備がツールチップ付きで並び、キャラクターの公開情報も載ります。見るだけで、それ以上のことはできません。装備を見るには相手が見える距離にいる必要があります。遠く離れた相手の名前をチャットから調べると、代わりにカードの公開されている半分、ポートレート、名前、レベル、クラス、ギルドが表示されます。",
    ...
    "social": {
      ...
      "lootRollBodyNeedBeatsGreed": "アイテムがロールにかけられると、対象となる各メンバーは、欲しければニード、余りなら受け取る程度ならグリード、辞退するならパスを選びます。ニードはグリードに優先します。誰か一人でもニードを選べば、アイテムはニードの中で最も高い出目の人に渡り、グリードの出目は数えません。ニードが誰もいなければ、グリードの中で最も高い出目が勝ちます。",
