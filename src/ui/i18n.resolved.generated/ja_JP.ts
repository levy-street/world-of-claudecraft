...
export const ja_JP: EnTranslations = {
  ...
  "hudChrome": {
    ...
    "options": {
      ...
      "shaderWarmOff": "オフ",
      "shaderWarmOn": "オン",
      "shaderWarmNote": "自動：グラフィックスバックエンドが対応している場合（Direct3D 11、Vulkan）、バックグラウンドワーカーでシェーダーを事前にリンクします。効果のないOpenGLではオフのままです。オンはフレームが悪化する環境も含めてすべてでワーカーを強制し、オフは一切使いません。次回ゲームを起動したときに反映されます。",
      "gpuBackend": "グラフィックスバックエンド（Linux）",
      "gpuBackendAuto": "自動（Vulkanを一度試す）",
      "gpuBackendVulkan": "Vulkan",
      "gpuBackendOpenGL": "OpenGL",
      "gpuBackendNote": "自動：起動時にVulkanを一度試し、動作すればそのまま使います。そこではシェーダー読み込みによるカクつきが大きく減ります。ドライバーが対応していない場合はOpenGLに戻ります。VulkanまたはOpenGLを選んだあとで再び自動を選ぶと、もう一度試します。次回ゲームを起動したときに反映されます。",
