...
export const ja_JP: EnTranslations = {
  ...
  "guide": {
    ...
    "glossary": {
      ...
      "fiveSecondDef": "マナが自然に回復し始めるのは、最後にマナを使ってから5秒が過ぎてからです。キャスターが撃ちっぱなしにせず、配分を考えて詠唱するのはこのためです。",
      "talentRowTerm": "タレント行",
    ...
    "talentsPage": {
      ...
      "loadoutNote": "ビルドを一つに決めてしまう必要はありません。パネルで名前をつけた構成を保存すると、タレントの選択とアクションバーの両方が記憶されるので、別の構成へ移るのはクリック一回です。条件は振り直しと同じで、戦闘外であること、そしてアリーナの試合や戦場の最中でないことです。"
    },
    "arenaPage": {
      "heading": "アリーナとPvP",
    ...
    "profPages": {
      ...
      "gatherIntro": {
        ...
        "farming": "農耕は奪うのではなく世話をして育てる唯一の採集職です。手入れした畑に種をまけば留守の間も作物は育ち、戻ったときに実りを引き抜けます。Eastbrookの割当地からEvergardenの花壇まで、農耕の拠点ごとに専用の畑があり、はしごの段ごとに固有の作物が一対ずつ実って、腕が上がれば上質な等級も採れます。仕事の入口は鍬です。Garden Hoeが最初の畑を受け持ち、その上の段の鍬はエンジニアが作ります。熟練度は専用のカウンターで上限100まで数えられます。"
      },
