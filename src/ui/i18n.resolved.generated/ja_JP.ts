...
export const ja_JP: EnTranslations = {
  ...
  "hudChrome": {
    ...
    "auraEffect": {
      ...
      "arcaneCharges": "秘術チャージ（最大4）。チャージごとに次のエーテルサージのダメージが上昇し詠唱が5%速くなるが、マナコストが大きく増加する。エーテルダートは全チャージを消費する。",
      "aetherRush": "次のエーテルサージはコストがゼロになり、詠唱速度が2倍になる",
      "perfectMoment": "秘術チャージが4に固定される。エーテルダートはチャージを消費しなくなり、5連射のフルバラージュを連続で放てる",
      "temporalEcho": "より健やかな瞬間の残響を刻まれる：術者が与える秘術ダメージの一部が引き戻され、あなたを回復する",
      "increase": {
        "ap": "攻撃力を{value}上昇させる",
