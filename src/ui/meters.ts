...
export type LootHistoryEntry = Extract<SimEvent, { type: 'lootAwarded' }> & {
  seq: number;
  capturedAt: number;
};
...
export class MeterData {
  ...
  private onLootAward(ev: Extract<SimEvent, { type: 'lootAwarded' }>, now: number): void {
    this.lootHistory.unshift({ ...ev, seq: this.nextLootSeq++, capturedAt: now });
    if (this.lootHistory.length > LOOT_HISTORY_CAP) this.lootHistory.pop();
  }
...
type CombatTab = 'dmg' | 'heal' | 'threat';
type Tab = CombatTab | 'loot';

const TAB_LABEL_KEY: Record<CombatTab, TranslationKey> = {
  dmg: 'hud.meters.damage',
  heal: 'hud.meters.healing',
  threat: 'hud.meters.threat',
};
const TAB_SHORT_LABEL_KEY: Record<CombatTab, TranslationKey> = {
  dmg: 'hud.meters.damageShort',
  heal: 'hud.meters.healingShort',
  threat: 'hud.meters.threat',
};
const ALL_TABS: Tab[] = ['dmg', 'heal', 'threat', 'loot'];

function tabLabel(tab: Tab): string {
  return tab === 'loot' ? t('hud.meters.loot') : t(TAB_LABEL_KEY[tab]);
}

function tabShortLabel(tab: Tab): string {
  return tab === 'loot' ? t('hud.meters.lootShort') : t(TAB_SHORT_LABEL_KEY[tab]);
}
...
export class Meters {
  ...
  constructor(private world: IWorld) {
    ...
    for (const tab of ALL_TABS) {
      ...
      tabButton.textContent = tabShortLabel(tab);
  ...
  render(force = false): void {
    ...
    if (this.tab === 'loot') {
      this.renderLootHistory();
      return;
    }
    const { enc, viewName } = this.viewedEncounter();
    this.titleEl.textContent = t('hud.meters.title', { tab: tabLabel(this.tab), view: viewName });
    ...
    for (const { t, value } of rows) {
      ...
      fill.style.width = String(Math.max(4, (value / top) * 100)) + '%';
  ...
  private renderLootHistory(): void {
    this.titleEl.textContent = t('hud.meters.title', {
      tab: t('hud.meters.loot'),
      view: t('hud.meters.lootHistory'),
    });
    this.rowsEl.innerHTML = '';
    const rows = this.data.lootHistory;
    if (rows.length === 0) {
      this.subEl.textContent = t('hud.meters.noLoot');
      return;
    }
    this.subEl.textContent = t('hud.meters.recentAwards', {
      count: formatNumber(rows.length, { maximumFractionDigits: 0 }),
    });
    for (const entry of rows) {
      const row = document.createElement('div');
      row.className = 'mt-row mt-loot-row';

      const main = document.createElement('div');
      main.className = 'mt-loot-main';
      const item = ITEMS[entry.itemId];
      const name = document.createElement('span');
      name.className = 'mt-loot-item';
      name.style.color = QUALITY_COLOR[entry.quality ?? item?.quality ?? 'common'] ?? '#fff';
      const count =
        entry.count && entry.count > 1
          ? ' ' +
            t('itemUi.bags.stackCount', {
              count: formatNumber(entry.count, { maximumFractionDigits: 0 }),
            })
          : '';
      name.textContent = (item ? itemDisplayName(item) : entry.itemName) + count;
      const method = document.createElement('span');
      method.className = 'mt-loot-method';
      method.textContent = lootMethodLabel(entry);
      main.append(name, method);

      const detail = document.createElement('div');
      detail.className = 'mt-loot-detail';
      detail.textContent = lootDetailText(entry);
...
function lootMethodLabel(entry: LootHistoryEntry): string {
  if (entry.method === 'need') return t('itemUi.lootRoll.need');
  if (entry.method === 'greed') return t('itemUi.lootRoll.greed');
  if (entry.method === 'returned') return t('hud.meters.methodReturned');
  if (entry.method === 'master') return t('hud.meters.methodMaster');
  if (entry.method === 'personal') return t('hud.meters.methodPersonal');
  if (entry.method === 'delve') return t('hud.meters.methodDelve');
  return t('hud.meters.methodLooted');
}

function lootDetailText(entry: LootHistoryEntry): string {
  const actor = entry.winnerName ?? t('hud.meters.noWinner');
  const roll =
    entry.roll !== undefined
      ? ' - ' +
        t('hud.meters.rollDetail', {
          roll: formatNumber(entry.roll, { maximumFractionDigits: 0 }),
        })
      : '';
  const encounter = entry.encounterName ? ' - ' + entry.encounterName : '';
  return actor + roll + encounter;
}
