...
export const zh_TW: Partial<Record<TranslationKey, string>> = {
  ...
  'hudChrome.raidBossGuide.nythraxis.phaseThroneSummary': '尼思拉克西斯坐鎮王座廳，施展蓄力正面順劈、恐懼詛咒坦克輪換、穿刺團隊的骨刺、留下燃燒地面的墓穴爆發，以及一波波復甦王室衛兵。',
  ...
  'hudChrome.raidBossGuide.nythraxis.graveEruptionHeroicSummary': '每{everyHeroic}秒，骸骨之手會在{countHeroic}名團員腳下標記半徑{radius}碼的圓圈。{warning}秒後，每個圓圈都會爆發，造成相當於最大生命值{burstHeroic}的暗影傷害，隨後化為墓焰持續燃燒至該階段結束，對站在其中的人每秒造成相當於最大生命值{tickHeroic}的傷害。火焰不會自行熄滅：若團隊不持續移動，地面將逐漸被火海填滿。',
  ...
  'hudChrome.raidBossGuide.nythraxis.soulfireSummary': '每次靈魂撕裂引爆，都會在標記所在處留下一灘寬{radius}碼的赤紅烈焰，燃燒{seconds}秒，每秒造成相當於最大生命值{tickNormal}的傷害。火池絕不會在護符石{clearance}碼內形成。',
  'hudChrome.raidBossGuide.nythraxis.soulfireHeroicSummary': '每次靈魂撕裂引爆，都會在標記所在處留下一灘寬{radius}碼的赤紅烈焰，持續燃燒至該階段結束，每秒造成相當於最大生命值{tickHeroic}的傷害。火池絕不會在護符石{clearance}碼內形成，且不會自行熄滅。',
  'hudChrome.raidBossGuide.nythraxis.soulfireResponse': '每次施放後都應更換靈魂撕裂的集合點，避免團隊疊站在上一次施放留下的火焰中。',
