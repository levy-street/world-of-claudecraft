...
export const zh_TW: Partial<Record<TranslationKey, string>> = {
  ...
  'hudChrome.itemAccountBound': '帳號綁定',
  'hudChrome.mailbox.clickAttachAccountBound': '點擊以附到信件上。帳號綁定：只有你自己的角色才能收到它。',
  'hudChrome.mailbox.result.noMailAccountBound': '帳號綁定的物品只能郵寄給你自己的角色。',
  'hudChrome.trade.woc.ineligibleReasonAccountBound': '帳號綁定物品、任務物品、已鎖定物品以及交易所類別之外的物品都無法出售換取 $WOC。',
  'guide.glossary.accountBoundTerm': '帳號綁定',
  'guide.glossary.accountBoundDef': '自你取得之時起便與你的帳號綁定的物品。它無法交易、賣給商人或在市場上架，而信件只能把它送到你自己的另一個角色手中。',
  'guide.gear.accountBoundTitle': '帳號綁定：在你的任何角色身上都屬於你',
  'guide.gear.accountBoundBody': '少數特殊獎勵是帳號綁定的，從你贏得它們的那一刻起便與你的帳號相連。帳號綁定的物品無法交易、賣給商人、在市場上架或存入公會銀行，寄給其他玩家的信件也不會帶上它。唯一敞開的門是你自己角色之間的渡鴉郵驛：一個角色不再需要的綁定裝備可以寄給同一帳號下的另一個角色，職業專屬裝備也包括在內，這樣合適職業的分身就能穿上它。這守護著英雄印記之類的獎勵代幣，也涵蓋了用榮譽購買的每一件戰爭裝備。你在世界中贏得的大多數裝備仍然可以自由交易、出售或分享。',
  'guide.economy.mailHowAccountBound': '收信的方式正好相反：站在任何石柱旁閱讀你的信件，把它們所載之物收進你的錢袋與背包。普通信件過一陣子便會消失，但仍附有錢幣或貨物的信件會一直等著你，無論你多久才來。有幾樣東西郵驛一概拒收：任務物品、已綁定或將隨交易綁定的物品，以及獨一無二的外觀代幣，它們要麼隨你同行，要麼哪兒也去不了；而帳號綁定的物品，渡鴉只會把它送到你自己的另一個角色那裡。另外，繳交一個漂亮的任務之後，記得留意石柱；有些任務給予者會寫信。',
  'guide.economy.buyingBodyAccountBound': '與商人交談並選擇瀏覽他的貨品，他的商店便會以單一面板開啟：他所有的存貨列在一處，只要你負擔得起，點一下便能買下。貨品上方的數量列決定每次點擊買多少，一件、五件或十件，也可以自訂數量，不過少數特殊商品（坐騎便是其中之一）一次只賣一件。可堆疊的錢幣標價商品在該列旁還有第二個選項，可一次買下你的錢幣所能負擔的數量，最多一整疊。出售同樣直接：商店開著時，點擊背包中的物品即可當場賣出，而商人不收的東西，例如任務物品和帳號綁定的物品，會原地留下。若你割捨了某樣讓你後悔的東西，商店會保留你近期售出物的買回清單，讓你能以當初賣得的錢幣把它們買回來。',
  'guide.social.guildBankRulesBodyAccountBound': '物品倉庫一開始是關閉的。由一位幹部自掏腰包開通，之後公會可以從公會金庫出錢把它擴大。任務物品、任何帳號綁定的東西，以及市場不收的裝備都進不了它，這和市場的規矩相同，因此公會銀行是給那些仍能在玩家之間易手的貨物用的。',
  'guide.arenaPage.honorFinalNoteAccountBound': '榮譽購買一經完成便無法反悔。以錢幣買下的東西還能從商人的買回清單裡取消，但榮譽購買永遠不會出現在那裡，而戰爭裝備在你買下的那一刻便帳號綁定，因此它永遠無法交易，也無法賣回換取任何東西，只能寄給你自己的另一個角色。商店會因此請你再確認一次：按下去之前，先把那件裝備看清楚。',
};
