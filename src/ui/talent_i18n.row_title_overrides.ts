import type { SupportedLanguage } from './i18n';

// Canonical non-grant choice-row titles. PR #1756 deliberately treats these
// fantasy names as proper names in non-CJK locales; the four CJK locales carry
// authored native titles because an untranslated Latin title is unreadable in
// those trees. Winning-Warrior titles and Master Armorer are localized in every
// release-tier locale. Keep this data separate from title-routing logic.
const RETAINED_ROW_TITLE_SOURCES = [
  'Double Charge',
  'Pursuit',
  'Crushing Charge',
  'Second Wind',
  'Lingering Dread',
  'Anger Management',
  'Combat Mastery',
  'Battle Rhythm',
  'Bloodbath',
  'Colossal Might',
  'Master Armorer',
  "Dawn's Reply",
  "Pilgrim's Light",
  'Ashen Sentence',
  'Twin Gavels',
  'Hallowed Snare',
  'Third Benediction',
  "Vigil's Refrain",
  'Afterglow Aegis',
  "Oath's Due",
  'Oathwheel',
  "Rite's Afterglow",
  'Venom Relay',
  'Twin Fletching',
  'Guisecraft',
  'Pinning Barb',
  "Mender's Signal",
  'Second Bearing',
  'Viperfletch',
  'Bloodbond',
  'Rainbreak',
  'Redline Draw',
  "Knife's Dividend",
  'Dusk Dividend',
  'Blindside Opening',
  'Paid in Pain',
  'Second Exit',
  'Borrowed Tempo',
  'Venom Dividend',
  'Ghostfoot Gambit',
  'Borrowed Breath',
  'Redline Habit',
  'First Cut, Last Word',
  'Third Verse',
  'Warding Refrain',
  'Dirgebound Thought',
  'Shattered Psalm',
  'Measured Mercy',
  'Gloam Siphon',
  'Mercy Deferred',
  'Last Blessing',
  'Halo Aftershock',
  'Rebounding Current',
  'Imbued Lifeblood',
  'Fault Rebuke',
  'Rime Lock',
  'Returning Current',
  'Elemental Attunement',
  'Cinder Rupture',
  'Imbued Tempo',
  'Tideflow',
  'Stonewake Shell',
  'Undertow Promise',
  'Cinder Reprise',
  'Ember Relay',
  'Third Current',
  'Aetherwell',
  'Rime Ambush',
  "Witch's Opening",
  'Brittle Moment',
  'Slow Burn',
  'Dartstride',
  'Glassward Reflex',
  'Grave Rhythm',
  'Blacktide',
  'Pact Reversal',
  'Blood Credit',
  'Walking Hunger',
  'Fiendward',
  'Ashen Relay',
  'Shadow Credit',
  'Cruel Awakening',
  'Hellglass Ward',
  'Hexstorm',
  'Moonkindle',
  'Typhoon',
  "Bloom's End",
  'Briar Ambush',
  'Brutal Bash',
  'Formrush',
  'Grove Covenant',
  'Redtooth Rhythm',
  'Moonspite',
  'Mercy Seed',
  'Oaken Reflex',
  'Ironhide Reflex',
  'Storm Refrain',
  'Red Haze',
  'Tempest Reprise',
] as const;

type RetainedRowTitle = (typeof RETAINED_ROW_TITLE_SOURCES)[number];
type TitleMap = Readonly<Record<RetainedRowTitle, string>>;

const ORIGINAL_TITLES = Object.freeze(
  Object.fromEntries(RETAINED_ROW_TITLE_SOURCES.map((title) => [title, title])),
) as TitleMap;

const WARRIOR_TITLES: Partial<Record<SupportedLanguage, Partial<TitleMap>>> = {
  es: {
    'Double Charge': 'Carga doble',
    Pursuit: 'Persecución',
    'Crushing Charge': 'Carga aplastante',
    'Second Wind': 'Segundo aliento',
    'Lingering Dread': 'Pavor persistente',
    'Anger Management': 'Control de la ira',
    'Combat Mastery': 'Maestría de combate',
    'Battle Rhythm': 'Ritmo de batalla',
    Bloodbath: 'Baño de sangre',
    'Colossal Might': 'Poder colosal',
    'Master Armorer': 'Maestro armero',
  },
  es_ES: {
    'Double Charge': 'Carga doble',
    Pursuit: 'Persecución',
    'Crushing Charge': 'Carga aplastante',
    'Second Wind': 'Segundo aliento',
    'Lingering Dread': 'Pavor persistente',
    'Anger Management': 'Control de la ira',
    'Combat Mastery': 'Maestría de combate',
    'Battle Rhythm': 'Ritmo de batalla',
    Bloodbath: 'Baño de sangre',
    'Colossal Might': 'Poder colosal',
    'Master Armorer': 'Maestro armero',
  },
  fr_FR: {
    'Double Charge': 'Double charge',
    Pursuit: 'Poursuite',
    'Crushing Charge': 'Charge écrasante',
    'Second Wind': 'Second souffle',
    'Lingering Dread': 'Terreur persistante',
    'Anger Management': 'Gestion de la colère',
    'Combat Mastery': 'Maîtrise du combat',
    'Battle Rhythm': 'Rythme de bataille',
    Bloodbath: 'Bain de sang',
    'Colossal Might': 'Puissance colossale',
    'Master Armorer': 'Maître armurier',
  },
  fr_CA: {
    'Double Charge': 'Double charge',
    Pursuit: 'Poursuite',
    'Crushing Charge': 'Charge écrasante',
    'Second Wind': 'Second souffle',
    'Lingering Dread': 'Terreur persistante',
    'Anger Management': 'Gestion de la colère',
    'Combat Mastery': 'Maîtrise du combat',
    'Battle Rhythm': 'Rythme de bataille',
    Bloodbath: 'Bain de sang',
    'Colossal Might': 'Puissance colossale',
    'Master Armorer': 'Maître armurier',
  },
  it_IT: {
    'Double Charge': 'Carica doppia',
    Pursuit: 'Inseguimento',
    'Crushing Charge': 'Carica devastante',
    'Second Wind': 'Secondo respiro',
    'Lingering Dread': 'Terrore persistente',
    'Anger Management': 'Gestione della rabbia',
    'Combat Mastery': 'Maestria del combattimento',
    'Battle Rhythm': 'Ritmo di battaglia',
    Bloodbath: 'Bagno di sangue',
    'Colossal Might': 'Potenza colossale',
    'Master Armorer': 'Mastro armaiolo',
  },
  de_DE: {
    'Double Charge': 'Doppelter Sturmangriff',
    Pursuit: 'Verfolgung',
    'Crushing Charge': 'Zermalmender Sturmangriff',
    'Second Wind': 'Zweiter Wind',
    'Lingering Dread': 'Anhaltender Schrecken',
    'Anger Management': 'Wutkontrolle',
    'Combat Mastery': 'Kampfmeisterschaft',
    'Battle Rhythm': 'Kampfrhythmus',
    Bloodbath: 'Blutbad',
    'Colossal Might': 'Kolossale Macht',
    'Master Armorer': 'Waffenmeister',
  },
  pt_BR: {
    'Double Charge': 'Carga Dupla',
    Pursuit: 'Perseguição',
    'Crushing Charge': 'Carga Esmagadora',
    'Second Wind': 'Segundo Fôlego',
    'Lingering Dread': 'Pavor Persistente',
    'Anger Management': 'Controle da Raiva',
    'Combat Mastery': 'Maestria de Combate',
    'Battle Rhythm': 'Ritmo de Batalha',
    Bloodbath: 'Banho de Sangue',
    'Colossal Might': 'Poder Colossal',
    'Master Armorer': 'Mestre Armeiro',
  },
  ru_RU: {
    'Double Charge': 'Двойной рывок',
    Pursuit: 'Погоня',
    'Crushing Charge': 'Сокрушительный рывок',
    'Second Wind': 'Второе дыхание',
    'Lingering Dread': 'Тянущийся ужас',
    'Anger Management': 'Управление гневом',
    'Combat Mastery': 'Боевое мастерство',
    'Battle Rhythm': 'Боевой ритм',
    Bloodbath: 'Кровавая баня',
    'Colossal Might': 'Колоссальная мощь',
    'Master Armorer': 'Мастер-оружейник',
    "Dawn's Reply": 'Ответ рассвета',
    "Vigil's Refrain": 'Припев дозора',
    "Oath's Due": 'Долг клятвы',
    'Tempest Reprise': 'Повтор бури',
    "Mender's Signal": 'Сигнал лекаря',
    'Second Bearing': 'Повторная наводка',
    Rainbreak: 'Прорыв сквозь ливень',
    'Venom Dividend': 'Ядовитый дивиденд',
    'Redline Habit': 'Привычка на пределе',
    'Dusk Dividend': 'Сумеречный дивиденд',
    'Ember Relay': 'Передача углей',
    Aetherwell: 'Эфирный колодец',
    Typhoon: 'Тайфун',
    'Grove Covenant': 'Завет рощи',
    'Red Haze': 'Багровая дымка',
    'Last Blessing': 'Последнее благословение',
    Tideflow: 'Приливный поток',
  },
  cs_CZ: {
    'Double Charge': 'Dvojitý výpad',
    Pursuit: 'Pronásledování',
    'Crushing Charge': 'Drtivý výpad',
    'Second Wind': 'Druhý dech',
    'Lingering Dread': 'Přetrvávající děs',
    'Anger Management': 'Ovládání hněvu',
    'Combat Mastery': 'Bojové mistrovství',
    'Battle Rhythm': 'Bojový rytmus',
    Bloodbath: 'Krvavá lázeň',
    'Colossal Might': 'Kolosální síla',
    'Master Armorer': 'Mistr zbrojíř',
  },
  nl_NL: {
    'Double Charge': 'Dubbele stormloop',
    Pursuit: 'Achtervolging',
    'Crushing Charge': 'Verpletterende stormloop',
    'Second Wind': 'Tweede adem',
    'Lingering Dread': 'Aanhoudende angst',
    'Anger Management': 'Woedebeheersing',
    'Combat Mastery': 'Gevechtsmeesterschap',
    'Battle Rhythm': 'Gevechtsritme',
    Bloodbath: 'Bloedbad',
    'Colossal Might': 'Kolossale macht',
    'Master Armorer': 'Meesterwapensmid',
  },
  pl_PL: {
    'Double Charge': 'Podwójna szarża',
    Pursuit: 'Pościg',
    'Crushing Charge': 'Miażdżąca szarża',
    'Second Wind': 'Drugi oddech',
    'Lingering Dread': 'Długotrwały lęk',
    'Anger Management': 'Kontrola gniewu',
    'Combat Mastery': 'Mistrzostwo walki',
    'Battle Rhythm': 'Rytm walki',
    Bloodbath: 'Krwawa łaźnia',
    'Colossal Might': 'Kolosalna potęga',
    'Master Armorer': 'Mistrz płatnerz',
  },
  id_ID: {
    'Double Charge': 'Serbuan Ganda',
    Pursuit: 'Pengejaran',
    'Crushing Charge': 'Serbuan Penghancur',
    'Second Wind': 'Napas Kedua',
    'Lingering Dread': 'Kengerian Membekas',
    'Anger Management': 'Pengendalian Amarah',
    'Combat Mastery': 'Penguasaan Tempur',
    'Battle Rhythm': 'Ritme Pertempuran',
    Bloodbath: 'Mandi Darah',
    'Colossal Might': 'Kekuatan Kolosal',
    'Master Armorer': 'Ahli Senjata',
  },
  tr_TR: {
    'Double Charge': 'Çifte Hücum',
    Pursuit: 'Takip',
    'Crushing Charge': 'Ezici Hücum',
    'Second Wind': 'İkinci Nefes',
    'Lingering Dread': 'Süren Dehşet',
    'Anger Management': 'Öfke Kontrolü',
    'Combat Mastery': 'Savaş Ustalığı',
    'Battle Rhythm': 'Savaş Ritmi',
    Bloodbath: 'Kan Banyosu',
    'Colossal Might': 'Devasa Kudret',
    'Master Armorer': 'Silah Ustası',
  },
  sv_SE: {
    'Double Charge': 'Dubbel stormning',
    Pursuit: 'Förföljelse',
    'Crushing Charge': 'Krossande stormning',
    'Second Wind': 'Andra andningen',
    'Lingering Dread': 'Kvardröjande skräck',
    'Anger Management': 'Ilskekontroll',
    'Combat Mastery': 'Stridsmästerskap',
    'Battle Rhythm': 'Stridsrytm',
    Bloodbath: 'Blodbad',
    'Colossal Might': 'Kolossal kraft',
    'Master Armorer': 'Vapenmästare',
  },
  vi_VN: {
    'Double Charge': 'Xung Phong Kép',
    Pursuit: 'Truy Đuổi',
    'Crushing Charge': 'Xung Phong Nghiền Nát',
    'Second Wind': 'Hồi Sức',
    'Lingering Dread': 'Nỗi Kinh Hoàng Dai Dẳng',
    'Anger Management': 'Kiểm Soát Cơn Giận',
    'Combat Mastery': 'Tinh Thông Chiến Đấu',
    'Battle Rhythm': 'Nhịp Điệu Chiến Đấu',
    Bloodbath: 'Tắm Máu',
    'Colossal Might': 'Sức Mạnh Khổng Lồ',
    'Master Armorer': 'Bậc Thầy Vũ Khí',
  },
  da_DK: {
    'Double Charge': 'Dobbelt stormløb',
    Pursuit: 'Forfølgelse',
    'Crushing Charge': 'Knusende stormløb',
    'Second Wind': 'Andet åndedrag',
    'Lingering Dread': 'Dvælende rædsel',
    'Anger Management': 'Vredeshåndtering',
    'Combat Mastery': 'Kampmesterskab',
    'Battle Rhythm': 'Kamprytme',
    Bloodbath: 'Blodbad',
    'Colossal Might': 'Kolossal styrke',
    'Master Armorer': 'Våbenmester',
  },
};

const CJK_TITLES: Partial<Record<SupportedLanguage, Partial<TitleMap>>> = {
  zh_CN: {
    'Double Charge': '双重冲锋',
    Pursuit: '追击',
    'Crushing Charge': '碾压冲锋',
    'Second Wind': '复苏之风',
    'Lingering Dread': '余惧',
    'Anger Management': '愤怒管理',
    'Combat Mastery': '战斗精通',
    'Battle Rhythm': '战斗节奏',
    Bloodbath: '浴血',
    'Colossal Might': '巨像之力',
    'Master Armorer': '军械大师',
    "Dawn's Reply": '黎明回应',
    "Pilgrim's Light": '朝圣者之光',
    'Ashen Sentence': '灰烬判词',
    'Twin Gavels': '双槌律令',
    'Hallowed Snare': '圣域禁足',
    'Third Benediction': '三叠圣祷',
    "Vigil's Refrain": '守望复唱',
    'Afterglow Aegis': '余晖神盾',
    "Oath's Due": '誓约之偿',
    Oathwheel: '誓轮流转',
    "Rite's Afterglow": '仪式余晖',
    'Venom Relay': '毒脉接续',
    'Twin Fletching': '双羽箭术',
    Guisecraft: '化形秘术',
    'Pinning Barb': '钉身倒刺',
    "Mender's Signal": '疗愈号令',
    'Second Bearing': '二次校准',
    Viperfletch: '蝰羽箭艺',
    Bloodbond: '血契羁绊',
    Rainbreak: '破雨突击',
    'Redline Draw': '赤弦极限',
    "Knife's Dividend": '刀口分赃',
    'Dusk Dividend': '暮影分赃',
    'Blindside Opening': '死角先机',
    'Paid in Pain': '以痛偿付',
    'Second Exit': '第二退路',
    'Borrowed Tempo': '借势节拍',
    'Venom Dividend': '毒酿分赃',
    'Ghostfoot Gambit': '鬼步赌局',
    'Borrowed Breath': '借命一息',
    'Redline Habit': '搏命恶习',
    'First Cut, Last Word': '首刃定终局',
    'Third Verse': '第三诗节',
    'Warding Refrain': '护佑复唱',
    'Dirgebound Thought': '挽歌缚念',
    'Shattered Psalm': '碎裂圣诗',
    'Measured Mercy': '有度慈悲',
    'Gloam Siphon': '暮色汲取',
    'Mercy Deferred': '慈悲暂寄',
    'Last Blessing': '最后祝福',
    'Halo Aftershock': '光轮余震',
    'Rebounding Current': '反涌雷流',
    'Imbued Lifeblood': '附灵命血',
    'Fault Rebuke': '裂地斥责',
    'Rime Lock': '霜锁',
    'Returning Current': '灵流回返',
    'Elemental Attunement': '元素调和',
    'Cinder Rupture': '烬痕爆裂',
    'Imbued Tempo': '灵契节律',
    Tideflow: '潮汐',
    'Stonewake Shell': '岩醒之壳',
    'Undertow Promise': '暗流之诺',
    'Cinder Reprise': '烬火复奏',
    'Ember Relay': '余烬接续',
    'Third Current': '三叠秘流',
    Aetherwell: '以太之井',
    'Rime Ambush': '寒霜伏袭',
    "Witch's Opening": '巫术先机',
    'Brittle Moment': '脆裂一瞬',
    'Slow Burn': '缓燃',
    Dartstride: '秘矢疾步',
    'Glassward Reflex': '琉璃瞬护',
    'Grave Rhythm': '墓冢节律',
    Blacktide: '黑潮',
    'Pact Reversal': '契约逆转',
    'Blood Credit': '血契赊账',
    'Walking Hunger': '饥渴随行',
    Fiendward: '邪魔护障',
    'Ashen Relay': '烬火接续',
    'Shadow Credit': '暗影赊账',
    'Cruel Awakening': '残酷惊醒',
    'Hellglass Ward': '狱璃护障',
    Hexstorm: '邪咒风暴',
    Moonkindle: '月辉初燃',
    Typhoon: '台风',
    "Bloom's End": '花期终章',
    'Briar Ambush': '荆棘伏袭',
    'Brutal Bash': '野蛮猛击',
    Formrush: '化形奔袭',
    'Grove Covenant': '林地盟约',
    'Redtooth Rhythm': '赤牙节律',
    Moonspite: '月怨',
    'Mercy Seed': '慈悲之种',
    'Oaken Reflex': '橡木瞬应',
    'Ironhide Reflex': '铁甲瞬应',
    'Storm Refrain': '风暴复唱',
    'Red Haze': '赤红迷雾',
    'Tempest Reprise': '风暴复奏',
  },
  zh_TW: {
    'Double Charge': '雙重衝鋒',
    Pursuit: '追擊',
    'Crushing Charge': '碾壓衝鋒',
    'Second Wind': '復甦之風',
    'Lingering Dread': '餘懼',
    'Anger Management': '憤怒管理',
    'Combat Mastery': '戰鬥精通',
    'Battle Rhythm': '戰鬥節奏',
    Bloodbath: '浴血',
    'Colossal Might': '巨像之力',
    'Master Armorer': '軍械大師',
    "Dawn's Reply": '黎明回應',
    "Pilgrim's Light": '朝聖者之光',
    'Ashen Sentence': '灰燼判詞',
    'Twin Gavels': '雙槌律令',
    'Hallowed Snare': '聖域禁足',
    'Third Benediction': '三疊聖禱',
    "Vigil's Refrain": '守望複唱',
    'Afterglow Aegis': '餘暉神盾',
    "Oath's Due": '誓約之償',
    Oathwheel: '誓輪流轉',
    "Rite's Afterglow": '儀式餘暉',
    'Venom Relay': '毒脈接續',
    'Twin Fletching': '雙羽箭術',
    Guisecraft: '化形祕術',
    'Pinning Barb': '釘身倒刺',
    "Mender's Signal": '療癒號令',
    'Second Bearing': '二次校準',
    Viperfletch: '蝰羽箭藝',
    Bloodbond: '血契羈絆',
    Rainbreak: '破雨突擊',
    'Redline Draw': '赤弦極限',
    "Knife's Dividend": '刀口分贓',
    'Dusk Dividend': '暮影分贓',
    'Blindside Opening': '死角先機',
    'Paid in Pain': '以痛償付',
    'Second Exit': '第二退路',
    'Borrowed Tempo': '借勢節拍',
    'Venom Dividend': '毒釀分贓',
    'Ghostfoot Gambit': '鬼步賭局',
    'Borrowed Breath': '借命一息',
    'Redline Habit': '搏命惡習',
    'First Cut, Last Word': '首刃定終局',
    'Third Verse': '第三詩節',
    'Warding Refrain': '護佑複唱',
    'Dirgebound Thought': '輓歌縛念',
    'Shattered Psalm': '碎裂聖詩',
    'Measured Mercy': '有度慈悲',
    'Gloam Siphon': '暮色汲取',
    'Mercy Deferred': '慈悲暫寄',
    'Last Blessing': '最後祝福',
    'Halo Aftershock': '光輪餘震',
    'Rebounding Current': '反湧雷流',
    'Imbued Lifeblood': '附靈命血',
    'Fault Rebuke': '裂地斥責',
    'Rime Lock': '霜鎖',
    'Returning Current': '靈流回返',
    'Elemental Attunement': '元素調和',
    'Cinder Rupture': '燼痕爆裂',
    'Imbued Tempo': '靈契節律',
    Tideflow: '潮汐',
    'Stonewake Shell': '岩醒之殼',
    'Undertow Promise': '暗流之諾',
    'Cinder Reprise': '燼火複奏',
    'Ember Relay': '餘燼接續',
    'Third Current': '三疊祕流',
    Aetherwell: '乙太之井',
    'Rime Ambush': '寒霜伏襲',
    "Witch's Opening": '巫術先機',
    'Brittle Moment': '脆裂一瞬',
    'Slow Burn': '緩燃',
    Dartstride: '祕矢疾步',
    'Glassward Reflex': '琉璃瞬護',
    'Grave Rhythm': '墓塚節律',
    Blacktide: '黑潮',
    'Pact Reversal': '契約逆轉',
    'Blood Credit': '血契賒帳',
    'Walking Hunger': '飢渴隨行',
    Fiendward: '邪魔護障',
    'Ashen Relay': '燼火接續',
    'Shadow Credit': '暗影賒帳',
    'Cruel Awakening': '殘酷驚醒',
    'Hellglass Ward': '獄璃護障',
    Hexstorm: '邪咒風暴',
    Moonkindle: '月輝初燃',
    Typhoon: '颱風',
    "Bloom's End": '花期終章',
    'Briar Ambush': '荊棘伏襲',
    'Brutal Bash': '野蠻猛擊',
    Formrush: '化形奔襲',
    'Grove Covenant': '林地盟約',
    'Redtooth Rhythm': '赤牙節律',
    Moonspite: '月怨',
    'Mercy Seed': '慈悲之種',
    'Oaken Reflex': '橡木瞬應',
    'Ironhide Reflex': '鐵甲瞬應',
    'Storm Refrain': '風暴複唱',
    'Red Haze': '赤紅迷霧',
    'Tempest Reprise': '風暴複奏',
  },
  ja_JP: {
    'Double Charge': '二連突撃',
    Pursuit: '追撃',
    'Crushing Charge': '粉砕突撃',
    'Second Wind': 'セカンドウィンド',
    'Lingering Dread': '残る恐怖',
    'Anger Management': '怒りの管理',
    'Combat Mastery': '戦闘の極意',
    'Battle Rhythm': '戦の拍子',
    Bloodbath: '血の饗宴',
    'Colossal Might': '巨像の力',
    'Master Armorer': '武具の達人',
    "Dawn's Reply": '暁の応え',
    "Pilgrim's Light": '巡礼者の灯',
    'Ashen Sentence': '灰燼の宣告',
    'Twin Gavels': '双槌の裁き',
    'Hallowed Snare': '聖域の戒縛',
    'Third Benediction': '三度目の祝祷',
    "Vigil's Refrain": '見張りの反唱',
    'Afterglow Aegis': '残光の守護',
    "Oath's Due": '誓約の報い',
    Oathwheel: '誓約輪転',
    "Rite's Afterglow": '儀式の残光',
    'Venom Relay': '毒矢の継走',
    'Twin Fletching': '双矢の仕立て',
    Guisecraft: '獣装の妙技',
    'Pinning Barb': '縫い留めの棘',
    "Mender's Signal": '癒やしの合図',
    'Second Bearing': '再照準',
    Viperfletch: '毒蛇の矢羽',
    Bloodbond: '血の絆',
    Rainbreak: '雨裂き',
    'Redline Draw': '死線の引き絞り',
    "Knife's Dividend": '刃の配当',
    'Dusk Dividend': '宵闇の配当',
    'Blindside Opening': '死角の好機',
    'Paid in Pain': '痛みの代価',
    'Second Exit': '第二の逃げ道',
    'Borrowed Tempo': '借りた律動',
    'Venom Dividend': '毒の配当',
    'Ghostfoot Gambit': '幽歩の賭け',
    'Borrowed Breath': '借り物の命',
    'Redline Habit': '死線の習性',
    'First Cut, Last Word': '初太刀、最後の言葉',
    'Third Verse': '第三の聖句',
    'Warding Refrain': '守護の反唱',
    'Dirgebound Thought': '葬歌に囚われた思念',
    'Shattered Psalm': '砕け散る聖詠',
    'Measured Mercy': '律された慈悲',
    'Gloam Siphon': '薄闇の吸命',
    'Mercy Deferred': '留め置く慈悲',
    'Last Blessing': '最後の祝福',
    'Halo Aftershock': '光輪の余震',
    'Rebounding Current': '跳ね返る電流',
    'Imbued Lifeblood': '付呪の命脈',
    'Fault Rebuke': '断層の叱責',
    'Rime Lock': '霧氷の封縛',
    'Returning Current': '還流',
    'Elemental Attunement': '元素調律',
    'Cinder Rupture': '熾火の破裂',
    'Imbued Tempo': '付呪の律動',
    Tideflow: '潮流',
    'Stonewake Shell': '目覚める岩殻',
    'Undertow Promise': '底流の約束',
    'Cinder Reprise': '熾火の反唱',
    'Ember Relay': '残り火の継承',
    'Third Current': '第三の奔流',
    Aetherwell: 'エーテルの井戸',
    'Rime Ambush': '霧氷の奇襲',
    "Witch's Opening": '魔女の好機',
    'Brittle Moment': '脆き一瞬',
    'Slow Burn': '遅燃',
    Dartstride: '魔矢の歩法',
    'Glassward Reflex': '玻璃結界の即応',
    'Grave Rhythm': '墓場の律動',
    Blacktide: '黒き潮',
    'Pact Reversal': '契約の逆転',
    'Blood Credit': '血の貸し',
    'Walking Hunger': '歩く飢餓',
    Fiendward: '悪鬼の結界',
    'Ashen Relay': '灰燼の連唱',
    'Shadow Credit': '影の貸し',
    'Cruel Awakening': '残酷な目覚め',
    'Hellglass Ward': '地獄硝子の結界',
    Hexstorm: '呪嵐',
    Moonkindle: '月の火種',
    Typhoon: 'タイフーン',
    "Bloom's End": '花散りの恵み',
    'Briar Ambush': '茨の奇襲',
    'Brutal Bash': '残忍な強打',
    Formrush: '変身の猛進',
    'Grove Covenant': '木立の盟約',
    'Redtooth Rhythm': '赤牙の律動',
    Moonspite: '月の怨嗟',
    'Mercy Seed': '慈悲の種',
    'Oaken Reflex': '樫皮の即応',
    'Ironhide Reflex': '鉄皮の即応',
    'Storm Refrain': '嵐の反唱',
    'Red Haze': '赤き霞',
    'Tempest Reprise': '嵐の再演',
  },
  ko_KR: {
    'Double Charge': '이중 돌진',
    Pursuit: '추격',
    'Crushing Charge': '분쇄 돌진',
    'Second Wind': '재기의 바람',
    'Lingering Dread': '남은 공포',
    'Anger Management': '분노 조절',
    'Combat Mastery': '전투 숙련',
    'Battle Rhythm': '전투 박자',
    Bloodbath: '피의 향연',
    'Colossal Might': '거인의 힘',
    'Master Armorer': '무기 장인',
    "Dawn's Reply": '새벽의 응답',
    "Pilgrim's Light": '순례자의 빛',
    'Ashen Sentence': '잿빛 선고',
    'Twin Gavels': '쌍둥이 심판봉',
    'Hallowed Snare': '성역의 결박',
    'Third Benediction': '세 번째 축도',
    "Vigil's Refrain": '경계의 후렴',
    'Afterglow Aegis': '잔광의 수호',
    "Oath's Due": '맹세의 대가',
    Oathwheel: '맹세의 수레바퀴',
    "Rite's Afterglow": '의식의 잔광',
    'Venom Relay': '맹독 연계',
    'Twin Fletching': '쌍화살깃',
    Guisecraft: '야수 형상술',
    'Pinning Barb': '옭아매는 가시',
    "Mender's Signal": '치유의 신호',
    'Second Bearing': '재조준',
    Viperfletch: '독사의 화살깃',
    Bloodbond: '피의 결속',
    Rainbreak: '빗줄기 돌파',
    'Redline Draw': '한계의 만작',
    "Knife's Dividend": '칼날의 배당',
    'Dusk Dividend': '황혼의 배당',
    'Blindside Opening': '사각의 틈',
    'Paid in Pain': '고통의 대가',
    'Second Exit': '두 번째 퇴로',
    'Borrowed Tempo': '빌린 박자',
    'Venom Dividend': '맹독의 배당',
    'Ghostfoot Gambit': '유령걸음의 승부수',
    'Borrowed Breath': '빌린 목숨',
    'Redline Habit': '사선의 버릇',
    'First Cut, Last Word': '첫 베기, 마지막 말',
    'Third Verse': '세 번째 성구',
    'Warding Refrain': '수호의 후렴',
    'Dirgebound Thought': '장송곡에 묶인 사념',
    'Shattered Psalm': '부서지는 성가',
    'Measured Mercy': '절제된 자비',
    'Gloam Siphon': '땅거미의 흡혼',
    'Mercy Deferred': '미뤄 둔 자비',
    'Last Blessing': '마지막 축복',
    'Halo Aftershock': '후광의 여진',
    'Rebounding Current': '되튀는 전류',
    'Imbued Lifeblood': '주입된 생명혈',
    'Fault Rebuke': '단층의 질책',
    'Rime Lock': '서리 봉쇄',
    'Returning Current': '돌아오는 전류',
    'Elemental Attunement': '원소 조율',
    'Cinder Rupture': '잉걸불 파열',
    'Imbued Tempo': '주입된 박자',
    Tideflow: '조수',
    'Stonewake Shell': '깨어난 돌껍질',
    'Undertow Promise': '저류의 약속',
    'Cinder Reprise': '잉걸불의 재현',
    'Ember Relay': '불씨 연계',
    'Third Current': '세 번째 마력류',
    Aetherwell: '에테르샘',
    'Rime Ambush': '서리 기습',
    "Witch's Opening": '마녀의 틈',
    'Brittle Moment': '깨질 듯한 순간',
    'Slow Burn': '서서히 타오름',
    Dartstride: '마탄 보법',
    'Glassward Reflex': '유리 결계의 즉응',
    'Grave Rhythm': '무덤의 율동',
    Blacktide: '검은 파도',
    'Pact Reversal': '계약의 역전',
    'Blood Credit': '피의 외상',
    'Walking Hunger': '걸어 다니는 허기',
    Fiendward: '악마의 결계',
    'Ashen Relay': '잿불의 연계',
    'Shadow Credit': '그림자의 외상',
    'Cruel Awakening': '잔혹한 각성',
    'Hellglass Ward': '지옥 유리의 결계',
    Hexstorm: '사술폭풍',
    Moonkindle: '달빛 불씨',
    Typhoon: '태풍',
    "Bloom's End": '낙화의 은총',
    'Briar Ambush': '가시덤불 기습',
    'Brutal Bash': '잔혹한 강타',
    Formrush: '변신 돌진',
    'Grove Covenant': '수풀의 맹약',
    'Redtooth Rhythm': '붉은 송곳니의 율동',
    Moonspite: '달의 원한',
    'Mercy Seed': '자비의 씨앗',
    'Oaken Reflex': '참나무껍질의 즉응',
    'Ironhide Reflex': '무쇠가죽의 즉응',
    'Storm Refrain': '폭풍의 후렴',
    'Red Haze': '붉은 안개',
    'Tempest Reprise': '폭풍의 재현',
  },
};

export const RETAINED_ROW_TITLE_OVERRIDES: Partial<
  Record<SupportedLanguage, Readonly<Record<string, string>>>
> = {
  es: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.es },
  es_ES: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.es_ES },
  fr_FR: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.fr_FR },
  fr_CA: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.fr_CA },
  it_IT: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.it_IT },
  de_DE: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.de_DE },
  pt_BR: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.pt_BR },
  ru_RU: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.ru_RU },
  cs_CZ: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.cs_CZ },
  nl_NL: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.nl_NL },
  pl_PL: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.pl_PL },
  id_ID: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.id_ID },
  tr_TR: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.tr_TR },
  sv_SE: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.sv_SE },
  vi_VN: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.vi_VN },
  da_DK: { ...ORIGINAL_TITLES, ...WARRIOR_TITLES.da_DK },
  zh_CN: { ...ORIGINAL_TITLES, ...CJK_TITLES.zh_CN },
  zh_TW: { ...ORIGINAL_TITLES, ...CJK_TITLES.zh_TW },
  ja_JP: { ...ORIGINAL_TITLES, ...CJK_TITLES.ja_JP },
  ko_KR: { ...ORIGINAL_TITLES, ...CJK_TITLES.ko_KR },
};
