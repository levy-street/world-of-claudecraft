 ...
 *  policy's threshold. An unknown quality reads as common, and so does a poor
 *  copy that is NOT true junk (instanced or crafted): it confirms at the
 *  default threshold exactly as before, and clears only a raised one. */
export function vendorSaleNeedsConfirm(
  ...
  const rank = Math.max(QUALITY_RANK[item.quality ?? 'common'], QUALITY_RANK.common);
