...
export class ActionBarController {
  ...
  isHotbarItemId(itemId: string): boolean {
    ...
    // not placeable (elixirs are the precedent that riding useItem does not imply
    // a slot, though their reason differs): a pattern is a one-shot unlock consumed
    // on its first successful use, so a hotbar slot would hold a dead button from
    // the first press on; the bags are its home.
