...
//
// The SVG glyph is a procedural "commit graph" device (a trunk with commit nodes
// and merged branches that grows richer per rung), deliberately distinct from the
// holder coins/gems and the Discord numerals; a reskin with bespoke art can swap
// the glyphs later without touching the ladder.
import {
  DEV_TIER_DEFS,
  type DevTierCore,
  type DevTierKey,
...
export interface DevTier extends Omit<DevTierCore, 'key'> {
  ...
  key: DevTierKey;
  /** Minimum landed-commit count to reach this rung. */
  threshold: number;
...
// polyline with a commit node at its midpoint. `sign` picks the side (-1 left,
// +1 right). Pure string builder, font-free so it rasterises crisply.
function commitBranch(cx: number, top: number, bot: number, sign: number): string {
  const bx = cx + sign * 16;
...
// A commit-graph glyph: a vertical trunk with `nodes` commit dots (the topmost an
...
// glance.
function commitGraphGlyph(nodes: number, branches: number): string {
  const cx = branches === 1 ? 39 : 32;
  const top = 13;
  const bot = 53;
  let out = `<line x1="${cx}" y1="${top}" x2="${cx}" y2="${bot}" stroke="${GLYPH_FILL}" stroke-width="3.2" stroke-linecap="round"/>`;
  if (branches >= 1) out += commitBranch(cx, top, bot, -1);
  if (branches >= 2) out += commitBranch(cx, top, bot, 1);
...
const DEV_TIER_PRESENTATION: Record<DevTierKey, DevTierPresentation> = {
  tinkerer: {
    name: 'Tinkerer',
    flavor: 'Your first commit landed in the realm.',
    ring: '#9aa7b4',
    glow: '#5d6b78',
    glyph: commitGraphGlyph(2, 0),
  },
  artificer: {
    name: 'Artificer',
    flavor: 'Ten commits in, and the world bends to your code.',
    ring: '#5aa9e6',
    glow: '#2f6fb0',
    glyph: commitGraphGlyph(3, 0),
  },
  runesmith: {
    name: 'Runesmith',
    flavor: 'Fifty commits forged into the running game.',
    ring: '#9b6cff',
    glow: '#6a37e0',
    glyph: commitGraphGlyph(4, 1),
  },
  architect: {
    name: 'Architect',
    flavor: 'An architect of the realm: 150 commits landed.',
    ring: '#ffd24a',
    glow: '#e0a52a',
    glyph: commitGraphGlyph(5, 1),
  },
  worldwright: {
    name: 'Worldwright',
    flavor: 'A wright of worlds: 500 commits shape the game.',
    ring: '#ffe9a8',
    glow: '#ffaa00',
    glyph: commitGraphGlyph(5, 2),
  },
};
 ...
 * ring->glow radial, the commit-graph glyph centred on top. Suitable for an <img>
