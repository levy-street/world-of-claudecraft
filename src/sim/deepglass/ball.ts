...
// ---------------------------------------------------------------------------
// Spin. The Tidesow is a ball in water, and a spinning ball in water CURVES —
// which is the whole reason spin is here rather than as decoration. Every shot
 ...
 * question — "is the ball inside the body's sphere at the END of the tick?" —
 * and at 20 Hz a struck ball hops two-and-a-half yards per tick while a boosting
...
/**
 * Resolve a contact between the ball and a body. ALWAYS changes the ball —
 * which is the whole contract: a fighter the ball has touched is a fighter the
...
export function applyBodyContact(
  ...
  if (dash) {
    ...
    // plus a slice of the closing speed. Allowed up to the burst ceiling —
    // this and the wall pinch are the two hits that break the cruise cap.
