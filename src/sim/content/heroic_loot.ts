...
export const HEROIC_BOSS_LOOT: Record<string, LootEntry[]> = {
  ...
  [IGNIVAR_BOSS_ID]: [
    // Four Heroic gear rows, one guaranteed: the three marquee weapons plus
    // Wand of Quenched Sparks, redistributed from Ignivar's Normal off-set
    // partition on 2026-09-07. Three weapons, not four before the wand:
    // Emberflight Longbow waits for the hunter ranged-slot rework.
    { itemId: 'forgefathers_warhammer', chance: 0.25, rollGroup: 'ignivar_h_exclusive' },
    { itemId: 'anvilguard_blade', chance: 0.25, rollGroup: 'ignivar_h_exclusive' },
    { itemId: 'springtouched_crozier', chance: 0.25, rollGroup: 'ignivar_h_exclusive' },
    { itemId: 'wand_of_quenched_sparks', chance: 0.25, rollGroup: 'ignivar_h_exclusive' },
  ...
  [VARKHUL_BOSS_ID]: [
    // Shields 0.50 / marquee weapons 0.40 / held offhands 0.10. Emberward
    // keeps its ABSOLUTE 3 percent per heroic kill inside the shield share
    // (the two epic shields split the remaining 0.47 evenly). The held
    // offhands came over from Varkhul's Normal off-set partition and took
    // their 0.10 from the marquee weapons. Every weight is chosen so the
    // group sums to EXACTLY 1 in floating point (tests/ignivar_loot.test.ts).
    ...
    { itemId: 'heart_of_the_end_greatblade', chance: 0.14, rollGroup: 'varkhul_h_exclusive' },
    { itemId: 'forgefire_spire', chance: 0.13, rollGroup: 'varkhul_h_exclusive' },
    { itemId: 'staff_of_the_last_spring', chance: 0.13, rollGroup: 'varkhul_h_exclusive' },
