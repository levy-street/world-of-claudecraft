...
export class Sim {
  ...
  addPlayer(
    ...
    // inside the reserved arena/delve window [ARENA_X_MIN, DUNGEON_BAND_RESUME_X_MIN)
    // (which includes the delve band), so the dungeon branch's `?? DUNGEON_LIST[0]`
    // fallback would otherwise swallow a delve position and eject the player to a
    // dungeon door instead of the board door (FR-1.6). The two bands are disjoint,
    // so `else if` keeps dungeon handling intact.
