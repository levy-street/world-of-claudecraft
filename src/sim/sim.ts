...
import { isDemonTowerSeed } from './content/rift/demon_tower';
...
export class Sim {
  ...
  private spawnBossAdds(boss: Entity, mobId: string, count: number): void {
    ...
    const riftTuning = riftInst
      ? isDemonTowerSeed(riftInst.seed)
    ...
    for (let k = 0; k < spawnCount; k++) {
      ...
      if (riftTuning) {
        add.rangedDamageMult = add.mechanicDamageMult = riftTuning.addDamageMultiplier;
