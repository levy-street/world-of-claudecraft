...
export class Sim {
  ...
  private effectiveArmor(e: Entity): number {
    ...
    // Sunder Armor (2% per stack, up to 10% at 5 stacks) and Faerie Fire (a flat
    // 10%) max-combine, so a fully-stacked Sunder and a Faerie Fire are redundant
    // rather than additive. Mob corrosion (Acid Spit) is a separate FLAT shred that
    ...
    for (const a of e.auras) {
      ...
      // in recalc.
      if (e.kind !== 'player' && a.kind === 'buff_armor_pct') armor += (baseArmor * a.value) / 100;
      ...
      if (a.kind === 'faerie_fire') reductionPct = Math.max(reductionPct, FAERIE_FIRE_ARMOR_PCT);
    }
