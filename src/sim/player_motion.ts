...
export function stepPlayerMotion(deps: PlayerMotionDeps, p: Entity, inp: MoveInput): void {
  ...
  if (slide || movingOnGround || (!p.onGround && (p.vx !== 0 || p.vz !== 0))) {
    ...
    if (p.onGround && !swimming) {
      // Gate from where the mover actually STANDS, not the static field under
      // them: a racer riding a Hodric's Castle platform stands at deck height
      // over the chasm, and stepping onto the equal-height landing must not read
      // as climbing a 40 yd wall. Scoped to the Hodric's band (like the
      // isSwimming/deepWater/fall-damage siblings) so it never touches the
      // universal climb gate: everywhere else a grounded mover has pos.y ==
      // ground, making the Math.max a no-op.
      const h0 = isHodricsPos(p.pos.x)
        ? Math.max(groundHeight(p.pos.x, p.pos.z, deps.seed), p.pos.y)
        : groundHeight(p.pos.x, p.pos.z, deps.seed);
  ...
  // Vertical: jumping, gravity, swimming, fall damage. The Hodric's Castle chasm
  // is dry: falls there run to the kill plane (checkpoint respawn), they never
  // splash into phantom water.
  const ground = groundHeight(p.pos.x, p.pos.z, deps.seed);
  const deepWater = !isHodricsPos(p.pos.x) && ground < waterLevelAt(p.pos.x, p.pos.z) - SWIM_DEPTH;
  ...
  if (!p.onGround) {
    ...
    if (p.pos.y <= ground) {
      ...
      // damage sources at all (falls respawn at a checkpoint), so a hard landing
      // after a hammer yeet never stings. Everywhere else, fall damage applies as
      // normal.
