...
export type AbilityEffect =
  ...
  // allies (the most injured within range of the previous target, never repeating one),
  // each hop healing `falloff` of the previous hop's amount. Range field: mainline uses
  // `radius`, the warrior-foundation branch authored some rows with `jumpRange`; the
  // dispatch honors either, so both are optional here.
  | {
      type: 'chainHeal';
      min: number;
      max: number;
