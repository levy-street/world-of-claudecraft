...
| `source_cave/` | the runtime Source Cave dungeon (GitHub-contributors raid): roster spec + synthesized `MobTemplate`s (`runtime.ts`, the pure leaf the guide-safe UI imports), enter/claim/leave over the shared instance pool (`dungeon.ts`), encirclement waves + guardian wake + seal, the well banter gate, mob banter, the reboot button, and clear/lockout/loot; excluded from the generic instance-reset lifecycle (its own lockout rules) |
