...
- `day_night.ts`: the world day/night cycle's period and phase math (`DAY_NIGHT_CYCLE_MS`, `cyclePhase`, `isDaylightPhase`, `crossedDawn`), a pure leaf the renderer's `day_night_core.ts` re-exports so the sky's dawn and a sleeping boss's dawn are one definition. The sim never reads a clock: the host injects one through `SimConfig.dayNightNowMs`, read only via `ctx.dayNightPhase()` (null on a clockless host, which every nocturnal rule treats as permanent day).
...
| `mob/` behavior siblings | a new mob behavior is another sibling the dispatcher routes to, never a branch inside `locomotion.ts`: `ambient.ts` (decorative wanderers, e.g. the Highwatch stable horses: never hostile, never combat), `charge.ts` (the heroic anti-kite gap closer, stamped only on HEROIC spawns, zero rng), `healer_channel.ts` (scripted interruptible mob channels), `dragonkin_brood.ts` + `egg_hatchling.ts`, `idle_rng.ts`, `chain_pull_transit.ts` (with `instances/boss_chain_pull.ts`), `boss_collateral.ts` (`splashNearbyMobs`: a world boss's telegraphed blasts hurt every OTHER CREATURE standing in them, not just players, because a fist that craters the road while the boars in the crater are untouched reads as a set piece rather than a monster loose in the world; template opt-in `MobTemplate.collateral`, and it takes the ability's AUTHORED midpoint rather than rolling, so the number of bystanders in a blast draws no rng and cannot fork the world), `boss_slams.ts` (the two AIMED boss attacks plus the shared `launchFromSlam`: a one-fist HAMMER dropped on a snapshot of where a player stood, dodged by moving, and a low CLEAVE arc dragged across the ground, dodged by JUMPING and nothing else, since the arm passes at shin height. `launchFromSlam` is called from every telegraphed detonation and PUNTS its victims into the air rather than sliding them, running the shove through `ctx.applyKnockback` first so it inherits every cliff/wall/water/immunity rule. Template opt-ins `MobTemplate.slams` and `MobTemplate.launch`, both inert without them), `warpath.ts` (a boss who walks a CIRCUIT of authored landmarks instead of parking in melee range: plants and fights, runs to the next stop ignoring threat while backhanding whoever is near and regenerating if unpunished, then slams the landmark behind a telegraph ring; template opt-in `MobTemplate.warpath`, inert without it, and it drags the leash anchor with it so the tether follows the circuit rather than yanking him home mid-run), `slumber.ts` (a mob who keeps hours: at dusk, once out of combat, he walks home to his spawn point and lies down NEUTRAL (unattackable through the one `isHostileTo` rule, never a new damage branch), and wakes at dawn with a yell and a realm-wide log, then holds the AI for `slumber.riseSeconds` while the body stands up (hostile throughout, so the skate of a wander step under the wake clip never happens); template opt-in `MobTemplate.slumber`, inert without it AND inert on any host with no day/night clock, since night comes only from `ctx.dayNightPhase()`) |
...
| `world_boss.ts` | world bosses: the spawn registry (fixed points, hourly cadence, participant HP scaling), announce, contributor tracking, personal loot (`rollWorldBossLoot`, including the `LootEntry.maxPlayerLevel` gate a zone boss pays its low-level locals through), per-boss loot lockouts. The scheduler on `Sim` also reads `MobTemplate.slumber`: a slain slumbering boss rises again at the next DAWN of the host day/night clock rather than on the interval |
