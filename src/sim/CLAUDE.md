...
| `stylist.ts` | the Stylist's counter: buying a character-redesign CREDIT for gold (`buyRedesignCredit`), following `learnRiding`'s shape (a gold-charging service that grants a `PlayerMeta` field and hands over no item). The credit is bound to the character and never enters the bags, so it cannot be traded, mailed, or listed on the World Market; it is SPENT at char-select through the redesign endpoint, not here. Validate-funds / deduct / increment all happen in the dispatching tick, so nothing awaits between the balance compare and the writes. Band prices are data: `content/redesign_pricing.ts` |
