...
| `party_gate.ts` | the mage Grand Portal and the warlock Hellgate: party-gated travel objects on the Soulwell doctrine (eligibility captured at cast, late joiners via the party hook); a portal moves an eligible clicker to its destination, a gate pulls the owner's targeted ally to it, and `updatePartyGates` (head of `runDespawnDecay`) ends a Hellgate and its toll with its warlock; draws NO rng |
