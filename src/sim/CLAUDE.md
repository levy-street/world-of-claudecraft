...
| `durability.ts` + `durability_rules.ts` | gear durability, the classic repair-bill gold sink (`docs/design/durability-repair.md`): the leaf owns the pool sizes, the current-value read, `isBrokenGear` (consumed by `entity.ts` `recalcPlayerStats` beside the over-level gate) and the `5c x ilvl x missing` bill; the ctx module is the ONE writer of a worn copy's `ItemInstancePayload.durability` (the death penalty from `combat/damage.ts` `handleDeath`, the Spirit Healer surcharge from `spirit.ts`, and the vendor `repairAllGear` command). Durability lives on the COPY (present only while damaged, stripped by repair), never on a per-slot character table; draws NO rng |
