...
function launchTargetLeapArc(
  ...
  const baseVerticalSpeed = Math.sqrt(Math.max(0, 2 * RETREAT_LEAP_GRAVITY * height));
  const expectedAirTime = Math.max(
    Math.max(0.1, duration),
    baseVerticalSpeed > 0 ? (2 * baseVerticalSpeed) / RETREAT_LEAP_GRAVITY : 0.1,
  );
  const targetVerticalSpeed =
    (targetY - p.pos.y + 0.5 * RETREAT_LEAP_GRAVITY * expectedAirTime * expectedAirTime) / expectedAirTime;
  const verticalSpeed = Math.max(baseVerticalSpeed, targetVerticalSpeed);
  const speed = Math.max(0, distance) / expectedAirTime;
  clearChargeState(p);
  clearAbilityDash(p);
  p.followTargetId = null;
  p.vx = Math.sin(facing) * speed;
  p.vz = Math.cos(facing) * speed;
  p.vy = Math.max(p.vy, verticalSpeed);
  p.onGround = false;
  p.jumping = true;
  p.fallStartY = Math.max(p.fallStartY, p.pos.y);
}
...
export function runEffects(
  ...
  for (const eff of res.effects) {
    switch (eff.type) {
      ...
      case 'targetLeap': {
        if (!target || target.dead || isRooted(p)) break;
        const dx = target.pos.x - p.pos.x;
        const dz = target.pos.z - p.pos.z;
        const dist = Math.hypot(dx, dz);
        const travel = Math.min(eff.distance, Math.max(0, dist - eff.stopDistance));
        if (travel <= 0.1 || dist <= 0.1) break;
        const facing = Math.atan2(dx, dz);
        p.facing = facing;
        p.prevFacing = facing;
        launchTargetLeapArc(p, facing, travel, eff.height, eff.duration, target.pos.y);
        ctx.emit({ type: 'spellfx', sourceId: p.id, targetId: target.id, school: ability.school, fx: 'projectile' });
        ctx.enterCombat(p, target);
        break;
      }
      case 'retreatLeap': {
