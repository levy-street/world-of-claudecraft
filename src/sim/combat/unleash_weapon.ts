...
// scales with Attack Power, then adds a rider keyed to whichever enchant the
// shaman is running, so the one button changes with the enchant:
...
export function runUnleashWeapon(
  ...
  const anchored = p.auras.some((a) => a.kind === 'earthbound_weapon');
  ...
  let dmg =
    ctx.rng.range(min, max) + directHitBonus(ctx.effectiveAttackPower(p), ability, ability.castTime);
  const crit = ctx.rng.chance(consumeNextAttackCrit(ctx, p) ? 1 : ctx.spellCrit(p));
  if (crit) dmg *= 1.5 + p.critDmgSpellBonus;
  ctx.dealDamage(p, target, Math.round(dmg), crit, school, ability.name, 'hit', false, undefined, true);
