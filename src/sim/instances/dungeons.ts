...
//
// The occupancy walk treats an Ignivar room's linked family as one unit (a member
// three rooms deep still blocks resetting the lift), but WHICH claims land in
// `resettable` is still computed per room, by content coincidence rather than
// family logic: every Ignivar room happens to sit in HEROIC_DUNGEON_IDS and a
// family always holds one shared difficulty (enterDungeon's checkpoint chain
// enforces that), so today a family is always either fully resettable or not
// resettable at all. A future room added to the chain without a heroic record
// would break that coincidence and needs this comment (and likely a family-wide
// `resettable` walk) revisited in the same change.
export function resetDungeonInstances(ctx: SimContext, pid?: number): void {
  ...
  for (const inst of resettable) {
    ...
    if (isIgnivarRaidRoom(inst.dungeonId) && ignivarPreviousRaidRoom(inst.dungeonId) !== null) {
      continue;
    }
