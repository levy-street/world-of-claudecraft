...
function decodeValue(reader: Reader, limits: SnapshotBinaryLimits, depth: number): unknown {
  ...
  if (tag === TAG_OBJECT) {
    ...
    for (let index = 0; index < length; index++) {
      ...
      value[key] = decodeValue(reader, limits, depth + 1);
    }
    return value;
  }
...
function encodeKnownObject(
  ...
  for (const key of Object.keys(value)) {
    if (!known.has(key)) extensions[key] = value[key];
  }
...
function decodeKnownObject(
  ...
  for (let index = 0; index < keys.length; index++) {
    if ((mask & (1 << index)) !== 0) {
      value[keys[index]!] = decodeValue(reader, limits, 1);
    }
  }
