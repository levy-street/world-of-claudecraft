// Host calendar shared by the offline browser and authoritative server.
//
...
//
// This lives outside src/sim/ on purpose: the reset zone is a host /
// wall-clock concern (like the lockout clock itself), so the deterministic sim core
// never reads the host time zone database. Each host computes the next-reset instant
 ...
 * daily boundary.
 */
const weeklyResetMemo = new Map<string, { from: number; until: number }>();
...
export function nextWeeklyRaidResetMs(
  ...
  const key = `${zone}:${weekday}`;
  const cached = weeklyResetMemo.get(key);
  if (cached && nowMs >= cached.from && nowMs < cached.until) return cached.until;
  const { y, mo, d } = zoneDate(nowMs, zone);
  for (let offset = 0; offset <= 7; offset += 1) {
    const candidate = zoneResetInstant(y, mo, d + offset, zone);
    if (candidate > nowMs && zoneWeekday(candidate, zone) === weekday) {
      if (weeklyResetMemo.size >= 16) weeklyResetMemo.clear();
      weeklyResetMemo.set(key, { from: nowMs, until: candidate });
      return candidate;
    }
  }
