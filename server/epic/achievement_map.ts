/*
 * Maps a Book of Deeds deed id to its Epic (EOS / EGS) achievement API id for
 * the server-side mirror that pushes unlocks to Epic Online Services.
 *
 * Stability contract (D14): once an achievement id has shipped to the Epic
 * Developer Portal it is PERMANENT. Renaming one orphans every player's
 * existing Epic unlock for that achievement, so entries here may be ADDED but
 * never renamed or reused. Treat a shipped achievement id the way you treat a
 * persisted id.
 *
 * At most 100 entries: matches the Steam App Admin soft cap used by the twin
 * map, and is a practical portal authoring bound for v1. Hidden deeds may
 * legitimately appear in this map; they are registered as hidden achievements
 * in the Epic portal.
 *
 * Launch set: the same deed set the Steam map ships, with the same ACH_* id
 * strings so portal authoring can keep one id vocabulary across storefronts.
 * Partner-site / portal registration of these ids is a maintainer release task
 * (Phase 8), not a code gate.
 */
...
export const ACHIEVEMENT_MAP: Record<string, string> = {
  ...
  // Professions 2.0 marquee deeds: the three titled milestones join the mirror.
...
export const MAX_EPIC_ACHIEVEMENTS = 100;
