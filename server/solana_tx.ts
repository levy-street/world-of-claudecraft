...
// primitive every $WOC commerce path reuses (rename burns here; future paid
// features build on the same checks). We read finalized transactions over raw
// Solana JSON-RPC (no @solana/web3.js in the server bundle, mirroring
// server/woc_balance.ts) and expose small, composable checks: token-balance
// deltas, burn amounts, memo presence, and a Token-2022 guard.
//
...
// Legacy SPL Token program. We pin to this and reject Token-2022 ($WOC is a
// legacy-program mint; a Token-2022 look-alike with transfer hooks or fees
// must never be accepted as payment).
...
/**
 * Latest finalized blockhash, for handing the client everything it needs to
 * build the burn tx without its own RPC endpoint (the RPC URL, and any API key
 * embedded in it, stays server-side). Null on any failure.
 */
export async function getLatestBlockhash(): Promise<string | null> {
  try {
    ...
    return typeof bh === 'string' && bh.length > 0 ? bh : null;
  } catch {
    return null;
  }
}
 ...
 */
export async function largestTokenAccountForOwner(
  owner: string,
  mint: string,
): Promise<string | null> {
  try {
    ...
    const rows: any[] = Array.isArray(r?.value) ? r.value : [];
    ...
    for (const row of rows) {
      ...
      if (typeof pubkey !== 'string' || typeof amt !== 'string' || !/^[0-9]+$/.test(amt)) continue;
      const amount = BigInt(amt);
      if (!best || amount > best.amount) best = { pubkey, amount };
    }
