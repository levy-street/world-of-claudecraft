...
import {
  allocateWagerMatchId,
  cancelStaleAwaitingMatches,
  insertWagerMatch,
  recordWagerStake,
  recoverableLockedMatches,
  setWagerPhase,
} from './arena_wager_db';
import { ArenaWagerService } from './arena_wager_service';
import { walletForAccount } from './db';
import { FlowLedger } from './flow_ledger';
import { activeSeasonStatus, PgFlowLedgerDb } from './flow_ledger_db';
...
export async function buildArenaWagerService(
  ...
  if (!programIdStr || !mintStr || !process.env.WOC_ARENA_SETTLER_SECRET) {
    throw new Error(
      'WOC_ARENA_WAGER_ENABLED=1 requires WOC_ESCROW_PROGRAM_ID, WOC_MINT, and WOC_ARENA_SETTLER_SECRET',
    );
  }
  ...
  const escrow = new ArenaEscrow({
    ...
    settler,
    ledger: new FlowLedger(new PgFlowLedgerDb()),
    seasonId: pinnedSeasonId,
    rakeBps,
  });
  const store: WagerStore = {
    insert: insertWagerMatch,
    recordStake: recordWagerStake,
    setPhase: setWagerPhase,
  };
  ...
  return new ArenaWagerService({
    ...
    // were taken under.
    seasonActive: async () =>
      (await activeSeasonStatus())?.seasonId === pinnedSeasonId && pinnedSeasonId !== 0,
    escrow,
    store,
    loadRecoverable: recoverableLockedMatches,
    cancelStale: cancelStaleAwaitingMatches,
  });
}
