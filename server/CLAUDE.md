...
| `arena_season_db.ts` / `arena_season_settlement.ts` / `arena_season_records.ts` / `arena_season.ts` | Arena seasons (`docs/design/arena-seasons.md`): the season SQL boundary (entrants, 2v2 pairs, the award ledger, the exactly-once settlement markers) / the pure champion picker plus the self-clocked settlement driver / the fire-and-forget bout observer and its pure elect-one-writer rule / the anonymous `GET /api/arena/seasons` readout. The server DECIDES a champion; the sim still writes the deed, replayed at join through `Sim.addPlayer`'s `arenaSeasonTitles` |
