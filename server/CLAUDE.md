...
| `account_reliquary.ts` / `account_reliquary_db.ts` | the account-bound Reliquary's server half: `AccountReliquaryFold` keeps one live ledger per online account, folds a character's catalogued fills into it at join (`remember` widens from the stored read), on every non-retro `reliquaryUnlock` (`detectActivity`), and on the 60s standing sweep (`refreshCuratorStanding`), and persists ONLY growth through a per-account serial writer; `curatorStandingFor` is the identity-wire `crk`/`cro`/`crt` stamp. Storage is `account_relics` (account, kind, relic id), insert-only `ON CONFLICT DO NOTHING` so realms never race, catalog-bounded per account (no retention sweep). `loadAccountCosmeticsWithRelics` is what `ws_auth` hands the join |
