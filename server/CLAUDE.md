...
| `character_redesign.ts` | the PAID character redesign (`POST /api/characters/:id/redesign`): spends ONE Stylist credit on a new authored look. Registry-only, with its own runtime injection and db seam; it borrows from `characters.ts` only the two things that must not be duplicated (the `activeGuard` auth gate, and `parseAppearanceBody`, the SAME validator initial creation runs, because looks render to everyone). The decrement is a CONDITIONAL single-statement update (`db.ts` `consumeRedesignCredit`, `WHERE credits >= 1`), so a concurrent double-submit yields one redesign and one clean 400. Online characters are allowed: all three live-session mirrors fire, and the CREDIT one is load-bearing (without it the session's 30 s autosave refunds the spend). Distinct from the free one-shot reroll that stays in `characters.ts` |
