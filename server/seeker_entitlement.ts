...
const REAL_RUNTIME: SeekerEntitlementRuntime = {
  // Keep module initialization compatible with focused tests that replace the
  // broad db module with a deliberately narrow mock. The real binding is read
  // only when the entitlement path actually asks for a wallet.
  walletForAccount: (accountId) => walletForAccount(accountId),
