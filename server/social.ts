...
export class SocialService {
  ...
  async guildCreate(
    ...
    if (priorPledge) {
      ...
      // the row does not linger there. Awaited directly, NOT through
      // afterGuildCreateCommit: that helper is synchronous (run: () => void),
      // so an async callback would float past its try/catch.
