...
// check, guild leadership/rename, post-rename rekeys) come in as
// `IdentityGameHooks` so this module stays free of the live client map and
// socket layer.
...
export interface IdentityGameHooks {
  ...
  // Post-rename bookkeeping the live server owns: rekey the World Market
  // seller entries and mail ownership that are stored by character name
  // (mirrors the free force_rename path in main.ts).
  onCharacterRenamed(characterId: number, oldName: string, newName: string): Promise<void>;
}
  ...
  return {
    ...
    async apply(accountId, kind: IdentityKind, payload): Promise<ApplyResult> {
      if (kind === 'rename_character') {
        ...
        // Read the current row first: the rekey hook below needs the old name.
        const before = await getCharacter(accountId, characterId);
        if (!before) return { status: 404, body: { error: 'character not found' } };
        try {
          const c = await renameCharacterVoluntary(accountId, characterId, name);
          if (!c) {
            // The force_rename = FALSE gated UPDATE matched no row: the character
            // was deleted, or a moderator flagged it mid-flight (that rename is
            // free and must go through the dedicated endpoint).
            return { status: 409, body: { error: 'character can no longer be renamed here' } };
          }
          await hooks.onCharacterRenamed(characterId, before.name, c.name);
          return {
            status: 200,
            body: {
              id: c.id,
