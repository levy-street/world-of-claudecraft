...
describe('buildManifest', () => {
  ...
  it('keeps the release catalog and all 59 UI cues in one 204-key inventory', () => {
    const keys = new Set(SFX.map((entry) => entry.key));
    expect(keys.size).toBe(204);
    expect([...keys].filter((key) => key.startsWith('ui_'))).toHaveLength(59);
    for (const key of [
      'cast_lightning_bolt',
      'mob_mudfin_attack',
      'mob_burrower_attack',
    ...
    expect(SFX_FIXED_CATALOG_KEYS).toHaveLength(204);
  });
});
