...
describe('buildManifest', () => {
  ...
  it('keeps the release catalog and all 32 UI cues in one 177-key inventory', () => {
    const keys = new Set(SFX.map((entry) => entry.key));
    expect(keys.size).toBe(177);
    expect([...keys].filter((key) => key.startsWith('ui_'))).toHaveLength(32);
    for (const key of [
      'cast_lightning_bolt',
      'mob_mudfin_attack',
      'mob_burrower_attack',
    ...
    expect(SFX_FIXED_CATALOG_KEYS).toHaveLength(177);
  });
});
