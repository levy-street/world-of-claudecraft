...
describe('terrain region index', () => {
  ...
  it('keeps sparse candidates wired into the production height path', () => {
    ...
    // Index 19 is reserved for the retired Farshore causeway; it deliberately
    // has no production height applier while keeping every later id stable.
    expect(heightBody.match(/terrainRegionHas\(region, TERRAIN_APPLIER\./g)).toHaveLength(33);
  });
