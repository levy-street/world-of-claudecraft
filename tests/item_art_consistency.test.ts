...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: 'b429dee82de1abd7768be7dff79a4f987bd5c6c30df250d436b09559f32b9a9f',
        acceptedBytes: 124_202,
      },
    ]);
  ...
  it('pins the final item-art visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(124_202);
    expect(sha256(verdictBytes)).toBe(
      'b429dee82de1abd7768be7dff79a4f987bd5c6c30df250d436b09559f32b9a9f',
    );
    ...
    expect(verdict.auditScope).toMatchObject({
      ...
      itemArtFilesReviewed: 1042,
      liveItemDefinitions: 1057,
    ...
    expect(verdict.auditScope.incrementalReviews.at(-1)).toEqual({
      ...
      reviewer: 'owner',
      removedIds: ['reins_chimeglass_tortoise', 'reins_mech_bird'],
      provenance: ['public/ui/items/mapping.json'],
      note: 'Mount skins conversion (PR: cosmetics window): reins_mech_bird and reins_chimeglass_tortoise retired as items; their icons moved to public/ui/store/mount_skins/<skin id>.webp unchanged. No new art reviewed; counts and passIds shrink by two.',
    });
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: '0f161d15614f566547cc85e5c398cb925c3bb461fc3b28d9a5b4c9e836094864',
      bytes: 568_223,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      '3963065ca57fee4ce67137e368f19cbd1fbd2b1e0f4dc6537c60b8f914bca958',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      '7c8c463db67845065dfd570a81ceb6d83e7bdb28c09af629bb362dd3169b1aa7',
    );
  ...
  it('keeps an exact, non-dangling supersession graph and one current mapping owner', () => {
    ...
    expect(mapping.entries).toHaveLength(42);
    ...
    expect(allCurrentOwnerIds).toHaveLength(1042);
    expect(new Set(allCurrentOwnerIds).size).toBe(1042);
  ...
  it('keeps the full shipping icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    ...
    if (ownerIds.length !== 1042)
      violations.push(`mapping owner count: ${ownerIds.length} != 1042`);
    if (fileIds.length !== 1042) violations.push(`shipping WebP count: ${fileIds.length} != 1042`);
    for (const id of ids) {
