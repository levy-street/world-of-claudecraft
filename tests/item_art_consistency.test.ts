...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: 'a245ed67c9aef35c49d2d616348d333bc16dc97380b34f5602f66c9ec5c5cac4',
        acceptedBytes: 114_010,
      },
    ]);
  ...
  it('pins the final 817-item visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(114_010);
    expect(sha256(verdictBytes)).toBe(
      'a245ed67c9aef35c49d2d616348d333bc16dc97380b34f5602f66c9ec5c5cac4',
    );
    ...
    expect(verdict.auditScope).toMatchObject({
      ...
      itemArtFilesReviewed: 860,
      liveItemDefinitions: 875,
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: '185028dc4fb1b563013a1c1c2bd812966269ef2f0e381057483065a7e53ef106',
      bytes: 471_776,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      'e0246fd65ad6efc6eb61e78ddb3286429f0c5bbd3b8e57a930f6a7729d4f45d5',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      '7260291be47b0dc122d3348d1668c8d1125701c001741d1ccebe177ce06184a5',
    );
  ...
  it('keeps an exact, non-dangling supersession graph and one current mapping owner', () => {
    ...
    expect(allCurrentOwnerIds).toHaveLength(860);
    expect(new Set(allCurrentOwnerIds).size).toBe(860);
  ...
  it('keeps the full 817-icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    ...
    if (ownerIds.length !== 860) violations.push(`mapping owner count: ${ownerIds.length} != 860`);
    if (fileIds.length !== 860) violations.push(`shipping WebP count: ${fileIds.length} != 860`);
    for (const id of ids) {
