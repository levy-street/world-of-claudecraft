...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: 'ee328e9cd645426fcf1505901d4d745fa6dd8de33e0fe83712e711e8b437dd65',
        acceptedBytes: 121_288,
      },
    ]);
  ...
  it('pins the final item-art visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(121_288);
    expect(sha256(verdictBytes)).toBe(
      'ee328e9cd645426fcf1505901d4d745fa6dd8de33e0fe83712e711e8b437dd65',
    );
    ...
    expect(verdict.auditScope).toMatchObject({
      ...
      itemArtFilesReviewed: 1043,
      liveItemDefinitions: 1058,
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: '1ab72f5dd31a27cf3b225103cc0cd6d94d5db23962614f28f83d2c6ccdad0ec3',
      bytes: 568_759,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      '64889dc25b8bb5cfda22a349ae09a033a04e4e4102e8879840c79eb803e2eaa9',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      '3c0120d25c3b7fb28a718192faf9b1e3c83fb569d8cc6dd535d63c78c3b68f0c',
    );
  ...
  it('keeps an exact, non-dangling supersession graph and one current mapping owner', () => {
    ...
    expect(oldGeneratedIds).toHaveLength(728);
    const allCurrentOwnerIds = [
      ...mapping.entries.map(({ itemId }) => itemId),
      ...mapping.generatedBatches.flatMap(({ itemIds }) => itemIds),
    ];
    expect(allCurrentOwnerIds).toHaveLength(1043);
    expect(new Set(allCurrentOwnerIds).size).toBe(1043);
  ...
  it('keeps the full shipping icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    ...
    if (ownerIds.length !== 1043)
      violations.push(`mapping owner count: ${ownerIds.length} != 1043`);
    if (fileIds.length !== 1043) violations.push(`shipping WebP count: ${fileIds.length} != 1043`);
    for (const id of ids) {
