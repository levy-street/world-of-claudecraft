...
describe('item-art consistency accepted-art provenance', () => {
  ...
  it('pins the final 824-item visual audit, Heroic accounting, and evidence digests', () => {
  ...
  it('keeps the full 824-icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    const mapping = readJson<ItemMapping>('public/ui/items/mapping.json');
    const ownerIds = [
      ...mapping.entries.map(({ itemId }) => itemId),
      ...mapping.generatedBatches.flatMap(({ itemIds }) => itemIds),
