...
describe('item-art consistency accepted-art provenance', () => {
  it('pins the exact scope, style contract, and byte-identical generation reports', () => {
    ...
    expect(value.sourceEvidence).toEqual([
      ...
      },
      {
        ...
        acceptedSha256: 'a9cb15dd1bc0dde523e1ea6cea8a2e54c517958a5d982d27a8ada7d3e6e168be',
        acceptedBytes: 124_970,
      },
    ]);
  ...
  it('pins the final item-art visual audit, Heroic accounting, and evidence digests', () => {
    ...
    expect(verdictBytes.length).toBe(124_970);
    expect(sha256(verdictBytes)).toBe(
      'a9cb15dd1bc0dde523e1ea6cea8a2e54c517958a5d982d27a8ada7d3e6e168be',
    );
    ...
    expect(verdict.auditScope.incrementalReviews.at(-1)).toEqual({
      ...
      branch: 'feature/cosmetics-window',
      reviewer: 'owner',
      removedIds: ['reins_rickshaw_mount'],
      provenance: ['public/ui/items/mapping.json'],
      note: 'Mount skins conversion, second wave (PR: cosmetics window): reins_rickshaw_mount retired as an item; its icon moved to public/ui/store/mount_skins/rickshaw_mount.webp unchanged. No new art reviewed; counts and passIds shrink by one.',
    });
    ...
    expect(verdict.evidence.catalog).toEqual({
      ...
      sha256: '90478b196f9d76e03d4c807bf6873c43bba9596bddacba72ab5e88ba92e7881a',
      bytes: 567_688,
    });
    ...
    expect(verdict.evidence.sheetSetSha256).toBe(
      '6ecbc68d6345ec50082353d6961651f1163e0527fcddba57f974429d79ac7e77',
    );
    ...
    expect(verdict.evidence.shippingCatalogSha256).toBe(
      '7e64fb41386b58529e7e7e38afa41ba1cb2fc241a800d29c241cdee1fad04d85',
    );
  ...
  it('keeps an exact, non-dangling supersession graph and one current mapping owner', () => {
    ...
    expect(mapping.generatedBatches).toHaveLength(23);
    ...
    expect(oldGeneratedIds).toHaveLength(726);
    const allCurrentOwnerIds = [
      ...mapping.entries.map(({ itemId }) => itemId),
      ...mapping.generatedBatches.flatMap(({ itemIds }) => itemIds),
    ];
    expect(allCurrentOwnerIds).toHaveLength(1041);
    expect(new Set(allCurrentOwnerIds).size).toBe(1041);
  ...
  it('keeps the full shipping icon catalog owned, decodable, opaque, budgeted, and unique', async () => {
    ...
    if (ownerIds.length !== 1041)
      violations.push(`mapping owner count: ${ownerIds.length} != 1041`);
    if (fileIds.length !== 1041) violations.push(`shipping WebP count: ${fileIds.length} != 1041`);
    for (const id of ids) {
