...
describe('mount reins items (the collection: owning the item is owning the mount)', () => {
  ...
  // The capture tool delegates collection setup to the catalog-driven dev
  // command (pinned exhaustively in chat.test.ts), so it cannot grow a second,
  // stale hardcoded reins list.
  it('the mounts capture script grants through the catalog-driven dev command', () => {
    const source = readFileSync(path.join(__dirname, '..', 'scripts/mounts_shot.mjs'), 'utf8');
    expect(source).toContain("sim.chat('/dev mounts')");
    expect(source).not.toMatch(/for \(const id of \[/);
  });
