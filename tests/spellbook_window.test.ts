...
describe('spellbook_window: inline mobile slot picker', () => {
  ...
  it('protects inset-pinned touch sheets while allowing the centered spellbook to resize', () => {
    // The shared drag controller still raises a touch sheet, but its handle
    // predicate rejects dragging before it can replace the inset pins with
    // inline left/top. Resize keeps the centered Spellbook as the one touch exception.
    expect(hud).toContain('this.windowDragController = installWindowDrag({');
    expect(hud).toMatch(
      /isDragHandle:[\s\S]{0,100}!this\.isMobileLayout\(\)[\s\S]{0,100}this\.isWindowDragHandle/,
    );
