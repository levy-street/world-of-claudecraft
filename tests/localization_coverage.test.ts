...
describe('i18n Localization Key Coverage', () => {
  ...
  it('should track item-set names and bonus text in the entity catalog', async () => {
    ...
    // 8 raid/dungeon families with name+bonus2+bonus3+bonus4 (every epic family
    // carries a 4-piece proc tier; the Spiritbinder mail caster set is the 8th),
    // plus 3 leveling haste kits carrying a single 3-piece tier (name+bonus3 only).
    expect(itemSetEntries).toHaveLength(8 * 4 + 3 * 2);
