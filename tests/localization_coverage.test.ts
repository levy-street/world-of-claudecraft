...
describe('i18n Localization Key Coverage', () => {
  ...
  it('should provide deed content translations for every supported locale', () => {
    ...
    // name + desc per deed, plus one title entry per title deed (40 as of the
    // Arena season titles; tests/deeds_content.test.ts pins the count).
    expect(deedEntries.length).toBe(Object.keys(DEEDS).length * 2 + 40);
