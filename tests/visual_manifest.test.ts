...
describe('character visual manifest', () => {
  it('dispatches every Demon Tower template to its authored visual', () => {
    for (const templateId of Object.keys(TOWER_MOBS)) {
      const key = `mob_${templateId}`;
      expect(visualKeyFor({ kind: 'mob', templateId } as never)).toBe(key);
