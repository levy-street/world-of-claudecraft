...
describe('HarborShipCueRegistry', () => {
  ...
  it('discards handles and pending cues on a world rebuild', () => {
    const { registry, setNow } = harness();
    const staleShip = parked('mainland');
    registry.register('mainland', staleShip);
    setNow(10);
    registry.cue('gullhaven', 'castOff');

    registry.clear();

    expect(registry.get('mainland')).toBeUndefined();
    const rebuilt = parked('gullhaven');
    setNow(12);
    registry.register('gullhaven', rebuilt);
    expect(rebuilt.segment).toBeNull();
