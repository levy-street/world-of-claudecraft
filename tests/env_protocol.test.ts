...
describe('headless environment protocol validation', () => {
  ...
  it('accepts every declared player class and rejects anything else', () => {
    ...
    expect(validatePlayerClass('swordmaster')).toBe('swordmaster');
  ...
  it('builds an identical-shape, full-size, finite observation for every class', () => {
    // Loop ALL_CLASSES (not a hardcoded subset) so all 10 obs vectors are guarded.
    // The obs space is content-scaled
    ...
    for (const cls of ALL_CLASSES) {
      ...
      // (python/wow_env.py), so the cross-language obs contract holds for all 10 classes
      expect(obs.every((v) => v >= -2 && v <= 2)).toBe(true);
      sizes.add(obs.length);
    }
    // a single distinct length across all 10 classes, equal to the advertised
  ...
  it('sizes the action space to the largest class kit so every class is castable', () => {
    ...
    // policy's action head stays valid across all 10 classes.
