...
describe('deed_i18n English resolution', () => {
  ...
  it('manifests one row per name and desc plus one per title reward', () => {
    ...
    // 275 deeds x (name + desc) + the 42 shipped title rewards (both counts
    ...
    // visit pair, the bank socket ladder pair (Bank Storage phase 06) and the
    // Proving Shore graduation deed. All three appends are reward-free, so the
    // title count stays 42. MEASURED on the merged tree.
