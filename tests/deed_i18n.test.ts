...
describe('deed_i18n English resolution', () => {
  ...
  it('manifests one row per name and desc plus one per title reward', () => {
    ...
    // 229 deeds x (name + desc) + the 40 shipped title rewards (both counts
    // pinned by tests/deeds_content.test.ts).
    expect(manifest.length).toBe(229 * 2 + 40);
    expect(manifest.filter((row) => row.field === 'title').length).toBe(40);
