...
describe('deed_i18n English resolution', () => {
  ...
  it('manifests one row per name and desc plus one per title reward', () => {
    ...
    // 247 deeds x (name + desc) + the 31 shipped title rewards (both counts
    ...
    // (dgn_rift, dgn_rift_s_rank), and the seven per-craft rare-tier profession
    // deeds, none of which carry a title reward.
    expect(manifest.length).toBe(247 * 2 + 31);
