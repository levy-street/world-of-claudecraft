...
describe.skipIf(!PG_TEST_URL)('season reward standings (real Postgres)', () => {
  ...
  async function seedPlayer(name: string, rating: number, wins: number): Promise<void> {
    ...
    await pool.query(
      ...
      [
        ...
        name,
        REALM,
        JSON.stringify({ arena1v1Rating: rating, arena1v1Wins: wins, arena1v1Losses: 1 }),
      ],
    );
  }
  ...
  it('ranks the realm arena ladder and splits the real pool into projected rewards', async () => {
    ...
    await pool.query(
      `INSERT INTO woc_flow_ledger(season_id,source,direction,amount_base,tx_sig) VALUES (1,'gamblefi_burn_rake','in','1000000000','seed')`,
    );
    ...
    const standings = projectSeasonRewards(
  ...
  it('excludes characters who have not played any arena matches', async () => {
    ...
    const acct = await pool.query(
      `INSERT INTO accounts(username, password_hash) VALUES ('idle', 'x') RETURNING id`,
    );
    await pool.query(
      ...
      [
        acct.rows[0].id,
        REALM,
        JSON.stringify({ arena1v1Rating: 3000, arena1v1Wins: 0, arena1v1Losses: 0 }),
      ],
    );
  ...
  it('yields no standings when the realm ladder is empty', async () => {
    await openSeason({ seasonId: 1, label: 'S1' });
    await pool.query(
      `INSERT INTO woc_flow_ledger(season_id,source,direction,amount_base,tx_sig) VALUES (1,'gamblefi_burn_rake','in','500000000','x')`,
    );
    const ladder = await topArenaRatings(10, '1v1');
    expect(
