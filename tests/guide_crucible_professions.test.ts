...
import { GUIDE_PROF_ENCHANTING } from '../src/guide/content.generated';
...
function render(id?: string): string {
  setLanguage('en');
  return professions.render({ params: id ? [id] : [], sub: 'professions', titleKey: 'guide.nav.professions' });
}
...
describe('Crucible professions public reference', () => {
  ...
  it('exports and renders the proc enchant effect and formula acquisition gate', () => {
    const row = GUIDE_PROF_ENCHANTING.enchants.find((entry) => entry.id === 'enchant_weapon_lastflame_zeal');
