...
import { type BurnExecutor, BurnKeeper, type BurnStore } from '../server/burn_keeper';
...
function fakeStore() {
  ...
  const store: BurnStore & { batches: Map<string, BurnBatchRow>; setLast(t: number): void } = {
    ...
    async markBurning(id, sig) {
      const b = batches.get(id)!;
      b.status = 'burning';
      b.burnTxSig = sig;
      b.burnBroadcastAt = new Date(NOW).toISOString();
    },
    async markBurned(id, woc) {
      const b = batches.get(id)!;
      b.status = 'burned';
      b.wocBurned = woc;
      last = NOW;
    ...
    async openBatches() {
      return [...batches.values()].filter((b) =>
        ['swapping', 'swapped', 'burning'].includes(b.status),
      );
    },
    async lastBurnAt() {
      return last;
...
function fakeExec(
  ...
  const exec: BurnExecutor = {
    ...
    quote: async (inUsdc) => {
      ...
      quoteN++;
      return over.quoteNull || quoteN === over.quoteFailAt
        ? null
        : { raw: { in: inUsdc.toString() } };
    },
    signSwap: async () => {
    ...
    signBurn: async (amt) => {
      calls.signBurn.push(amt);
      const signature = `burn${++burnN}`;
      return {
        signature,
        send: async () => {
          calls.burnSends.push(signature);
        },
      };
    },
  };
...
const keeper = (exec: BurnExecutor, store: BurnStore) =>
  new BurnKeeper(exec, store, DEFAULT_BURN_POLICY, {
    now: () => NOW,
    newBatchId: (() => {
      let n = 0;
      return () => `b${++n}`;
...
describe('BurnKeeper.runCycle — swap → burn', () => {
  ...
  it('leaves the batch in-flight ("burning") when the burn broadcasts but does not confirm', async () => {
    ...
    const { exec, calls } = fakeExec({
      usdc: usdc(300),
      received: 700n,
      confirm: (s) => (s.startsWith('burn') ? 'unknown' : 'confirmed'),
    });
    await keeper(exec, store).runCycle();
    const batch = [...store.batches.values()][0];
...
describe('BurnKeeper.recover — finish in-flight batches by recorded signature', () => {
  ...
  it('re-issues a burn whose recorded signature reverted', async () => {
    ...
    const { exec, calls } = fakeExec({
      confirm: (s) => (s === 'burnOld' ? 'failed' : 'confirmed'),
    });
    await keeper(exec, store).recover();
  ...
  it('RECOVERS a stale swap that actually landed — burns the received $WOC, never strands it', async () => {
    ...
    const { exec, calls } = fakeExec({
      confirm: (s) => (s.startsWith('burn') ? 'confirmed' : 'unknown'),
      received: 640n,
    });
    await keeper(exec, store).recover();
  ...
  it('times burning-staleness from burn broadcast, not swap creation (no premature re-burn)', async () => {
    ...
    store.seed({
      ...
      burnBroadcastAt: new Date(NOW - 60 * 1000).toISOString(),
    });
