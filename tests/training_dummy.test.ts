...
describe('training dummy', () => {
  ...
  it('stays inert when attacked — never aggros or moves, but takes damage', () => {
    ...
    expect(sim.player.inCombat).toBe(true); // attacking puts the player in combat
    expect(d.aiState).toBe('idle'); // dummy never retaliates
  ...
  it('drops combat and heals to full ~5s after the last hit', () => {
    ...
    expect(sim.player.inCombat).toBe(false); // player drops combat after the 5s window
  });
  ...
  it('drops a Bale of Straw and respawns ~10s after dying', () => {
    ...
    for (let i = 0; i < 20 * 9; i++) sim.tick(); // still a corpse before 10s
    expect(d.dead).toBe(true);
    for (let i = 0; i < 20 * 2; i++) sim.tick(); // after 10s the same entity is back
    expect(d.id).toBe(id);
