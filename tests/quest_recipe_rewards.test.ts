...
const RECIPE: ProfessionRecipeRecord & { consumeOnCraft: true } = {
  id: 'recipe_test_one_use_quest', professionId: 'weaponcrafting',
  resultItemId: 'eastbrook_arming_sword', resultCount: 1,
  reagents: [{ itemId: 'bone_fragments', count: 1 }],
  skillReq: 125, itemLevelBudget: 1, level: 1,
  acquisition: ['quest'], consumeOnCraft: true,
};

const QUEST: QuestDef & { recipeReward: string } = {
  id: 'q_test_recipe_reward', name: 'Test recipe reward',
  giverNpcId: 'foreman_odell', turnInNpcId: 'foreman_odell',
  text: '', completionText: '', objectives: [{ type: 'collect', itemId: 'spider_leg', count: 1, label: 'Leg' }],
  xpReward: 5, copperReward: 10, itemRewards: {}, recipeReward: RECIPE.id,
};
...
describe('quest recipe rewards and single-use knowledge', () => {
  ...
  it.each([124, 0, Number.NaN, Number.POSITIVE_INFINITY])('refuses invalid or below-floor skill %s before rewards or consumption', (skill) => {
  ...
  it('refuses an unknown recipe reward before marking the quest complete', () => {
    const { sim, meta } = readySmith();
    expect(turnInQuestCore(sim.ctx, QUEST.id, { ...QUEST, recipeReward: 'missing' } as typeof QUEST, meta)).toBe(false);
