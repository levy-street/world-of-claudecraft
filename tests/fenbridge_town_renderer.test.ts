...
describe('Fenbridge dedicated town renderer', () => {
  ...
  it('renders the optimized shipping GLBs at the exact whole-town draw and triangle totals', {
    ...
    // Re-minted with tests/fenbridge_town_assets.test.ts after the 0.41.0 bump
    // forced a Fenbridge re-export: the Fenbridge totals are pinned in BOTH
    // files, so a re-export has to move both or this one reds on its own.
