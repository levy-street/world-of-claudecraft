...
import { enterStoryInstance } from '../src/sim/instances/story_instances';
import { FARE_CHOICE_OUT } from '../src/sim/last_bell/campaign';
...
import { placePlayerAtWalkEndpoint } from '../src/sim/scenes/player_walk';
...
describe('teleports cancel a live session', () => {
  ...
  it('Last Bell ferry travel cancels a live fishing session', () => {
    const sim = makeSim();
    const pid = sim.playerId;
    const p = startFishingSession(sim, pid);
    const meta = sim.players.get(pid);
    const ewald = [...sim.entities.values()].find(
      (entity) => entity.templateId === 'ferryman_ewald',
    );
    if (!meta || !ewald) throw new Error('missing mainland ferry fixture');
    meta.copper = 25;
    teleportTo(sim, pid, ewald.pos.x, ewald.pos.z);
    p.targetId = ewald.id;
    sim.drainEvents();
    sim.interact(pid);
    expect(answerSceneChoice(sim.ctx, FARE_CHOICE_OUT, 'pay', pid)).toBe(true);
    expectSessionEnded(sim, p);
  });

  it('Last Bell story entry cancels a session and resets both facing samples', () => {
    const sim = makeSim();
    const pid = sim.playerId;
    const p = startFishingSession(sim, pid);
    p.facing = 1.25;
    p.prevFacing = 1.25;
    sim.drainEvents();
    expect(enterStoryInstance(sim.ctx, 'lb_riftline', pid)).toBe(true);
    expectSessionEnded(sim, p);
    expect(p.facing).toBe(0);
    expect(p.prevFacing).toBe(0);
  });

  it('scene player-walk skip placement cancels a live session', () => {
    const sim = makeSim();
    const pid = sim.playerId;
    const p = assignFishingSession(sim, pid);
    sim.drainEvents();
    placePlayerAtWalkEndpoint(sim.ctx, p, { x: p.pos.x + 10, z: p.pos.z + 10 });
    expectSessionEnded(sim, p);
  });
  ...
  it('/dev named tp cancels a session', () => {
    const sim = makeSim();
    const pid = sim.playerId;
    const p = startGatherSession(sim, pid);
    sim.drainEvents();
    handleDevChat(sim.ctx, '/dev tp gullhaven', pid);
    expectSessionEnded(sim, p);
  });
});
