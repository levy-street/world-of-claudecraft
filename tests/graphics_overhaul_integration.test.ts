...
describe('graphics-overhaul integration', () => {
  it('keeps object obstruction opacity-only and never changes chase-camera distance', () => {
    ...
    const chaseStart = renderer.indexOf('private updateCamera(');
    const chaseEnd = renderer.indexOf('// Spatial-audio listener', chaseStart);
    expect(chaseStart).toBeGreaterThanOrEqual(0);
