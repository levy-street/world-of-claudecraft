...
function tx(o: {
  ...
  return {
    ...
    transaction: {
      message: { accountKeys: o.keys ?? [{ pubkey: PAYER }], instructions: o.instructions ?? [] },
    },
  };
}
...
describe('parseSplitPayment', () => {
  it('computes net per-owner deltas for the target mint (post − pre), ignoring other mints', () => {
    const r = parseSplitPayment(
      tx({
        pre: [
          { owner: PAYER, mint: MINT, amount: '1000' },
          { owner: PAYER, mint: OTHER, amount: '5' },
        ],
        post: [
          { owner: PAYER, mint: MINT, amount: '300' },
          { owner: RECIP, mint: MINT, amount: '700' },
          { owner: PAYER, mint: OTHER, amount: '999' },
        ],
      }),
      MINT,
    );
  ...
  it('treats a freshly-created ATA (absent from preTokenBalances) as starting at zero', () => {
    const r = parseSplitPayment(
      tx({ pre: [], post: [{ owner: RECIP, mint: MINT, amount: '640' }] }),
      MINT,
    );
  ...
  it('extracts a memo from spl-memo (by program name and by program id)', () => {
    expect(
      parseSplitPayment(tx({ instructions: [{ program: 'spl-memo', parsed: 'quote-123' }] }), MINT)
        .memo,
    ).toBe('quote-123');
    expect(
      parseSplitPayment(
        tx({
          instructions: [
            { programId: 'MemoSq4gqABAXKb96qnH8TysNcWxMyWCqXgDLGmfcHr', parsed: 'quote-xyz' },
          ],
        }),
        MINT,
      ).memo,
  ...
  it('flags the target mint held under Token-2022 (rejected upstream as a fee/hook look-alike)', () => {
    ...
    const t22 = parseSplitPayment(
      tx({ post: [{ owner: RECIP, mint: MINT, programId: SPL_TOKEN_2022_PROGRAM, amount: '1' }] }),
      MINT,
    );
  ...
  it('reports success only when the tx landed without error', () => {
    ...
    expect(
      parseSplitPayment(tx({ err: { InstructionError: [0, 'Custom'] } }), MINT).succeeded,
    ).toBe(false);
