...
export const GUIDE_WIKI_AUDIT_FILLS: Record<string, Record<GuideWikiAuditFillLocale, string>> = {
  ...
  'guide.interfacePage.winMoreBodyNoValeCup': {
    ...
    zh_TW:
      '世界地圖（M）、PvP 視窗（G）、排行榜（K）與活動行事曆（I），全都是同樣的用法。表情輪盤（X）是例外：按住它的按鍵，輪盤便會出現，接著在某個表情上放開按鍵即可播放它。排行榜值得你初次造訪時多看一眼：它有一個玩家分頁、一個公會分頁、一個依功績之書的名望為整個帳號排名的分頁、一個每日榜分頁，還有一個「開發者」分頁，列著打造這款遊戲的人，除非你關閉「顯示開發者徽章」，否則它一直都在。\n\n選取另一位玩家並在目標框架上按右鍵（觸控裝置上則是雙點或長按它），或是在聊天中對他的名字按右鍵，「玩家資訊」就會開啟一張關於對方的卡片：對方身上穿的裝備，附帶提示，以及其角色的公開資料。這只是看一眼，僅此而已。要看裝備，對方必須近到你看得見：當對方遠在他處時從聊天中查他的名字，你得到的會是這張卡片公開的那一半，也就是頭像、名字、等級、職業與公會。',
    ja_JP:
      'ワールドマップ（M）、PvPウィンドウ（G）、リーダーボード（K）、イベントカレンダー（I）も、すべて同じように使えます。エモートホイール（X）だけは例外で、キーを押し続けるとホイールが現れ、エモートの上でキーを離すとそのエモートが再生されます。リーダーボードは初めて開くときに少し眺めてみる価値があります。プレイヤーのタブ、ギルドのタブ、功績の書の名声でアカウント全体を順位付けするタブ、デイリーの順位のタブ、そしてゲームを作っている人々のための「開発者」タブがあり、開発者タブは「開発者バッジを表示」をオフにしない限り表示されます。\n\nほかのプレイヤーをターゲットしてターゲットフレームを右クリックするか（タッチではダブルタップか長押し）、チャットでその名前を右クリックすると、「プレイヤー情報」がその人物のカードを開きます。身につけている装備がツールチップ付きで並び、キャラクターの公開情報も載ります。見るだけで、それ以上のことはできません。装備を見るには相手が見える距離にいる必要があります。遠く離れた相手の名前をチャットから調べると、代わりにカードの公開されている半分、ポートレート、名前、レベル、クラス、ギルドが表示されます。',
    ko_KR:
  ...
  'guide.social.finderBodyLeaderQueues': {
    zh_CN:
      '你不必在“寻求组队”频道里喊人才能凑齐一趟。打开地下城查找器，选好你想跑的内容和你愿意承担的职责，然后独自加入队列，或者让你的队长把现成的队伍排进去（只有队长才能为队伍排队）。查找器会一直等到职责配齐，然后同时向所有人发出邀请：一个弹窗请你们每个人确认，最后一个人点下同意的瞬间，队伍就成立了。拒绝邀请或者让它超时，会让你连同一起排队的队伍一并退出队列，并先进入一小段冷却，之后才能重新加入；邀请中的其他人都保留各自的位置，好让队列一直往前走。',
    zh_TW:
      '你不必在「尋找隊伍」頻道裡大喊也能湊滿一趟旅程。打開地城搜尋器，選擇你想跑的內容與你願意擔任的定位，然後獨自加入排隊，或由你的隊長帶著你們現有的隊伍排隊（只有隊長才能把隊伍送進佇列）。搜尋器會一直等到定位湊齊，再同時把隊伍提供給所有人：一個彈出視窗請你們每個人確認，等最後一人點下同意，隊伍便當場成形。拒絕一次邀請，或讓它逾時，會把你連同和你一起排隊的隊伍移出佇列，並讓你進入一段短暫的冷卻，之後才能再次加入；邀請中的其他人都保住原本的位置，好讓佇列持續往前推進。',
    ja_JP:
  ...
  'guide.social.lootRollBodyNeedBeatsGreed': {
    ...
    ja_JP:
      'アイテムがロールにかけられると、対象となる各メンバーは、欲しければニード、余りなら受け取る程度ならグリード、辞退するならパスを選びます。ニードはグリードに優先します。誰か一人でもニードを選べば、アイテムはニードの中で最も高い出目の人に渡り、グリードの出目は数えません。ニードが誰もいなければ、グリードの中で最も高い出目が勝ちます。',
    ko_KR:
      '물품이 입찰에 부쳐지면, 자격 있는 구성원은 저마다 원하면 입찰을, 남는 것만 받겠다면 가치 입찰을, 빠지겠다면 포기를 고릅니다. 입찰이 가치 입찰에 앞섭니다. 누구든 입찰을 골랐다면 물품은 입찰을 고른 사람 가운데 가장 높은 수를 낸 사람에게 가고 가치 입찰의 수는 셈에 들지 않으며, 입찰을 고른 사람이 없을 때에만 가치 입찰을 고른 사람 가운데 가장 높은 수를 낸 사람이 가져갑니다.',
    ru_RU:
