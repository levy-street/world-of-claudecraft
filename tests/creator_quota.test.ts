...
describe('isTrustedCreator — earned badge, faster at higher tiers, strike-revoked', () => {
  ...
  it('any strike in the window revokes trust regardless of tier/approvals', () => {
    expect(
      isTrustedCreator({ approvedCount: 99, tierIndex: 18, accountAgeDays: old, recentStrikes: 1 }),
    ).toBe(false);
  });
  it('whale+ (tier ≥7) is trusted on the first clean approval, age-relaxed', () => {
    expect(
      isTrustedCreator({ approvedCount: 1, tierIndex: 7, accountAgeDays: 0, recentStrikes: 0 }),
    ).toBe(true);
    expect(
      isTrustedCreator({ approvedCount: 0, tierIndex: 7, accountAgeDays: old, recentStrikes: 0 }),
    ).toBe(false);
  });
  it('mid-tiers need the per-tier approval count AND the minimum account age', () => {
    // coppercrest needs 5 approvals
    expect(
      isTrustedCreator({ approvedCount: 4, tierIndex: 3, accountAgeDays: old, recentStrikes: 0 }),
    ).toBe(false);
    expect(
      isTrustedCreator({ approvedCount: 5, tierIndex: 3, accountAgeDays: old, recentStrikes: 0 }),
    ).toBe(true);
    ...
    expect(
      isTrustedCreator({
        approvedCount: 5,
        tierIndex: 3,
        accountAgeDays: TRUST_MIN_ACCOUNT_AGE_DAYS - 1,
        recentStrikes: 0,
      }),
    ).toBe(false);
    // gilded needs only 2
    expect(
      isTrustedCreator({ approvedCount: 2, tierIndex: 5, accountAgeDays: old, recentStrikes: 0 }),
    ).toBe(true);
  });
