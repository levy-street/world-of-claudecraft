...
describe('baked map backgrounds', () => {
  ...
  it('matches the current world generation (re-bake with npm run assets:mapbg)', () => {
    for (const zone of ZONES) {
      ...
      expect(entry.w, `${zone.id} plate width`).toBe(BAKED_MAP_BG.plateWidth);
      expect(entry.h, `${zone.id} plate height`).toBe(
        mapCanvasHeight(BAKED_MAP_BG.plateWidth, region),
      );
      expect(
        hashPaintedRows(region, BAKED_MAP_BG.seed, BAKED_MAP_BG.plateWidth, BAKED_MAP_BG.hashRows),
