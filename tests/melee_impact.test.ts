...
describe('anatomical contact ownership', () => {
  it('allows the earned auto hit without letting it restart the body during an authored action', () => {
    ...
    expect(
      damageEventStartsAttackAnimation(
        ...
        view,
        false,
        ABILITIES.sinister_strike.name,
        'sinister_strike',
      ),
    ).toBe(true);
  });
  it('does not replay attacks on bleed ticks or their final expiry tick', () => {
    for (const id of ['deep_wounds', 'garrote', 'rupture', 'hemorrhage', 'venomrend']) {
      ...
      expect(
        damageEventStartsAttackAnimation({ kind: 'player' }, null, false, ABILITIES[id].name, id),
      ).toBe(true);
    }
    expect(
      damageEventStartsAttackAnimation({ kind: 'player' }, null, false, ABILITIES.bloodthirst.name),
    ).toBe(true);
    expect(damageEventStartsAttackAnimation({ kind: 'player' }, null, false, null)).toBe(true);
  });
  it.each(['garrote', 'deep_wounds'])(
    'gives %s periodic damage a wound without another flinch hold',
    (id) => {
  ...
  it('keeps edge, crush, piercing, and ragged cuts topologically distinct and finite', () => {
    const paths = [];
    for (const id of ['heroic_strike', 'shield_slam', 'mongoose_bite', 'sinister_strike']) {
