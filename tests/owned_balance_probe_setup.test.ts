...
describe('owned balance probe custom equipment setup', () => {
  it('runs a DPS setup once after the fixed equipment and talents are ready', () => {
    ...
    probe('warspirit', { targets: 1, seconds: 15, window: 'burst' }, 29900, 'test', undefined, 'pbe', (sim: Sim) => {
      observations.push({ level: sim.player.level, chest: sim.equipment.chest, spec: sim.talents.spec });
    });
  ...
  it('runs a healer setup once after the fixed equipment and talents are ready', () => {
    ...
    expect(observations).toEqual([{ level: 20, chest: 'shroud_of_the_gravewyrm', spec: 'discipline' }]);
  });
});
