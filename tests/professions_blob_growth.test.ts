...
describe('the whole-character gear-heavy maximal blob (Phase 18 U-MEASURE)', () => {
  it('settles to a fixed point with every container at its legal ceiling, inside the band', () => {
    ...
    expect(
      ...
      // terms the settled-ceiling equation above names.
      questsDone: 100,
      knownRecipes: 164,
      deeds: 672,
      deedStats: 889,
    ...
    // attributed in the growth equation above: 213,035 bytes. Floor at
    // measurement minus 380, edge at measurement plus one: 212,655..213,036.
    expect(bytes, reMint).toBeGreaterThan(212655);
    expect(bytes, reMint).toBeLessThan(213036);
