...
const db = vi.hoisted(() => ({
  ...
  walletForAccount: vi.fn(
    async (): Promise<any> => ({ pubkey: 'So11111111111111111111111111111111111111112' }),
  ),
}));
const own = vi.hoisted(() => ({
  ...
  nftMetadata: vi.fn(
    async (): Promise<any> => ({
      attributes: [{ trait_type: 'Fur', value: 'Solid Gold' }],
...
import {
  claimNftSkin,
  MAX_NFT_SKINS_PER_ACCOUNT,
  nftSkinId,
  parseNftCollectionsEnv,
} from '../server/nft_skins';
...
beforeEach(() => {
  for (const m of [...Object.values(db), ...Object.values(own), ...Object.values(assets)])
  ...
  own.nftMetadata.mockResolvedValue({
    attributes: [{ trait_type: 'Fur', value: 'Solid Gold' }],
    image: 'https://cdn/x.png',
    collection: 'c',
  });
...
describe('claimNftSkin — guards', () => {
  it('rejects an unknown chain', async () => {
    expect(await claimNftSkin({ ...CLAIM, chain: 'dogecoin' })).toEqual({
      ok: false,
  ...
  it('enforces the per-account cap for NEW skins only', async () => {
    db.listNftGrantsForAccount.mockResolvedValue(
      Array.from({ length: MAX_NFT_SKINS_PER_ACCOUNT }, (_, i) => ({
        skinId: `cs_nft_${i}`,
        revoked: false,
      })),
    );
    ...
    db.listNftGrantsForAccount.mockResolvedValue([
      ...Array.from({ length: MAX_NFT_SKINS_PER_ACCOUNT - 1 }, (_, i) => ({
        skinId: `cs_nft_${i}`,
        revoked: false,
      })),
...
describe('claimNftSkin — happy path', () => {
  it('upserts an nft row with the trait-derived design + portrait, then grants', async () => {
    ...
    db.upsertCreatorSkin.mockImplementation(async (r: Record<string, unknown>) => {
      row = r;
    });
  ...
  it('claims body-only when the portrait image is missing/invalid', async () => {
    own.nftMetadata.mockResolvedValue({
      attributes: [{ trait_type: 'Fur', value: 'Zombie' }],
      image: null,
      collection: 'c',
    });
    let row: Record<string, unknown> | null = null;
...
describe('parseNftCollectionsEnv', () => {
  it('parses + normalizes valid rows, lower-casing ETH contracts', () => {
    const rows = parseNftCollectionsEnv(
      JSON.stringify([
        {
          chain: 'ethereum',
          contract: '0xBC4CA0EDA7647A8AB7C2061C2E118A18A936F13D',
  ...
  it('respects enabled:false', () => {
    const [row] = parseNftCollectionsEnv(
      JSON.stringify([
        { chain: 'ethereum', contract: '0xAbC', standard: 'cryptopunks', enabled: false },
      ]),
    );
