...
vi.mock('../src/game/audio', () => ({ audio: { perfectingSuccess: vi.fn(), perfectingAttempt: vi.fn() } }));

import { perfectingInfoFrom, type PerfectItemRef } from '../src/sim/professions/perfecting';
...
class SwapWorld {
  ...
  equipmentInstances = {};
  inventory: InvSlot[] = [
    { itemId: CHEST, count: 1, instance: { perfected: true, enchant: 'enchant_lucent_infusion', rolled: { stats: { sta: 13 } } } },
  ...
  perfectingSwapInfo(request: { source: PerfectItemRef; target: PerfectItemRef }) {
    const source = this.perfectingInfo(request.source);
    const target = this.perfectingInfo(request.target);
    return { sourceItemId: source?.itemId, targetItemId: target?.itemId, sourceRank: source?.rank ?? 0, targetRank: target?.rank ?? 0, craftId: 'armorcrafting', skillReq: 125, reason: this.reason };
  }
}
...
const target = (index = 0) => root().querySelectorAll<HTMLButtonElement>('[data-swap-target]')[index];
...
beforeEach(() => {
  ...
  win = new PerfectingWindow({
    ...
    itemIcon: () => '', itemTooltip: () => '', attachTooltip: () => {}, moneyHtml: () => '',
    closeOthers: () => {}, captureFocus: () => null, restoreFocus: () => {},
  });
});
afterEach(() => { win.close(); setLanguage('en'); vi.useRealTimers(); vi.clearAllMocks(); });
...
describe('Perfecting rank exchange inside the existing window', () => {
  ...
  it('revalidates captured references and refuses a changed copy under the confirm prompt', () => {
    win.open(); target().click(); action().click();
  ...
  it('does not rearm a pending exchange for unrelated errors or another result', () => {
    ...
    expect(root().getAttribute('aria-busy')).toBe('true');
    win.onSwapResult({ type: 'perfectingSwapResult', pid: 2, ok: false, sourceItemId: CHEST, targetItemId: WAIST, reason: 'busy' });
    expect(root().getAttribute('aria-busy')).toBe('true');
    win.onSwapResult({ type: 'perfectingSwapResult', pid: 1, ok: false, sourceItemId: CHEST, targetItemId: WAIST, reason: 'out_of_range' });
  ...
  it('closes a confirmation without sending and releases inert state on teardown', () => {
    win.open(); target().click(); action().click(); win.close();
  ...
  it('gates station admission from the shared read and repaints when it changes', () => {
    win.open(); target().click(); expect(action().disabled).toBe(false);
  ...
  it('keeps a promoted piece available for rank attempts after its Perfected status moved', () => {
    world.inventory[0] = { itemId: CHEST, count: 1, instance: { name: 'Old Friend', perfecting: 1, boundTo: 1, rolled: { quality: 'legendary' } } };
    world.inventory.push({ itemId: 'makers_ember', count: 1 }, { itemId: 'sundered_essence', count: 1 }, { itemId: 'prismglass_setting', count: 1 });
    win.open();
  ...
  it('relocalizes the selection and open confirmation without changing the captured request', () => {
    win.open(); target().click(); action().click();
    const before = document.querySelector('.pf-swap-prompt')?.textContent;
    setLanguage('en_XA'); win.relocalize();
