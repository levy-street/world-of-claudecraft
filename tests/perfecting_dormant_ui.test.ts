...
const chest: ItemDef = { id: 'test_chest', name: 'Test chest', kind: 'armor', armorType: 'mail', slot: 'chest', sellValue: 0, stats: { sta: 10 } };
const dormant: ItemInstancePayload = { enchant: 'enchant_lucent_infusion', perfecting: 1, rolled: { quality: 'legendary', stats: { str: 2, sta: 13 } } };
...
describe('dormant Perfected-only enchant presentation', () => {
  it('omits inactive enchant power from both comparison sides without losing unrelated stats', () => {
    ...
    expect(itemStatDeltas(chest, chest, undefined, dormant)).toEqual([{ stat: 'str', delta: -2, decimals: 0 }]);
  });
