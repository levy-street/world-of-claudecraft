...
const MONOLITHS: MonolithRow[] = [
  ...
  },
  {
    ...
    // Merging the sled into the cart under release/v0.40.0: the merge lands at
    // 13568, which is the sled's own post-merge count (the cart's renderer
    // additions all live in mount_presentation.ts and the vehicle modules).
    // Pinned to that exact merged count, the rule both parents already follow.
    ceiling: 13568,
