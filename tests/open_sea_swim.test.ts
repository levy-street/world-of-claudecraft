...
function findBeachEntry(): { x: number; z: number; facing: number } {
  for (let z = -240; z <= 160; z += 6) {
    for (let x = -640; x <= 640; x += 6) {
      // The active Last Bell ferry occupies the old first qualifying harbor
      // beach. Keep this open-sea motion probe away from authored ship hulls;
      // ferry collision has its own exhaustive boarding-walk contract.
