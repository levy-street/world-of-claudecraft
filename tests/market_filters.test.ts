...
describe('World Market filters', () => {
  ...
  it('groups wearable armor separately from weapons and consumables', () => {
    expect(
      filterMarketListings(listings, { itemType: 'armor', rarity: 'all' }).map((l) => l.itemId),
    ).toEqual(['greyjaw_pelt_cloak', 'maldrecs_soulbinder']);
    expect(
      filterMarketListings(listings, { itemType: 'weapon', rarity: 'all' }).map((l) => l.itemId),
    ).toEqual(['keen_dirk']);
    expect(
      filterMarketListings(listings, { itemType: 'consumable', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
  ...
  it('groups mech cosmetics separately from ordinary materials', () => {
    ...
    expect(
      filterMarketListings(mixed, { itemType: 'cosmetic', rarity: 'all' }).map((l) => l.itemId),
    ).toEqual(['amber_crimson_armor_plate', 'alien_armor_plate']);
    expect(
      filterMarketListings(mixed, { itemType: 'material', rarity: 'all' }).map((l) => l.itemId),
    ).toEqual(['simple_fishing_pole', 'bone_fragments']);
  });

  it('matches rarities by the game quality names', () => {
    expect(
      filterMarketListings(listings, { itemType: 'all', rarity: 'poor' }).map((l) => l.itemId),
    ).toEqual(['wolf_fang', 'bone_fragments']);
    expect(
      filterMarketListings(listings, { itemType: 'all', rarity: 'common' }).map((l) => l.itemId),
    ).toEqual(['roasted_boar', 'minor_healing_potion']);
    expect(
      filterMarketListings(listings, { itemType: 'all', rarity: 'uncommon' }).map((l) => l.itemId),
    ).toEqual(['keen_dirk', 'greyjaw_pelt_cloak', 'elixir_of_the_bear']);
  });

  it('combines item type and rarity filters', () => {
    expect(
      filterMarketListings(listings, { itemType: 'armor', rarity: 'uncommon' }).map(
        (l) => l.itemId,
      ),
  ...
  it('narrows armor filters by wearable slot', () => {
    ...
    expect(
      filterMarketListings(armor, { itemType: 'armor', subtype: 'helmet', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['acolytes_circlet']);
    expect(
      filterMarketListings(armor, { itemType: 'armor', subtype: 'legs', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['greyjaw_pelt_cloak']);
    expect(
      filterMarketListings(armor, { itemType: 'armor', subtype: 'chest', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['recruit_tunic']);
    expect(
      filterMarketListings(armor, { itemType: 'armor', subtype: 'trinket', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['maldrecs_soulbinder']);
  });
  ...
  it('narrows weapon filters by weapon family', () => {
    ...
    expect(
      filterMarketListings(weapons, { itemType: 'weapon', subtype: 'sword', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['worn_sword']);
    expect(
      filterMarketListings(weapons, { itemType: 'weapon', subtype: 'dagger', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['keen_dirk']);
    expect(
      filterMarketListings(weapons, { itemType: 'weapon', subtype: 'staff', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['gnarled_staff']);
    expect(
      filterMarketListings(weapons, { itemType: 'weapon', subtype: 'mace', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
    ).toEqual(['training_mace']);
    expect(
      filterMarketListings(weapons, { itemType: 'weapon', subtype: 'axe', rarity: 'all' }).map(
        (l) => l.itemId,
      ),
