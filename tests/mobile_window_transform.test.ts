...
describe('tier player-frame seats remain centered after the mobile action-pad reflow', () => {
  // The old radial ring needed compact (-44px) and tablet (-10px) horizontal
  // compensation. The two-row pad no longer reaches into the player-frame seat,
  // so keep those geometry-specific offsets out of every orientation and tier.
  const css = readFileSync(
    fileURLToPath(new URL('../src/styles/hud.mobile.css', import.meta.url)),
    'utf8',
  ).replace(/\/\*[\s\S]*?\*\//g, '');

  it('does not reintroduce radial-ring horizontal compensation', () => {
    expect(css).not.toContain('left: calc(50% - 44px)');
    expect(css).not.toContain('left: calc(50% - 10px)');
  });
});
