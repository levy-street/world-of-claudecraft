...
describe('ability tooltip data reflects selected talents', () => {
  ...
  it('a damage talent raises Gaze of Ruin channel damage', () => {
    const base = resolved('demon_hunter', 'eye_beam', emptyModifiers())!;
    const baseBeam = base.effects.find((e) => e.type === 'directDamage') as
      | { min: number; max: number }
      | undefined;
    expect(baseBeam).toBeDefined();
    const known = resolved('demon_hunter', 'eye_beam', modsFor('eye_beam', { dmgPct: 0.12 }))!;
    const beam = known.effects.find((e) => e.type === 'directDamage') as {
      min: number;
      max: number;
    };
    expect(beam.min).toBe(Math.round(baseBeam!.min * 1.12));
    expect(beam.max).toBe(Math.round(baseBeam!.max * 1.12));
  });
