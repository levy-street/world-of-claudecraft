...
describe('websocket authentication', () => {
  it('pins the strict world auth epoch for symmetric mixed-release rejection', () => {
    ...
    // The previous server accepts only `auth-world-3`, so the new
    // client discriminator must remain necessarily unrecognizable to it.
    expect(ONLINE_WORLD_AUTH_TYPE).not.toBe('auth-world-3');
  });
