...
describe('websocket authentication', () => {
  it('pins the strict world-layout auth epoch for symmetric mixed-release rejection', () => {
    expect(ONLINE_WORLD_LAYOUT_VERSION).toBe(21);
    expect(ONLINE_WORLD_AUTH_TYPE).toBe(`auth-world-${ONLINE_WORLD_LAYOUT_VERSION}`);
    expect(ONLINE_WORLD_AUTH_TYPE).toBe('auth-world-21');
    // The previous layout-gated server accepts only `auth-world-9`, so the new
