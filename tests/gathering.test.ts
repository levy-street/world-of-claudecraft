...
describe('isHarvestableCorpse', () => {
  ...
  it('answers for every shipped template, and none is excluded any more', () => {
    ...
    // trash beast instead of dead content. Claw and tusk joining the yield table
    // then retires fen_troll from `excluded` into this list, so every tagged
    // template is harvestable: 42.
    expect(included).toHaveLength(42);
    // ...and the untagged templates are counted rather than assumed: 194 of
    // them ship, all excluded before this change and all excluded after it.
    // fen_troll was already tagged (claw, tusk), so it moves directly from
    // `excluded` into `included` without changing this count.
