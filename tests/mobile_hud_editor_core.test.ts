...
describe('mobile HUD scaled footprints and temporary viewport clamps', () => {
  ...
  it('protects the complete Minimap cluster including its satellite controls', () => {
    ...
    expect(resolved.canonicalRect).toEqual({ x: 162, y: 57, width: 156, height: 132 });
...
describe('one canonical mobile HUD context validator', () => {
  ...
  it('allows Delve text to overlap controls but reserves its one affix pocket', () => {
    const delveTracker = {
      ...get('tracker.delve'),
    ...
    expect(textOverlap.filter((failure) => failure.reason === 'overlap')).toEqual([]);
    expect(affixOverlap).toContainEqual(
      expect.objectContaining({
        reason: 'overlap',
        surfaceIds: ['action.a1', 'tracker.delve'],
      }),
    );
  ...
  it('blocks Save when an interactive player aura viewport overlaps the Target action', () => {
    ...
    expect(failures.some((failure) => failure.reason === 'overlap')).toBe(true);
  });
  ...
  it('protects every clickable slot in the expanded Consumables tray', () => {
    ...
    expect(expandedOnly).toMatchObject([
  ...
  it('never grandfathers an overlap between two interactive controls', () => {
    const registry = buildMobileHudRegistry({
    ...
    expect(check(130)).toMatchObject([
...
describe('complete mobile HUD profile matrix validator', () => {
  ...
  it('rejects a layout that becomes overlapping only after left-handed mirroring', () => {
  ...
  it('rejects a default control moved outside the safe viewport', () => {
    const phone = {
      ...MOBILE_HUD_REGISTRY.defaults.phone,
      'control.movement': {
  ...
  it('attaches active dynamic variants to a failing geometry', () => {
    ...
    expect(failures).toMatchObject([
      ...
      },
      {
        reason: 'out-of-bounds',
        handedness: 'left',
        activeVariantIds: ['closed', 'expanded-left-6'],
        safeAreaFixtureId: 'side-none/bottom-0',
      },
    ]);
  });
