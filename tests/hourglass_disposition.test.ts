...
describe('Hourglass safety is authored for the actual observer', () => {
  it.each(['2v2', 'fiesta', 'yumi3', 'battleground'] as const)(
    ...
    (format) => {
      ...
      const allies = [sim.playerId],
        enemies: number[] = [];
      for (let i = 1; i < size; i++) allies.push(sim.addPlayer('warrior', `Ally${i}`));
      for (let i = 0; i < size; i++) enemies.push(sim.addPlayer('rogue', `Enemy${i}`));
      if (format === 'battleground') startBgMatch(sim.ctx, allies, enemies);
      else if (format === 'yumi3') startYumiMatch(sim.ctx, format, allies, enemies);
      ...
      expect(sim.temporalHourglassDispositionFor(allies[0], enemies[0])).toBe('hostile');
      ...
      if (format === 'battleground') {
        ...
        expect(sim.temporalHourglassDispositionFor(allies[0], allies[1])).toBe('unknown');
      }
    },
  );
  ...
  it('changes a party member from protection to hostile control when a real duel starts', () => {
    ...
    expect(sim.temporalHourglassDispositionFor(sim.playerId, allyId)).toBe('protective');
    ...
    expect(sim.temporalHourglassDispositionFor(sim.playerId, allyId)).toBe('hostile');
  });
