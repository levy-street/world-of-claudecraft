...
describe('chat module (direct, no Sim)', () => {
  ...
  it('/dev mounts grants every catalog reins and raises the level to the riding gate', () => {
    ...
    // 10 since the Cluckwork Mech Bird joined the catalog. Spelled as a literal
    // on purpose rather than derived from MOUNT_KEYS.length: the row above
    // already proves ownership against the catalog, so deriving this one too
    // would let a catalog that silently lost a mount pass both.
    expect(
