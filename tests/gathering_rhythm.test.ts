...
describe('rod synergy is literal-pinned on one shared draw (QA pins)', () => {
  ...
  it('first-cast delay ticks at seed 4242: tier-1 pole 65, tier-2 rod 64, tier-3 rod 62', () => {
    // Re-recorded again after the v0.32.1 realm loot fill appended the four
    // restored Galecrest quest camps (the world-gen camp loop draws 5 rng
    // values per mob at Sim construction, so 12 more mobs shift the shared
    // stream and move the delay draw); the pinned property is unchanged: one
    // seed, one hidden draw, monotonically shorter with each rod tier.
