...
describe('classic formulas', () => {
  ...
  it("Reaver's Recoil launches Demon Hunters backward into the air", () => {
    ...
    const recoilDistance = startZ - p.pos.z;
    expect(recoilDistance).toBeGreaterThan(12);
    expect(recoilDistance).toBeLessThan(17);
  });

  it('Soul Bite resolves immediately without waiting for an auto-attack swing', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
  ...
  it('Soul Leech heals Demon Hunters for 7% of damage dealt', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    const wolf = nearestMob(sim, 'forest_wolf');
    wolf.level = 1;
    teleportTo(sim, wolf.pos.x + 2, wolf.pos.z);
    ...
    expect(p.auras).toContainEqual(
      expect.objectContaining({ id: 'demon_hunter_soul_leech', kind: 'buff_lifesteal', value: 0.07 }),
    );
    p.hp = p.maxHp - 50;
    p.resource = 0;
    sim.drainEvents();
    ...
    const damage = events.find(
      (e): e is Extract<SimEvent, { type: 'damage' }> =>
        e.type === 'damage' && e.sourceId === p.id && e.targetId === wolf.id,
    );
    const heal = events.find(
      (e): e is Extract<SimEvent, { type: 'heal' }> => e.type === 'heal' && e.targetId === p.id,
    );
    expect(damage?.amount ?? 0).toBeGreaterThan(0);
    expect(heal?.amount).toBe(Math.max(1, Math.round((damage?.amount ?? 0) * 0.07)));
    expect(heal).toEqual(
    ...
    expect(p.hp).toBeGreaterThan(p.maxHp - 50);
  });

  it('Fel Lunge unlocks at 6 and leaps from 10 yards toward the target', () => {
    expect(abilitiesKnownAt('demon_hunter', 5).map((k) => k.def.id)).not.toContain('felblade');
    expect(abilitiesKnownAt('demon_hunter', 6).map((k) => k.def.id)).toContain('felblade');
    ...
    wolf.pos = sim.groundPos(20, 29.8);
    ...
    p.resource = 0;
    const start = { ...p.pos };
    const startDistance = dist2d(p.pos, wolf.pos);

    sim.castAbility('felblade');

    expect(p.resource).toBeGreaterThan(0);
    expect(p.onGround).toBe(false);
    expect(p.jumping).toBe(true);
    expect(p.vz).toBeGreaterThan(10);
    expect(dist2d(start, p.pos)).toBeLessThan(0.1);

    for (let i = 0; i < 60 && !p.onGround; i++) sim.tick();

    expect(dist2d(p.pos, wolf.pos)).toBeLessThan(startDistance);
    expect(dist2d(p.pos, wolf.pos)).toBeLessThan(2.5);
    expect(dist2d(start, p.pos)).toBeGreaterThan(8);
    expect(dist2d(start, p.pos)).toBeLessThan(10);
  });

  it('Felsurge dashes horizontally without adding a jump', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    ...
    expect(p.vy).toBe(0);
    expect(p.abilityDashRemaining).toBeGreaterThan(15);
    ...
    expect(rushDistance).toBeGreaterThan(14);
    expect(rushDistance).toBeLessThan(18);
  });

  it('Felsurge preserves the current height if cast while airborne', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    sim.setPlayerLevel(20);
    teleportTo(sim, 20, 20);
    const p = sim.player;
  ...
  it('Demon Ascendance applies the demon form aura used by the renderer', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    sim.setPlayerLevel(20);
    const p = sim.player;
    p.resource = p.maxResource;
  ...
  it('Flame Rune emits a duration-carrying ground ring cue', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    sim.setPlayerLevel(20);
    const p = sim.player;
    p.resource = p.maxResource;
  ...
  it('Burning Brand deals its damage over time around the Demon Hunter', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    sim.setPlayerLevel(20);
    const wolf = nearestMob(sim, 'forest_wolf');
    wolf.level = 1;
    ...
    for (let i = 0; i < 25; i++) {
      tickDamage += sim.tick().filter(
        (ev) => ev.type === 'damage' && ev.ability === 'Burning Brand' && ev.amount > 0,
      ).length;
    }
  ...
  it('Gaze of Ruin requires a target and channels beam damage into it', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    sim.setPlayerLevel(20);
    const wolf = nearestMob(sim, 'forest_wolf');
    wolf.level = 1;
    wolf.maxHp = 500;
    wolf.hp = 500;
    teleportTo(sim, wolf.pos.x + 6, wolf.pos.z);
    facePlayerAt(sim, wolf);
    const p = sim.player;
    p.resource = p.maxResource;
    ...
    let events = sim.tick();
    expect(events.some((ev) => ev.type === 'error' && ev.text === 'You have no target.')).toBe(true);

    sim.targetEntity(wolf.id);
    sim.castAbility('eye_beam');

    const channeled: SimEvent[] = [];
    for (let i = 0; i < 20; i++) channeled.push(...sim.tick());

    expect(channeled.some((ev) => ev.type === 'spellfx' && ev.fx === 'beam' && ev.targetId === wolf.id)).toBe(true);
    expect(channeled.some((ev) => ev.type === 'damage' && ev.targetId === wolf.id && ev.ability === 'Gaze of Ruin')).toBe(true);
    expect(wolf.hp).toBeLessThan(500);
  });

  it('Razor Cyclone blinks around the selected enemy three times and returns to the cast point', () => {
    const sim = new Sim({ seed: 42, playerClass: 'demon_hunter', playerName: 'Illidari' });
    sim.setPlayerLevel(20);
    const wolf = nearestMob(sim, 'forest_wolf');
    wolf.level = 1;
