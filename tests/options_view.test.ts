...
describe('options_view: controller dispatch matrix (cluster 5)', () => {
  it('lists the enable/follow/invert toggles then the three controller sliders', () => {
    const controls = buildControllerControls(makeSource());
    expect(keysOf(controls)).toEqual([
      'gamepadEnabled',
      'gamepadCameraAutoFollow',
      'gamepadInvertX',
      'gamepadInvertY',
      'gamepadInvertMoveX',
      'gamepadInvertMoveY',
    ...
    expect(find(controls, 'gamepadCameraAutoFollow')).toMatchObject({ control: 'boolToggle' });
    expect(find(controls, 'gamepadInvertX')).toMatchObject({ control: 'boolToggle' });
