...
| hudHotDomWrites | 1062 | ARM 3 anchor: elision-bypass writes, every viewport (worst healthy capture 1054, DESKTOP). KEPT and RE-GROUNDED 2026-08-30 at the eighth v0.41.0 sync (same capture machine): the CLEAN RELEASE-TIP capture the Phase 16 QA recorded as owed ran against origin/release/v0.41.0 3e801dc925 itself (a scratch worktree on :5177, two headed runs) and reads desktop 1054 cold with one warm 1074 outlier under six-lane vitest load, mobile 573/589, against the merged branch tip 4f72218ed4's steady 1054/1054 desktop and 573/564 mobile (two runs on :5173): the anchor's 1054 + 8 derivation now rests on a clean release capture rather than a chain of same-machine A/B raises, and the branch adds zero writes on either viewport. One release-side warm run read 1074 under concurrent six-lane vitest load, never reproduced on the branch side: recorded as load jitter in the committed evidence, not absorbed. Evidence: docs/screenshots/masterwrought-phase-16/perf-tour-ab-2026-08-30-eighth-sync.json. Prior derivation kept below for the history. Re-captured 2026-08-30 (the Masterwrought Phase 16 QA, same capture machine): release-side growth again, branch-neutral by a same-machine A/B in the Phase 14 discipline: the pre-phase base f90b3f5812, which already contains the seventh v0.41.0 sync (e19d832b47: the bank and daily-rewards standing HUD windows among it), measures 1054 on BOTH runs, and the branch tip after the whole Phase 16 span plus the QA fix round measures 1045 to 1054, byte-identical at the top: zero branch-authored desktop writes. Mobile 573 to 589 on both sides of the A/B (base 573/573, branch 589/573), still far under the desktop row. The prior anchor (1014, re-captured 2026-08-28 pre-seventh-sync) is superseded; its attribution note is kept below for the history. frameLong50 on the 08-30 warm captures: 0 to 3 (the 12 anchor and 500 floor KEPT; warm desktop frames 1418+). Same 8-write headroom discipline (1054 + 8). The A/B's per-run numbers are COMMITTED at docs/screenshots/masterwrought-phase-16/perf-tour-ab-2026-08-30.json (both sides, both runs, both viewports), so the raise is checkable against evidence rather than this prose alone. PRIOR ROW, superseded: re-captured 2026-08-28 (the Masterwrought Phase 14 QA, same capture machine): release-side growth, branch-neutral by a three-way same-machine A/B (the pre-phase base 6bf465acab, release cb10309ba6 plus Phases 1 to 13, measures 987; the release tip at the sync, 8592df3866, measures 1006; the branch after that sync and the Phase 14 commits together 981 to 986; mobile 547 to 569 on all three). Desktop was already the worst viewport at the pre-phase base, so the flip from mobile happened between the 2026-08-08 capture and cb10309ba6. THAT CLEAN CAPTURE NOW EXISTS (2026-08-31, Masterwrought Phase 18, same capture machine): a throwaway worktree checked out at cb10309ba6 itself, its own pnpm install and vite on :5197, two headed PERF_GPU=1 PERF_VIEWPORT=both runs, reading desktop 1006 then 1004 and mobile 524 then 542. So the whole 706-to-1014 move is release-side and had ALREADY LANDED at cb10309ba6: no branch commit and nothing after cb10309ba6 established it, and the desktop/mobile flip is confirmed present at cb10309ba6 (1004 desktop against 542 mobile) rather than inferred from the base. It also cross-checks the three-way A/B in this row from the other side: clean cb10309ba6 at 1004 against the pre-phase base 6bf465acab (cb10309ba6 plus Phases 1 to 13) at 987 means Phases 1 to 13 added no desktop writes either, measuring one band BELOW the release commit they sit on. What stays unmeasured is narrower and now bounded: WHICH release change inside the 2026-08-08-to-cb10309ba6 span did it, which needs a bisect over that span rather than another tip capture. Frame figures from these two runs are NOT anchor-grade and were not used as such: the machine carried a load average near 19 on 16 cores (concurrent agent work), which the bypass COUNT is immune to by construction (run-length-independent, and the two runs agree within 2 writes) while frameLong50/frameP95 are not. The cb10309ba6-to-8592df3866 span adds nothing measurable on the branch. Frame counts on the 08-28 warm captures: desktop 1418 to 1438, mobile 1564 to 1569, all clearing the 500 floor. The 2026-08-08 row (706, worst 698 mobile) is superseded. Same 8-write headroom discipline. frameLong50 on the same captures: 1 to 2 on every warm run (a first cold headed launch measured 13 once, the cold-start artifact, not a regression); the 12 anchor and the 500 floor are KEPT. |
