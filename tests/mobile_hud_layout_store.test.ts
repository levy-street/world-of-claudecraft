...
describe('LocalMobileHudLayoutStorage', () => {
  it('implements the async adapter contract with the versioned key', async () => {
    ...
    expect(values.get('woc_mobile_hud_layout_v1')).toBe('serialized-layout');
...
describe('validated mobile HUD layout loading', () => {
  ...
  it('falls back one invalid profile without replacing its valid sibling', async () => {
    const serialized = JSON.stringify({
      ...
      profiles: {
        phone: {
          'action.a1': { anchor: 'top-left', offsetX: 720, offsetY: 20, scale: 1 },
        },
        tablet: {
  ...
  it('falls back a stored profile whose button overlaps the expanded Consumables tray', async () => {
    ...
    expect(result.profileFallbacks).toContain('phone');
    expect(result.document.profiles.phone).toEqual(MOBILE_HUD_REGISTRY.defaults.phone);
  });
  ...
  it('never rewrites stored bytes when matrix validation falls back', async () => {
    const serialized = JSON.stringify({
      ...
      profiles: {
        phone: {
          'action.a1': { anchor: 'top-left', offsetX: 9999, offsetY: 20, scale: 1 },
        },
      },
    });
    let writes = 0;
  ...
  it('does not let an unavailable class-specific surface block saving', async () => {
    ...
    const draft = {
      ...
      enabled: false,
      profiles: {
        phone: classDefaults.phone,
        tablet: classDefaults.tablet,
      },
    };
    const writes: string[] = [];
