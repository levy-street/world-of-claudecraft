...
describe.skipIf(!PG_TEST_URL)('payout_db buyback_batches (real Postgres)', () => {
  ...
  it('advances a batch through the full swap→settle lifecycle, setting each column', async () => {
    ...
    const r = await pool.query(
      `SELECT status, woc_bought::text wb, woc_settled::text ws, settle_tx_sig, settle_broadcast_at IS NOT NULL sb, executed_at IS NOT NULL ex FROM buyback_batches WHERE batch_id='b1'`,
    );
    expect(r.rows[0]).toMatchObject({
      status: 'settled',
      wb: '700',
      ws: '700',
      settle_tx_sig: 'settle1',
      sb: true,
      ex: true,
    });
  });
  ...
  it('markBatchFailed truncates an overlong reason to 500 chars', async () => {
    ...
    const r = await pool.query(
      `SELECT status, length(fail_reason) len FROM buyback_batches WHERE batch_id='b1'`,
    );
  ...
  it('withPayoutKeeperLock grants exclusive access — a second holder gets null while the first holds it', async () => {
    ...
    const first = dbm.withPayoutKeeperLock(async () => {
      acquired();
      await releaseP;
      return 'a';
    });
