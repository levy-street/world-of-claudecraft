...
const db = vi.hoisted(() => ({
  ...
  touchCharacterLogin: vi.fn(async (): Promise<void> => {}),
  ...
  markAccountQuestComplete: vi.fn(
    async (): Promise<any> => ({ completedQuestIds: [], mechChromaIds: [] }),
  ),
  grantAccountMechChroma: vi.fn(
    async (): Promise<any> => ({ completedQuestIds: [], mechChromaIds: [] }),
  ),
  revokeAccountMechChroma: vi.fn(
    async (): Promise<any> => ({ completedQuestIds: [], mechChromaIds: [] }),
  ),
  ...
  const sim = (
    server as unknown as {
      sim: {
        ...
        setPlayerSkin: (pid: number, skin: number, cat: string, csk: string | null) => void;
      };
    }
  ).sim;
...
beforeEach(() => {
  ...
  db.getNftCollection.mockResolvedValue({
    chain: 'ethereum',
    contract: '0xc',
    name: 'C',
    standard: 'erc721',
