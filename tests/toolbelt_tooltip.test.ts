// Toolbelt item tooltip line: the pure string-builder composed inside
...
describe('toolbeltTooltipLines', () => {
  it('states what the belt is for', () => {
    expect(toolbeltTooltipLines(ITEMS.toolbelt)).toBe(`<div class="tt-desc">${DESC}</div>`);
  });
  ...
  it('covers every toolbelt in the catalog, not just the one id', () => {
