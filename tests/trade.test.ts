...
describe('trade module (direct, no Sim)', () => {
  ...
  // mirroring how takeInstanceSlot/addItemInstance behave on the real Sim
  // (src/sim/items.ts), so the trade payload-preservation fix and the capacity
  // gate (src/sim/social/trade.ts giveOffer/fitsAfterSwap) are exercised end
  ...
  function makeInstancedTradeCtx(inv1: any[], inv2: any[]) {
    ...
    const ctx = {
      ...
      tradeRails: (): TradeRailsView => OFFLINE_RAILS,
    } as unknown as SimContext;
  ...
  it('preserves an instanced item payload (enchant/signature/rolled quality) across a swap', () => {
    ...
    // Instance-aware offers (F1/F1a): a signed/rolled copy is offered ONLY as an
    // explicit count-1 instance row, never a bare fungible count.
  ...
  it('rejects a trade that would push the receiver over bag capacity via an instanced grant', () => {
    ...
    tradeMod.tradeRequest(ctx, 2, 1);
    tradeMod.tradeAccept(ctx, 2);
    // Instance-aware offers (F1/F1a): the giver's only copy is instanced, so the
    // offer must select it via an explicit count-1 instance row.
    tradeMod.tradeSetOffer(ctx, [{ itemId: 'wolf_fang', count: 1, instance }], 0, 0, '0', 1);
    tradeMod.tradeConfirm(ctx, 1);
    tradeMod.tradeConfirm(ctx, 2);
  ...
  it('carries a mixed offer of the giver’s plain and instanced copies in one transfer', () => {
    ...
    // offers never auto-split a single count across sources, F1/F1a), so
    // giveOffer's plain and instance-row arms both fire in the same confirm.
