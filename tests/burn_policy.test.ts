...
describe('shouldRunBatch — threshold / cadence / fee floor', () => {
  ...
  it('runs below threshold once the cadence (6h) has elapsed, if above the floor', () => {
    const now = 100 * HOUR;
    expect(
      shouldRunBatch({ availableUsdc: usdc(50), lastBurnAt: now - 7 * HOUR, now, policy: P }),
    ).toBe(true);
    expect(
      shouldRunBatch({ availableUsdc: usdc(50), lastBurnAt: now - 5 * HOUR, now, policy: P }),
    ).toBe(false); // cadence not elapsed
  });

  it('NEVER runs below the fee-aware floor ($25), even long past cadence', () => {
    const now = 1000 * HOUR;
    expect(shouldRunBatch({ availableUsdc: usdc(24.99), lastBurnAt: 0, now, policy: P })).toBe(
      false,
    );
  ...
  it('treats a never-burned vault (lastBurnAt null) as cadence-elapsed', () => {
    expect(shouldRunBatch({ availableUsdc: usdc(30), lastBurnAt: null, now: 0, policy: P })).toBe(
      true,
    );
    expect(shouldRunBatch({ availableUsdc: usdc(10), lastBurnAt: null, now: 0, policy: P })).toBe(
      false,
    ); // still below floor
  });
  ...
  it('fires at EXACTLY the cadence boundary (>=, not >)', () => {
    const now = 100 * HOUR;
    expect(
      shouldRunBatch({ availableUsdc: usdc(50), lastBurnAt: now - P.cadenceMs, now, policy: P }),
    ).toBe(true); // sinceLast === cadenceMs
    expect(
      shouldRunBatch({
        availableUsdc: usdc(50),
        lastBurnAt: now - P.cadenceMs + 1,
        now,
        policy: P,
      }),
    ).toBe(false); // one ms short
  });
  ...
  it('the floor takes precedence over the threshold when misconfigured (floor > threshold)', () => {
    ...
    expect(
      shouldRunBatch({ availableUsdc: usdc(260), lastBurnAt: 0, now: 1000 * HOUR, policy: skewed }),
    ).toBe(false);
    expect(
      shouldRunBatch({ availableUsdc: usdc(300), lastBurnAt: 0, now: 1000 * HOUR, policy: skewed }),
    ).toBe(true);
  });
});
...
describe('planTwapChunks — split large pools, conserve the total', () => {
  ...
  it('the split fires the moment the pool exceeds the split threshold by one base unit', () => {
    ...
    expect(planTwapChunks(P.twapSplitAboveUsdc + 1n, P)).toEqual([
      usdc(250),
      usdc(250),
      usdc(250),
      usdc(250),
