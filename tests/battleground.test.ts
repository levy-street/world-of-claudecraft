...
describe('Thornhollow Fields: the graveyard rite', () => {
  ...
  it('outgoing damage cancels a battleground wall-trap Unstuck countdown', () => {
    const { sim, pids } = tenInQueue();
    const match = must(sim.bgMatchFor(pids[0]), 'bg match');
    toActive(sim, match);
    const pid = match.teams[0][0];
    ...
    sim.ctx.dealDamage(e, target, 1, false, 'physical', null, 'hit');
    expect(meta.counters.damageDealt).toBeGreaterThan(damageDealt);
    e.inCombat = true;
    e.combatTimer = 0;
  ...
  it('owned companion damage cancels a battleground wall-trap Unstuck countdown', () => {
    ...
    sim.drainEvents();
    const damageDealt = meta.counters.damageDealt;
    sim.ctx.dealDamage(pet, target, 1, false, 'physical', null, 'hit');
    expect(meta.counters.damageDealt).toBe(damageDealt);
    e.inCombat = true;
    e.combatTimer = 0;
