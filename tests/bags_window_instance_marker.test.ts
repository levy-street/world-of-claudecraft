...
describe('bags grid instanced-slot marker', () => {
  ...
  it('composes procedural identity with enchanting in both the glyphs and accessible name', () => {
    const root = windowFor([
      {
        itemId: 'ironedge_longsword',
        count: 1,
        instance: {
          ...
          procedural: {
            version: 1,
            uid: 'pi1:bag:legendary-enchanted',
            baseId: 'ironedge_longsword',
            itemLevel: 20,
            rarity: 'legendary',
            affixes: [],
    ...
    const cell = root.querySelector('button.bag-item');
    expect(cell?.getAttribute('aria-label')).toBe(
      "Greyjaw's Edge, Legendary, item level 20, quantity 1, enchanted copy",
    );
    expect(cell?.querySelector('.bi-glyph-enchanted')).not.toBeNull();
    expect(cell?.querySelector('.bi-power-rune')).not.toBeNull();
    expect(cell?.querySelector('.bi-instance')).toBeNull();
