...
const MOBILE_WINDOW_EXCEPTIONS: Record<string, string> = {
  ...
  'hodrics-window':
    "the Hodric's Castle race window is a JS-populated .window.panel sized entirely by the shared body.mobile-touch .window sheet base (max-width/max-height); the board fills the sheet width like the other full-width panels, so it carries no id-scoped pin of its own",
};
