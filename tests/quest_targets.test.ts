...
describe('questObjectiveAreas', () => {
  ...
  it('groups gather nodes into stable clusters across the whole threshold plateau', () => {
    ...
    // After the Farshore relocation, the identical-partition band shared by
    // every gathering type is 27 to 31 yards; 26 and 32 each change at least
    // one type. The shipped 30 therefore remains inside a decisive plateau.
    ...
    for (const type of GATHER_NODE_TYPES) {
      ...
      expect(key(gatherNodeClusters(type, 31)), `${type} at 31yd`).toBe(at30);
