...
describe('Reliquary Conqueror catalog structure', () => {
  ...
  it('pins the catalog totals through the production completion math', () => {
    ...
    // Wildheart Basin, boneglass_shiv on Spoils): 340, plus the
    // barrel on the Horizons shelf: 341. Catalog growth reverts
  ...
  it('pins the final measured catalog shape: total slots and distinct marks', () => {
    ...
    // the barrel (tavern_barrel) on the Horizons mounts shelf: 376 total.
...
const SOURCE_PENDING_RULING: Readonly<Record<string, readonly string[]>> = {
  ...
  // tavern_barrel: the barrel ships
  ...
  horizons_mounts: ['drakemaw_raptor', 'terrorspark_groundshaker', 'tavern_barrel'],
...
describe('Reliquary source hint coverage', () => {
  ...
  it('SOURCE_PENDING_RULING is exactly the un-hinted set (no stale exclusions)', () => {
    ...
    // excluded. Literal: tighten as rulings land. 368 = 376 slots minus the
    // four retired vault slots minus the THREE gap mounts minus the pended
    // masterwork:engineering. It tracks the slot total, so it moved with the
    // three daggers the v0.36.0 release merge added; the barrel adds one slot
    // and one gap at once, so the floor holds where it was.
