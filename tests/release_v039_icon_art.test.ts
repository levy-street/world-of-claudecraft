...
const SECOND_PASS_RECORD_SHA256 =
  'f675f2439b8b83b59f47f168eca58d6d88c73457ba7fc33ef46d5a2e431fe95f';
const EVIDENCE = {
...
describe('release v0.39 icon-art second-pass lineage', () => {
  it('seals every cohort and new-family shipping catalog behind one immutable record', () => {
    ...
    expect(aggregate).toMatchObject({
      ...
      runtimeClosure: {
        ...
        hotbarItems: { live: 74, painted: 74 },
  ...
  it('derives the sealed ability and hotbar-item totals from live production inventories', () => {
    ...
    // 74 with the Riftbound Boulder's reins: a mount reins is a usable item, so
    // it enters the hotbar inventory the same way every other reins does.
    expect(liveHotbarItemIds, 'production isHotbarItemId inventory').toHaveLength(74);
    expect(
