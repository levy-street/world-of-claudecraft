...
import {
  ...
  onWocSeasonChange,
  setWocSeason,
  setWocSeasonUiEnabled,
  type WocSeasonPayload,
  wocSeasonUiEnabled,
} from '../src/ui/woc_season';

const NOW = Date.parse('2026-06-21T00:00:00.000Z');
const season = (
  over: Partial<NonNullable<WocSeasonPayload['season']>> = {},
  standings: WocSeasonPayload['standings'] = [],
): WocSeasonPayload => ({
  decimals: 6,
  standings,
  season: {
...
describe('formatSeasonView', () => {
  ...
  it('maps projected standings to exact $WOC reward strings, preserving order', () => {
    const v = formatSeasonView(
      season({ poolBase: '2700000000' }, [
  ...
  it('derives pool/sink/emission as exact $WOC and the emitted percentage', () => {
    const v = formatSeasonView(
      season({ sinkBase: '4200000000', emissionBase: '1500000000', poolBase: '2700000000' }),
      NOW,
    );
  ...
  it('reports 0% emitted when there are no sinks yet (avoids divide-by-zero)', () => {
    expect(
      formatSeasonView(season({ sinkBase: '0', emissionBase: '0', poolBase: '0' }), NOW).emittedPct,
    ).toBe(0);
  });
  ...
  it('computes a countdown for a future end time', () => {
    ...
    expect(v.countdown).toEqual({
      days: 3,
      hours: 6,
      minutes: 30,
      totalMs: 3 * 86_400_000 + 6 * 3_600_000 + 30 * 60_000,
    });
  });
  ...
  it('marks a closed/finalized season as ended even if its clock has not run out', () => {
    const v = formatSeasonView(
      season({ status: 'closed', endsAt: '2099-01-01T00:00:00.000Z' }),
      NOW,
    );
    expect(v.state).toBe('ended');
    expect(v.countdown).toBeNull();
