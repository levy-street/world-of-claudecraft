...
// stay real.
const db = vi.hoisted(() => ({
  createMarketplaceQuote: vi.fn(),
  getMarketplaceQuote: vi.fn(),
  deleteMarketplaceQuote: vi.fn(),
  redeemPurchase: vi.fn(),
  listLiveCreatorSkins: vi.fn(async () => []),
  getCreatorSkin: vi.fn(async (_id: string) => null as unknown),
  walletForAccount: vi.fn(async (_a: number) => ({
  ...
  setCreatorSkinReview: vi.fn(async () => {}),
  setCreatorSkinStatus: vi.fn(async () => {}),
  setCreatorTrusted: vi.fn(async () => {}),
  bumpApprovedListings: vi.fn(async () => {}),
  recordCreatorStrike: vi.fn(async () => {}),
}));
vi.mock('../server/db', () => db);
const woc = vi.hoisted(() => ({
  holderInfoForPubkey: vi.fn(async () => ({ tier: 3, balance: 100 })),
}));
vi.mock('../server/woc_balance', () => woc);
const pin = vi.hoisted(() => ({
  pinFileToIPFS: vi.fn(async () => 'QmDEADBEEF'),
  unpinFromIPFS: vi.fn(async () => {}),
  pinataEnabled: vi.fn(() => true),
}));
...
beforeEach(() => {
  vi.clearAllMocks();
  woc.holderInfoForPubkey.mockResolvedValue({ tier: 3, balance: 100 });
  pin.pinataEnabled.mockReturnValue(true);
  ssa.validateSkinPng.mockReturnValue({ ok: true, sha256: 'abc', width: 1024, height: 1024 });
});
...
describe('createHostedListing — quota gate + pin + review', () => {
  ...
  it('below quota: pins to IPFS and lists pending (untrusted creator)', async () => {
    ...
    const row = db.upsertCreatorSkin.mock.calls[0][0] as {
      source: string;
      ipfsCid: string;
      reviewStatus: string;
      assetUrl: string;
  ...
  it('trusted creator (whale, 1 approval) lists instant-live', async () => {
    ...
    db.getCreatorTrust.mockResolvedValue({
      approvedCount: 1,
      strikes: 0,
      lastStrikeAt: null,
      trusted: true,
      firstListingAt: new Date(Date.now() - 1e9).toISOString(),
    });
    const r = await createHostedListing({ ...base, bytes: Buffer.from('x') }, 7, WALLET);
  ...
  it('refuses when Pinata is unconfigured', async () => {
    ...
    expect(await createHostedListing({ ...base, bytes: Buffer.from('x') }, 7, WALLET)).toEqual({
      ok: false,
      reason: 'hosting_unavailable',
    });
  });
...
describe('createSelfHostedListing — SSRF-mapped failure + free path', () => {
  it('maps an SSRF/transport throw to a clean failure (nothing persisted)', async () => {
    ...
    expect(
      await createSelfHostedListing({ ...base, originUrl: 'http://169.254.169.254/x' }, 7, WALLET),
    ).toEqual({ ok: false, reason: 'url_private_host' });
    expect(db.upsertCreatorSkin).not.toHaveBeenCalled();
  });
  it('free path persists with origin_url + opaque assetUrl, pending review', async () => {
    const r = await createSelfHostedListing(
      { ...base, originUrl: 'https://cdn.example/s.png' },
      7,
      WALLET,
    );
    expect(r.ok).toBe(true);
...
describe('enforceHostedQuota — park newest over-quota, keep oldest', () => {
  it('hides the newest (count - quota) and leaves the rest', async () => {
    ...
    db.listHostedSkinsByAccount.mockResolvedValueOnce([
      { id: 'd', overflowHidden: false },
      { id: 'c', overflowHidden: false },
      { id: 'b', overflowHidden: false },
      { id: 'a', overflowHidden: false },
    ]);
  ...
  it('restores parked skins on a top-up (quota now covers all)', async () => {
    db.listHostedSkinsByAccount.mockResolvedValueOnce([
      { id: 'd', overflowHidden: true },
      { id: 'c', overflowHidden: true },
      { id: 'b', overflowHidden: false },
      { id: 'a', overflowHidden: false },
    ]);
...
describe('reviewListing / removeListing — moderation + trust accrual', () => {
  it('approve credits the creator + recomputes trust', async () => {
    db.getCreatorSkin.mockResolvedValueOnce({
      id: 's1',
      source: 'hosted',
      creatorAccountId: 7,
    } as never);
