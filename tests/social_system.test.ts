...
describe('guilds', () => {
  ...
  describe('renameGuildAsLeader (paid $WOC rename path)', () => {
    ...
    it('announces the new name to online guild members', async () => {
      await h.svc.guildCreate(h.actor(1), 'Iron Vanguard');
      await h.svc.guildInvite(h.actor(1), 'Bet');
      await h.svc.guildAccept(h.actor(2));
      h.tx.clear();
      await h.svc.renameGuildAsLeader(1, 'Golden Vanguard');
      expect(h.tx.textFor(2)).toContain('The guild has been renamed to <Golden Vanguard>.');
    });
  });
