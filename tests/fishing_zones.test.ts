...
const DECORATIVE_LAKES = ['farshore_isle:1040,-60', 'veiled_hollow:-120,1105'];
...
describe('every rod-tier row stands on real water and a real schedule row', () => {
  ...
  it('the hub-shore flood can fail: open sea is not dry foot and stays unreached', () => {
    ...
    const OFF_STRAND_SEA_FLOOR = { x: 740, z: -180 };
  ...
  it('the open sea is unfishable: a seaward coastal cast denies with the no-water error', () => {
    ...
    // it stays green. The Last Bell island's west-facing Landing beach is
    // the campaign equivalent of the release island's old south strand.
    expect(terrainHeight(734, -30, WORLD_SEED)).toBeLessThan(waterLevel());
    expect(Number.isFinite(waterLevelAt(734, -30, WORLD_SEED))).toBe(true);
    expect(isInWaterBody(734, -30)).toBe(false);
    const sim = makeSim();
    const meta = sim.meta(sim.playerId) as PlayerMeta;
    sim.addItem('silverstream_fishing_rod', 1);
    teleportTo(sim, 758, -30);
    sim.player.facing = -Math.PI / 2; // toward -x, off the Landing and out to sea
    const evStart = sim.events.length;
    startFishing(sim.ctx, sim.player, meta);
    expect(sim.player.castingAbility).toBeNull();
    expect(sim.events.slice(evStart)).toContainEqual(
  ...
  it('Gull Mere is deeply fishable with a walkable dry shore, at the shipped seed', () => {
    ...
    // below the campaign port's measured 0.67 so terrain noise never flakes it.
    const farshore = ZONES.find((z) => z.id === 'farshore_isle');
    const mere = farshore?.lakes.find((l) => l.x === 891 && l.z === 115);
    expect(mere).toEqual({ x: 891, z: 115, radius: 10 });
