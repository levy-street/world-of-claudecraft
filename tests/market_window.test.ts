...
describe('ClientWorld: reconnect re-push of session preferences (#2723 review)', () => {
  function bareClientWithSocket(): { client: any; sent: Array<Record<string, unknown>> } {
    ...
    // A hello always queues authoritative scene/choice convergence before it
    // replays session preferences. This bare prototype fixture needs the same
    // queue the real constructor creates so the reconnect path reaches the
    // preference assertion instead of failing on unrelated scene plumbing.
    client.eventQueue = [];
