...
// 15303, 30 to 785, 122 to 492), and re-hunted AGAIN after the mob-loot fill
// appended four camps. World-gen draws 5 rng values per camp mob, so the 12
// new mobs put every seed's stream back near its pre-respacing position and
// the ORIGINAL seeds reproduce their slots' rolls again (60 back to 3, 991
// back to 14, 12 back to 22, 15303 back to 23, 785 back to 30). Each seed
// reproduces its slot's ORIGINAL roll exactly, quantity and rarity and kind,
// so every `yields` literal below is unchanged: the seeds moved and nothing
// else did. The two downgrade arms that sat on seed 5 moved to 22 (the same
// seed the per-arm bag-full cases use): seed 5's focused rolls stopped
// clearing the signable floor, and those arms exist to drive the mark-loss and
// find-loss grant sites. The three-and-four-property slots are genuinely
// scarce (the all-three-kinds row below is about a 1-in-4600 draw).
...
describe('one harvestResult per harvest command (#2457)', () => {
  ...
  it('reports quantities that match what actually landed in the bags', () => {
    ...
    // Never vacuous on the arm this case was widened for: seed 14 must really
...
describe('the four grant arms each report themselves (#2457)', () => {
  ...
  it('full-bag mark loss: the entry reports the PLAIN top-up it actually became', () => {
    ...
    const { sim, internals, a, mob } = setup(22);
...
describe('the worst case in shipped content (#2457)', () => {
  it('a Mire Prowler double proc is four entries, one per distinct item, in one event', () => {
    ...
    // Re-seeded 1 -> 11 (#2514) -> 104 (the v0.32.0 base merge) -> 276 (the
    // mob-loot fill's appended camps). mire_prowler is a MIXED corpse and claw
    // no longer costs a tier roll, so every draw after the first shifts
    // whenever either branch touches the harvest or world-gen sequence, and the
    ...
    // yields below are unchanged from #2514 precisely because 276 was picked to
    // reproduce them.
    const { sim, a, mob } = setup(276, 'mire_prowler');
