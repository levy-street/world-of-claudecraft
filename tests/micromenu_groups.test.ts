...
function sectionsOf(html: string, colId: string): Array<[string, string[]]> {
  ...
  return column
    ...
    .slice(1)
    .map((chunk) => {
      const name = /^\s+data-group="([a-z]+)"/.exec(chunk)?.[1] ?? '';
      const body = chunk.slice(0, chunk.indexOf('</div>'));
...
describe('micro-menu sections: the keyline and its hidden-section rule', () => {
  ...
  it('draws the divider on the later sibling, in the shared showcase keyline token', () => {
    const rule = block('.micro-group + .micro-group');
    expect(rule).toMatch(/border-top:\s*1px solid var\(--color-border-showcase\)/);
    // Later-sibling only: a border on `.micro-group` itself would put a stray
    // rule above the topmost section in each column.
    expect(block('.micro-group')).not.toMatch(/border/);
    // No new colors: the token has to exist upstream in tokens.css.
  ...
  it('stacks the divider perpendicular to the column axis', () => {
    ...
    // so the divider must be a horizontal rule (border-TOP), never an inline one.
  ...
  it('hides a section that has nothing visible left in it', () => {
    // Structural, not per-button: this is what keeps a keyline from painting
    // with no content under it when the section's only entries are hidden.
  ...
  it('keeps .micro-group free of anything that opens a stacking context', () => {
    ...
    const rule = block('.micro-group');
    for (const prop of ['position', 'z-index', 'opacity', 'transform', 'filter']) {
      expect(rule, `.micro-group must not declare ${prop}`).not.toMatch(
        new RegExp(`(?:^|[\\s;])${prop}\\s*:`),
      );
    }
