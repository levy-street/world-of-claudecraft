...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    ...
    // (castAbilityOn, pickRowTalent, rowPicks, readyCheckRespond, ...) yields 208/55/153.
    expect(IWORLD_MEMBERS.length).toBe(208);
    expect(DATA_MEMBERS.length).toBe(55);
    expect(METHOD_MEMBERS.length).toBe(153);
  });
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 25 facets', () => {
  ...
  it('the union of the 25 facets equals the pinned 208-member IWORLD_MEMBERS set', () => {
