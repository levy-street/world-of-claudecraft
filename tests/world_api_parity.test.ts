...
//
// `IWorld` (src/world_api.ts, 221 members) is the ONE seam render/ui depend
...
// it lands the matching edit here in the SAME commit. The count pins (221 / 59 / 162)
...
// The facet interfaces the W1 split produced (src/world_api/<facet>.ts), plus the
// bank, vale-cup, gauntlet, and hodrics facets added by later features. Imported
...
import type { IWorldHodrics } from '../src/world_api/hodrics';
...
// The 221 members of `interface IWorld`, in interface order (world_api.ts).
// Partition: 59 `data` + 162 `method` (read-returning + command-void + async).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    expect(IWORLD_MEMBERS.length).toBe(221);
  ...
  it('the sorted method-kind set is exactly the pinned 162', () => {
...
//   (4) their union, sorted, equals the pinned 221-name IWORLD_MEMBERS set (a member
...
const FACET_HODRICS = [
  ...
  'hcQueueLeave',
  'hcPracticeStart',
] as const satisfies readonly (keyof IWorldHodrics)[];
type _ExhaustHodrics = AssertNever<Exclude<keyof IWorldHodrics, (typeof FACET_HODRICS)[number]>>;
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 27 facets', () => {
  ...
  it('the union of the 27 facets equals the pinned 221-member IWORLD_MEMBERS set', () => {
    const union = Object.values(FACET_MEMBER_ARRAYS).flatMap((arr) => [...arr]);
    expect(union.length, 'union size before dedup (catches a duplicated member)').toBe(221);
    expect(new Set(union).size, 'union size after dedup (catches a duplicated member)').toBe(221);
