...
//
// `IWorld` (src/world_api.ts, 235 members) is the ONE seam render/ui depend
...
// it lands the matching edit here in the SAME commit. The count pins (235 / 62 / 173)
...
// The 235 members of `interface IWorld`, in interface order (world_api.ts).
// Partition: 62 `data` + 173 `method` (read-returning + command-void + async).
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  ...
  it('the full sorted member set is exactly the pinned 235', () => {
...
//   (4) their union, sorted, equals the pinned 235-name IWORLD_MEMBERS set (a member
