...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  it('pins total / data / method counts', () => {
    ...
    // (Thornhollow Fields) adds the four battleground facet members (the bgInfo
    // data member plus the bgQueueJoin / bgQueueLeave / bgFlagAction commands),
    // and the class wave adds Paladin Consecration ground-state, priest marker
    // projections, and the controlled Warlock pet's pet-bar mirror (three data
    // members) plus the pet's signature-skill command and autocast toggle (two
    // methods), leaving 298.
