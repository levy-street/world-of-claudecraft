...
//
// `IWorld` (src/world_api.ts:341-510, 151 members) is the ONE seam render/ui depend// on. `tsc` already proves both the offline `Sim` and the online `ClientWorld` satisfy
...
// it lands the matching edit here in the SAME commit. The count pins (151 / 36 / 115)// plus the sorted-name `toEqual` snapshots (modeled on the anti-loosening exclude-set
...
describe('IWORLD_MEMBERS is the pinned IWorld contract (anti-loosening)', () => {
  ...
  it('the full sorted member set is exactly the pinned 151', () => {    expect(IWORLD_MEMBERS.map((m) => m.name).sort()).toEqual([
      'abandonPet',
      'abandonQuest',
      'acceptLinkedQuest',
      'acceptQuest',
...
//   (4) their union, sorted, equals the pinned 151-name IWORLD_MEMBERS set (a member//       dropped from the split reddens).
...
describe('W1: aggregate IWorld member set equals the disjoint union of the 20 facets', () => {
  ...
  it('the union of the 20 facets equals the pinned 151-member IWORLD_MEMBERS set', () => {
    ...
    expect(new Set(union).size, 'union size after dedup (catches a duplicated member)').toBe(151);    const sortedUnion = [...union].sort();
