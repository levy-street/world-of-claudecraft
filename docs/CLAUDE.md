...
Fiesta kills, the FURY Honor Quartermaster vendor, and the Warfare PvP stat): SHIPPED
in #1817 on release/v0.25.0, kept as the reference spec with an as-shipped note.
`prd/frontier-pvp.md` (was `frontier-pvp-honor.md`) is the **factionless / free-for-all
open-world PvP zone** (RuneScape-Wilderness-style: no teams, everyone-hostile-in-band, a
depth danger gradient, resource cargo extraction, 2x honor on open-world kills) plus the
$WOC stakes layer; it builds on the shipped honor system and is the active next-phase
spec. `prd/FRONTIER_PHASE1_HANDOFF.md` is the slice-by-slice handoff for the zone's
factionless F1 skeleton; read it before starting a slice, and re-anchor its symbol
references against the release branch.
