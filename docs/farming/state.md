...
| STALE PLAYER PROSE FOUND BY THIS SWEEP: guide.profPages.gatherDeeds.farming (src/ui/i18n.catalog/guide.ts:2920) still says farming "keeps no deeds of its own yet ... arrive in a later patch", FALSE since Phase 10 shipped seven deeds; the P7 deferral said reword at Phase 9, only gatherIntro was reworded; reword needs M16 fills and the i18n-semantic-regressions care | P7 / P10 | Phase 13 / orchestrator | open ruling-owed |
| Disposable-PG TOAST/WAL measurement (P3 QA made it a Phase 9 HARD gate; NO execution is recorded in any Phase 9 / 9b / QA block) | P3 QA | maintainer / orchestrator | open ruling-owed (could not place execution) |
| Online resumed mid-growth live render check (P7 QA moved it to Phase 9 QA, which owns the online rig; no execution recorded) | P7 QA | orchestrator | open ruling-owed (could not place execution) |
