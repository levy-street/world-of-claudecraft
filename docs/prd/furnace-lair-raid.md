...
4. RESOLVED: the raid's 32 player-facing strings (wing names + enter/leave
   text, five boss names, Runeseeker Maerin, the three pre-quests) are now
   filled in all five non-Latin locales (zh_CN/zh_TW/ja_JP/ko_KR/ru_RU);
   `i18n_completeness` passes. Placeholders preserved. The maintainer may
   refine wording at release, but the PR-tier gate is green.
