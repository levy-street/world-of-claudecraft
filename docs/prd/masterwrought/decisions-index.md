...
| `DECISION <A..F>` inside a phase file | A per-PHASE settled call, scoped to that phase and never global: the letters restart at A in every phase file, so a bare "DECISION B" is meaningless without its phase. Where a phase's decisions were settled ahead of the session that executes them, the authoritative record is state.md's own dated section and the phase's ledger records them AS EXECUTED (worked example: Phase 11i's A to F, settled 2026-08-20 in "Decisions closed 2026-08-20 (the full delegation)" and executed in the Phase 11i ledger, where DECISION C is also recorded as having been costed on the rarity axis alone) | the owning `docs/prd/masterwrought/phase-NN-*.md`, with the executed record in `state.md` | the phase ledger that executed them; source comments cite them as "masterwrought Phase NN DECISION X" in full, never bare |
| bare `NN`, 01 to 20 | A masterwrought phase. 18 to 20 were APPENDED 2026-08-30 under ruling qr-18-REOPEN (state.md): 18 the perfection sweep (with its QA twin), 19 the rulings gate, 20 the release-tier fill and the re-close; the PR opens after 20. The ruling re-opens the judged refusals and the code-shaped recorded reads across every ledger for Phase 18, leaves the R5 rulings and the accepted-by-design rows untouched, and routes every open maintainer DECISION into Phase 19's table | `docs/prd/masterwrought/phase-NN-*.md` | the `docs/screenshots/masterwrought-phaseNN*` cone rows |
...
full-path citations re-pointed (eleven in the move commit: the two naming tests, the
three R5 test suites, `scripts/r5_envelope_probe.ts`, the two `src/sim/content` KNOWN
COST comments, `src/sim/content/farm_patterns.ts`, `src/sim/content/rift/infernal_citadel.ts`,
and `src/ui/i18n.catalog/items.ts`; the Phase 17 QA round found and re-pointed two more,
the root `CLAUDE.md` and `src/sim/content/CLAUDE.md` naming-audit rows; the archived
`ip-refactor/NAME-MAP.md` keeps its historical path); each promoted file carries a dated banner resolving
