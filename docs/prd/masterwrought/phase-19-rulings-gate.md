...
| 12 | R5 on the merged world (the eighth v0.41.0 sync moved the fixture surface: WAR_BIS 355 to 165 hit under the Crucible hit rebalance c920f39c85, the tank baseline 3332 to 3532 health under the legendary retune 4ed7a279b4): re-measure the record on the merged catalog (re-opens R5 under the four rulings' own conditions), or ratify the record as a measurement of the pre-raid catalog and retire the seven expected-fail pins | state.md, the eighth-sync AMENDED block in the Phase 15 ledger; the seven it.fails pins in r5_envelope_probe / masterwrought_budget / dev_bis_gear | to be priced |
| 13 | The apex tier's placement against the Crucible raid tier (the raid catalog out-scores every apex piece on bestEpicGearFor and the chest lead measures -1 against the ratified cap of 2): re-tier the apex pieces above the Crucible tier, accept the raid tier above apex and amend the power placement, or fold into row 12's re-measure | same block; tests/dev_bis_gear.test.ts and the masterwrought_budget lead-cap arm; the rogue identity re-anchor awaits the same authority's confirmation | to be priced |
