...
authority is `docs/prd/masterwrought/farming/state.md` (locked decisions D1 to D24). This file is the
...
- Step 1, load context: spawn an Explore agent to read and summarize
  `docs/prd/masterwrought/farming/state.md`, `docs/prd/masterwrought/farming/progress.md`, this phase's file, and the
...
- Step 3, validate + review: run the state.md validation matrix rows the diff demands.
  ...
  no BLOCKING remains.
- Step 4, docs + memory: update `docs/prd/masterwrought/farming/progress.md` and `docs/prd/masterwrought/farming/state.md`
...
- Delivery (REWRITTEN at the 2026-08-20 absorb; the pre-absorb per-phase-PR
  bullet was already dead under D22 and is doubly superseded now): delivery is
  the MASTERWROUGHT packet's, one branch and one PR for both packets when the
  maintainer says go; per-phase work still gates with
  `node scripts/gate_select.mjs` (deep check: `npm run gate`) and stays local.
  No Claude attribution or session links in commits or PR text.
...
REWRITTEN at the 2026-08-20 absorb: this packet no longer tears down on its
own. It is part of the Masterwrought packet
(`docs/prd/masterwrought/`), and ONE teardown decision covers both packets'
planning docs, taken with the maintainer at the Masterwrought packet's own
close (its Phase 17 final QA), never unilaterally. The asset handoff manifest
lives in `docs/design/farming-asset-manifest.json` precisely so it survives
whatever that decision is, and `tests/farming_asset_manifest.test.ts` guards
that it never points into the packet.
