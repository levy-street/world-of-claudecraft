...
ready-to-run recipe and the two pins the harvest will red ('the identity is
non-vacuous: real rows on both sides, and every carried row is a row' and
'an undeclared row, a mis-stated ms, and a fabricated fallback block each
RED it', both in tests/ci_shard_partition.test.ts) are recorded on the D168
row of docs/prd/masterwrought/phase-19-new-rows.md; the handoff row in
