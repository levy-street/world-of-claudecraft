...
- Locked deviations from phase files: Phase 1: (a) PENDING_ART_IDS allowlist in
  ...
  - SMALLER LEDGERED NOTES: FarmPatchVisuals.dispose() is never called by
    ...
    clock the renderer advances unconditionally, so no plant in the game
    reads the setting today and gating the crops alone would make farm
    beds the only one that stops. ONE PREMISE CORRECTION so the record
