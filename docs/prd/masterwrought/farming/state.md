...
2. THE MAINTAINER READS OWED (detail at each ledger letter): (w) the
   structural affinity exemption (standing since Phase 4, AMENDED
   2026-09-01 by qr-19-structural-affinity-exemption: the 27 is stale, the
   live derived set is 39, and the exemption is RATIFIED as standing);
   (al) the art-suite guard
   ...
   seed-back rates 0.08/0.40 tier 3 and 0.06/0.35 tier 4 (CLOSED 2026-09-01
   by qr-19-seed-back-rates, ratified as shipped); compost 2 sell /
...
| (w) structural affinity exemption, 27 materials (standing since Phase 4) | P4 / P5 | maintainer | CLOSED 2026-09-01 by ruling qr-19-structural-affinity-exemption (Phase 19, under qr-19-best-for-project): the blanket structural exemption is RATIFIED as standing, and the row figure is AMENDED rather than rewritten: 27 is stale, the live derived set is 39 (FARM_MATERIAL_ITEM_IDS, src/sim/content/farm_crops.ts:355, pinned by LENGTH at tests/professions_zone_rollout.test.ts with its 27-plus-11e derivation stated beside it, and by EXACT SET, which is the anti-abuse gate this ledger's own (w) detail names, at tests/material_taxonomy.test.ts, where all 39 ids are listed as literals). The claim survives its stale count because the watch fee still consumes produce, and the PHASE 4 QA addenda (state.md:3806, the four no-action calls re-judged and ALL UPHELD at :3839-3843, deviation (w) among them) already re-judged and upheld the auto-exemption with the exact-set pin as its gate. WHAT RATIFYING CONCEDES, stated rather than smoothed over: nothing in the tree can then notice a future crop shipping produce with no dish, because the census arm blanket-skips all 39 and the closure arm at tests/professions_zone_rollout.test.ts:976-1065 counts every seed and produce id live by construction. |
| (al) art-suite guard extensions plus the bundle-hash renderer/content split | P6 / P6 QA | maintainer | CLOSED 2026-09-01 by Phase 19 ruling qr-19-art-suite-guard-extensions: the guard-extensions half is RATIFIED as shipped. The ART-SUBJECT rule and the bookkeepingOnly receipt-free refresh stand exactly as written, each policed in both directions and each self-clearing when the art wave lands; refusing them is blocked behind that same parked art wave and by the seventeenth-absorb rule (never a raised release literal, never a gutted arm), and the release has since adopted the same mechanism on its own content side (IGNIVAR_ART_PENDING_ITEM_IDS). Ratification BINDS the standing obligations: both CLIs re-run after every absorb and every sim-content phase, manifest and verdict conflicts resolved by re-running on the merged tree, the same ART-SUBJECT split for any future release seal, and the three literals (catalogCount 1125, liveItemCount 1140, pendingArtCount 81) re-minted at every content move. The row's other half, the bundle-hash renderer/content split, is answered here: RULED 2026-09-01 by qr-19-portrait-bundle-hash-split, the SINGLE bundle hash is RATIFIED and the digest is not split. The tightening the ratification rests on is live and was re-verified: bookkeepingOnly requires changedTrackedFiles to be empty (scripts/lib/mob_portrait_manifest_diff.mjs:54-61) and PORTRAIT_RENDER_FILES tracks all three stills-renderer scripts (scripts/lib/mob_portrait_jobs.mjs:28-32), so a stills-renderer script edit can no longer pass as bookkeeping-only and demands a rendered receipt. The residual is conceded rather than closed: a pixel-affecting edit INSIDE the bundle's src/render reach still reads as content churn at this layer, and it is bounded by the fact that if pixels really move, portrait outputs move and the row-drift arm fires. The split is refused on price, honestly stated: schemaVersion 2 exists to prevent a bootstrap migration and its 242-row re-render, so an additive field plus a reworked withoutBundleFingerprint, a reworked describeManifestDrift in both the .mjs and the .d.mts, new drift arms, a re-mint of every committed manifest baseline and two receipts per absorb forever is 12 to 16 hours buying diagnosability no defect has yet needed. With BOTH halves answered the row is CLOSED. |
...
| Dish foodHp/sellValue assignments and reagent counts (all curve-point reuses) | P6 | maintainer | CLOSED 2026-09-01 by ruling qr-19-dish-curve-point-assignments (Phase 19, under qr-19-best-for-project): the eight pairs (90/6, 117/12, 243/25, 432/40, 552/60, 552/75, 980/150, 980/150) and the live bills are RATIFIED as shipped. Every pair still reuses a point a live NON-dish food carries, backed against the live ITEMS table rather than a hand list, so a dish-only point is refused by construction; every bill is gold-negative at the exact per-row input literal pinned in EXPECTED_INPUT_VALUE, with the beet braise deliberately at break-even on the raw sell basis (bog_beet 8 x3 plus fine_bog_beet 16 x1 is 40 against an output of 40) so cooking converts produce without minting copper; and four of the eight bills (bannock, gourd soup, sunmelon tart, harvest platter) were already revised once deliberately under 11e add-never-substitute and re-derived at the time, so these numbers have survived a real revision plus the packet economy sweeps with no finding against them. The D071 contingency is DISCHARGED rather than waived: qr-19-food-per-point-inversions landed ratifying both reads with nothing moving, so every crafted curve point this row reuses stands where it was. A re-set needs the maintainer's own numbers, which the packet records do not hold: they carry one set, the shipped one. |
...
| fine_marsh_rice / fine_highland_barley dish consumers (hoe-reagent-only today) | P6 | maintainer | CLOSED 2026-09-01 by ruling qr-19-fine-twin-dish-consumers (Phase 19, under qr-19-best-for-project): hoe-reagent-only is RATIFIED as shipped design and no def, reagent or pin moves. THE ROW SET IS AMENDED IN PLACE rather than rewritten: this row's closing clause read "These two remain the only twins whose sole consumer is a hoe rung", and live code FALSIFIES it. The set is THREE. fine_vale_wheat has exactly one reagent site in the whole of src/sim, recipe_bronze_hoe in src/sim/content/recipes.ts, and this row never named it; the shipped pins already know, since tests/farm_recipes.test.ts pins HOE_REAGENT_TWINS as four members with fine_vale_wheat among them and tests/professions_zone_rollout.test.ts already says "the three hoe twins". WHY RATIFY: the record calls the state deliberate twice in shipped source (the two "A hoe reagent, and ONLY that" blocks in src/sim/content/items.ts), and once calls a change to it "a content decision, not a gap in the closure pin" (the Phase 6 sign-off passage in this file, whose seeded address 4449-4452 IS STALE and reads at 4461-4464 today, corrected here rather than rewritten); the load-bearing invariant, that every twin has a consumer by SOME route, is already green over the whole twin column; and the loaf and the rice bowl are the only two plain dishes carrying no fine twin at all, for exactly the coherent reason that their crop's twin went to the hoe ladder, so this is a shape rather than an inconsistency to fix. THE COUNTERWEIGHT, recorded so the ratification is informed: Phase 11j retired the exclusivity rule that made hoe-reagent-only structural, because fine_evergarden_greens is deliberately double-booked across three sites, which is why the partition claim this row used to carry, that a hoe twin has no dish slot, was retired from tests/farm_recipes.test.ts rather than kept. If route diversity is ever wanted, the shipped-pattern way to buy it is a count-1 slot on each twin's own existing bill, and it must then cover fine_vale_wheat too rather than leave the identical question standing one twin over. NOT OWED, each checked rather than assumed: no wiki regen (no content id, reagent or t() key moves), no M16 non-Latin fills, no deed row, no Reliquary page, no art park, no census row, no parity golden, no IWorld member, no monolith ceiling (content is exempt). The only code-adjacent follow-on is comment-class with no assertion behind it: the three shipped comments that still frame the question as open or as two are restamped with this ruling id. |
...
| Gathering wiki tools note: five stale non-Latin fills after an English reword (`guide.profPages.toolsNoteThreeRods`) | P11j QA | maintainer | CLOSED 2026-09-02 by ruling qr-19-toolsnote-stale-nonlatin-fills (Phase 19F, under qr-19-best-for-project): the key is RETIRED AND RE-KEYED as guide.profPages.toolsNoteFiveLadders (the old key deleted outright), with the five non-Latin fills riding the same change and the 15 Latin rows to the Phase 20 fill; re-keyed once more at the 19F review round (2026-09-02) to guide.profPages.toolsNoteFourStarters, the same shape, after the note's English was found to count three fenced 20-copper starters where the set is four (the Garden Hoe included), to place the allotment farmer off any hub, and to omit the rods' Delve Marks route; and a third time to guide.profPages.toolsNoteFishingPageMarks at that round's fresh read, because the note said 'the fishing table below' on all five gathering pages. The masterwrought Phase 11j QA corrected the English of the note that renders above the tool table on EVERY gathering page, because on the FARMING page it said each land trade has two crafted tools, that every character knows the land recipes (all four hoe rungs are `acquisition: ['trainer']`), and that the land trades' top tools buy no access (planting a tier-N bed needs a tier-N hoe). Corrected in the English catalog only, per the contributor rule that a contributor never edits `src/ui/i18n.locales/`. THE EXPOSURE IS THAT NO GATE CAN SEE IT: a reword leaves a translated row translated, so `ja_JP`, `ko_KR`, `ru_RU`, `zh_CN` and `zh_TW` still carry the retired claims and M16 stays green. Either re-fill those five at release or rule that the key is re-keyed so the pending machinery catches it. The fifteen Latin locales were pending and were re-English-filled by the regen |
| ONLINE_WORLD_LAYOUT_VERSION epoch bump at farming go-live (dated note 2026-08-30: the epoch has since moved repeatedly and reads 26 after the eighth v0.41.0 sync, which renumbered the branch's 12 past the release's raid ladder 13 to 25; the go-live question this row records stands unchanged) | 19th absorb | maintainer | CLOSED 2026-09-01 by ruling qr-19-epoch-bump-at-golive (Phase 19, under qr-19-best-for-project): the shipped combined epoch 26 is RATIFIED as the one masterwrought and farming go-live bump, and the server-first deploy order is written into docs/ota-updates.md as a named exemplar. |
| renderer.ts exact-count ceiling re-pin (13774) prepared for feature review | 19th absorb | maintainer | CLOSED 2026-09-01 by qr-19-monolith-ceiling-repin, widened to cover hud.ts: the exact-count posture is KEPT and no margin is minted (root CLAUDE.md's ratchet: never grow a coordinator, extract then lower, and a raise is a maintainer decision). Figure amended in the same ruling rather than rewritten: 13774 is stale; when this row was written renderer.ts pinned 13023 against a file of 13020 and hud.ts pinned 18728 against 18708, so the slack was three and twenty lines, not zero. AMENDED AGAIN at the 19B close, because the same wave then spent part of that slack: D111 and its fix round take hud.ts to 18717, leaving ELEVEN lines under the 18728 pin (renderer.ts is untouched at 13020). The posture is unchanged and no margin is minted; only the measured figure moves. |
...
| Journey script joins the gate when the merged Masterwrought PR ships (reworded at the 11b absorb: D22 is superseded, delivery is the one Masterwrought PR, and the script stays off CI until that ships) | P9b QA | maintainer | CLOSED 2026-09-01 by ruling qr-19-journey-script-gate-entry (Phase 19, under qr-19-best-for-project): the journey stays permanently OFF CI as a hand-run acceptance rig, with the layer-honesty source pin (tests/farm_verb_reachability.test.ts:175-206) as the standing guard. It does not join the gate when the PR ships either: the entry question is answered no, not deferred. |
| Tier-4-to-tier-1 wellfed downgrade legibility (buff hover is the one surface) | P11 QA | maintainer | CLOSED 2026-09-01 by ruling qr-19-wellfed-downgrade-legibility (Phase 19, under qr-19-best-for-project): the downgrade stays invisible in the combat log by design and the buff hover remains the one surface, identically in both hosts. Scope that precisely: the log is not silent about the MEAL, since the gain arm still fires and prints "You gain Well Fed." with no refresh gate, so what is invisible is the DIRECTION and the MAGNITUDE, never the event. Verified live rather than taken from the record, and the seeded chain is corrected here rather than rewritten: the ONE mint in src/sim/wellfed.ts stamps WELL_FED_AURA_ID and sourceId on every well-fed aura (the seven payloads themselves carry only the name, kind, value and duration), so a newer meal is a replacement conflict, and all seven ALSO carry the identical display name 'Well Fed', which is the separate fact that takes the silent branch in Sim.applyAura (a differing displaced name emits the fade, a matching one only sets refresh); the own-aura log line interpolates the name alone, so it could not name a tier even if it fired. The surviving hover is the BUFF BAR's rather than the item tooltip's, auraTooltipBodyHtml falling through to auraEffectDescriptor over the LIVE aura, so the hover renders the ACTIVE payload the meal carried and the state a player acts on is legible the moment they look. A log line would owe a new SimEvent on the displacement path in both hosts for a self-inflicted, reversible outcome. |
...
| Idle sway ignores reducedMotion (foliage precedent) | P7 QA | maintainer | CLOSED 2026-09-01 by ruling qr-19-idle-sway-reducedmotion (Phase 19, under qr-19-best-for-project): the ambient plant wind exemption is RATIFIED and the class boundary is recorded in docs/design/graphics-settings-fairness.md. Verified live at the close rather than taken from the record: src/render/farm_patches.ts holds no reducedMotion read at all, its sway being a per-plot JS phase advanced by dt and composed onto the bed's seat quaternion, and the foliage wind it follows is gated only by the STATIC GFX.windSway preset knob over a sharedUniforms.uTime clock the renderer advances unconditionally, so no plant's ambient motion reads the setting today and gating the crops alone would make farm beds the only one that stops. THE BOUNDARY IS NARROWER THAN "WORLD MOTION" IN TWO DIRECTIONS, stated here so the ratification is not read wider than it is: an ability marker drawn in the world is still gated (umbral_anchor_marker.ts freezes its uTime to zero, which it can do because it owns a PRIVATE uTime rather than the shared clock), and a camera-driven TRANSITION inside the foliage subsystem itself is still gated (tree_hide_fade.ts's occluder ghost ramp, threaded in from foliage.ts). So what is exempt is the CONTINUOUS AMBIENT plant motion carrying no actionable state, never the subsystem. No code, pin or ceiling moves. |
...
| guide.profPages.rareBody (shared across the four gathering pages, filled translated key) names only the three node-trade windfall flavors; farming's golden-harvest flavor is absent (the farming page's new tableBody covers it, so no falsehood renders); the reword needs the new-key treatment plus fills | P13 lane A | release fill / maintainer | CLOSED 2026-09-02 by ruling qr-19-rarebody-reword-landmine (Phase 19F, under qr-19-best-for-project): retired and re-keyed as guide.profPages.rareBodyFourFlavors with the maintainer's crop-windfall wording ('a golden harvest from a garden bed'), five M16 fills in-change (machine-authored, flagged for the maintainer's read; the old key's five non-Latin fills had omitted the flavor names and the deed sentence since they were written), the 15 Latin rows to the Phase 20 fill; the old key stays in the catalog, retired, so its 21 reviewed rows never strand. |
| FarmBiomePalette.trim has no draw-time consumer (soil tints the bed, wood tints the bin; the core comment claimed trim covered the bin and is now corrected); wire trim in the higher-tier pass or drop the channel | P13 integration | maintainer / art batch | DEFERRED 2026-09-01 by qr-19-farm-biome-palette-trim: the channel is kept and wired in the maintainer's higher-tier art pass; stays handed-to-maintainer. The dated line lands IN this cell rather than after the row, because a markdown table row cannot carry an inserted annotation line (the qr-18-REOPEN rule stated below this table), and every Phase 19 flip on this table carries its ruling inside the status cell. Premise re-read live at the close rather than taken from the record: the interface comment in src/render/farm_patches_core.ts still says the channel is unconsumed at draw time, the FOUR FARM_BIOME_PALETTES rows and the fallback still carry a trim value (the seeded count of five was wrong, and tests/farm_patches_core.test.ts pins four biomes), and src/render/farm_patches.ts still tints only the bed through palette.soil and the bin through palette.wood, so no third consumer appeared. Wiring it now is not cheap: no trim mesh exists in the shipped bed and bin set, so a draw-time consumer means choosing which surface trim tints, the art judgment the batch owns, and it mints a new per-instance GPU write. Dropping it is wider than the seeded one hour: besides the interface, the four rows, the fallback and the concatenated signature arm in tests/farm_patches_core.test.ts, it reds the two soil/wood/trim channel tuples and the channelUse containment pin in tests/farming_asset_manifest.test.ts and forces a hand edit of docs/design/farming-asset-manifest.json, which has no generator. |
...
- Locked deviations from phase files: Phase 1: (a) PENDING_ART_IDS allowlist in
  ...
  the open call.
  RULED (qr-19-portrait-bundle-hash-split, 2026-09-01, under
  qr-19-best-for-project): KEEP THE SINGLE HASH. The residual above is
  accepted, not closed, and its blast radius is smaller than the sentence
  reads: a src/render edit that actually moves pixels moves portrait
  OUTPUTS too, and row drift is not bookkeeping-only, so what the single
  digest really hides is a pixel-affecting edit that changes no shipped
  byte. The bookkeepingOnly tightening this rests on was re-verified at
  execution: the verdict requires an empty changedTrackedFiles and
  PORTRAIT_RENDER_FILES tracks all three stills-renderer scripts, so a
  script edit demands a rendered receipt. The split is refused on price:
  schemaVersion 2 exists to prevent a bootstrap migration and the 242-row
  re-render, so the flip is an additive field plus reworked
  withoutBundleFingerprint and describeManifestDrift in both the .mjs and
  the .d.mts, new drift arms, a re-mint of every committed baseline, and
  two receipts per absorb forever. Nothing in the tree moves under this
  ruling except the guard's own residual comment, which stops calling the
  split an open call.
  QA-round evidence re-mints, superseding the
  ...
  - SMALLER LEDGERED NOTES: FarmPatchVisuals.dispose() is never called by
    ...
    RULED (qr-19-idle-sway-reducedmotion, 2026-09-01, under
    qr-19-best-for-project): the idle-sway note above is ANSWERED. The
    farm crop sway is ambient decorative plant motion and stays exempt
    from the viewer's prefers-reduced-motion setting, the foliage
    precedent: the wind sway on every canopy, bush and grass card reads
    only the STATIC GFX.windSway preset knob over a sharedUniforms.uTime
    clock the renderer advances unconditionally, so no plant's ambient
    motion reads the setting today and gating the crops alone would make
    farm beds the only one that stops. THE EXEMPT CLASS IS THE CONTINUOUS
    AMBIENT MOTION, not the foliage subsystem: a camera-driven TRANSITION
    is a fade rather than ambient motion, and tree_hide_fade.ts's occluder
    ghost ramp DOES honor the setting, threaded in from foliage.ts. ONE
    PREMISE CORRECTION so the record
    does not assert a mechanism that does not exist: the crops do not
    ride that shader clock. src/render/farm_patches.ts holds no uTime, no
    sharedUniforms and no reducedMotion read at all; its sway is a
    per-plot JS phase advanced by dt and composed onto the bed's seat
    quaternion. What the crops and the foliage share is the CLASS, not
    the clock. THE CLASS BOUNDARY IS NARROWER THAN "WORLD MOTION", and is
    now recorded in docs/design/graphics-settings-fairness.md: an ability
    marker drawn in the world is still gated, since
    src/render/umbral_anchor_marker.ts freezes its uTime to zero under
    the setting, so what is exempt is ambient plant motion carrying no
    actionable state. No code, no pin and no ceiling moves.
...
- Phase 9 ledger (go-live, 2026-08-17). NPCs: farmer_jessica (zone1.ts
  ...
  RULED (qr-19-pendingsend-dropped-command-stranding, 2026-09-01, under
  qr-19-best-for-project): the online stranding residual above is ACCEPTED as
  family-consistent and no family-wide re-arm is ordered. THE PRICED FIX NAMED
  THE WRONG FILE and is corrected here rather than rewritten: the gate row
  costed "pendingSend re-arm on reconnect/spectate across the whole command
  family in src/net/online.ts", and pendingSend does not exist anywhere under
  src/net. Re-grepped live at the close, the flag is HUD window state in two
  places only, src/ui/hud/professions/farming_plant_sheet_window.ts (the
  error-toast forward documented on notifyErrorToast, the send guard on the
  plant path) and src/ui/hud/professions/perfecting_window.ts (the in-flight
  belief contract in its header), under the family rule in
  src/ui/hud/professions/CLAUDE.md. So a fix would be a shared
  clear-on-spectate-or-reconnect arm across the HUD window family, not a
  transport change, and neither window carries a spectate or a reconnect arm
  today. WHY ACCEPT: the row is ledgered optional, every window under spectate
  strands the same way, and the control recovers on close and reopen, since
  both open() and close() clear the flag in both windows. The fix arm was
  costed at six to eight hours: a shared re-arm hook on both windows and every
  future window carrying the flag, a design for the double-send race the
  ledger flags (the sim re-validates every gate, so a re-click is safe; a
  re-arm racing an in-flight answer is the case to pin), new arms in the two
  window suites, and the family pin in that CLAUDE.md restated. Nothing moves:
  no code, no pin, no wiki regen, no M16 fill, no deed row, no Reliquary page,
  no art park, no census row, no parity golden, no IWorld member and no
  monolith ceiling.
  ...
  RULED (qr-19-journey-script-gate-entry, 2026-09-01, under
  qr-19-best-for-project), on the journey-script clause: the script stays
  PERMANENTLY off CI as a hand-run acceptance rig, and the layer-honesty
  source pin is the standing guard in its place. The guard for the exact
  class the journey exists to catch already runs on every CI leg
  (tests/farm_verb_reachability.test.ts:175-206, non-vacuous in BOTH
  directions: it reds on a script that drives a farming or quest verb
  through window.__game AND on an emptied or rewritten script that no
  longer drives the real controls). No scripts/*_e2e.mjs has ever run on
  any workflow (ten such rigs exist, zero hits for _e2e under
  .github/workflows/), so putting the first one on a lane would mint a new
  job key and its two whole-file pins, and the packet holds no measured
  wall for the journey to bound it with. The D22 wording this clause
  inherits is superseded: entry was never conditional on go-public, it is
  refused outright.
  ...
  - The tier-4-to-tier-1 downgrade is invisible in the combat log by
    ...
    identically in both hosts. Scope that precisely: the log is not
    silent about the MEAL. The gain arm still fires and the HUD prints
    "You gain Well Fed." with no refresh gate, so what is invisible is
    the DIRECTION and the MAGNITUDE, never the event. Verified live
    rather than taken from the record, and the seeded chain is restated
    because it was incomplete: the ONE mint in src/sim/wellfed.ts stamps
    WELL_FED_AURA_ID and sourceId on every well-fed aura (the seven
    payloads themselves carry only the name, kind, value and duration),
    so a newer meal is a replacement conflict, AND all seven also carry
    the identical display name 'Well Fed', which is the
...
- ONLINE_WORLD_LAYOUT_VERSION at farming go-live (raised by the nineteenth-absorb
  ...
  RULED (qr-19-epoch-bump-at-golive, 2026-09-01, under qr-19-best-for-project): the
  shipped combined ONLINE_WORLD_LAYOUT_VERSION 26 is RATIFIED as the one
  masterwrought-and-farming go-live bump. src/world_api.ts already reads 26 and its
  own comment names farming's fplot delta as half the bump's ground; Perfecting
  forces the bump regardless, so "no bump" is not on the table, and a farming-only
  split (epoch 27) buys no observable deploy under the one-PR delivery rule while
  costing a five-file re-pin. The server-first deploy order is written into
  docs/ota-updates.md as a named exemplar (option 2's cheap rider).
...
- Phase 6 proposed values awaiting sign-off: the eight dish foodHp/sellValue
  ...
  RULED (qr-19-dish-curve-point-assignments, 2026-09-01, under qr-19-best-for-project): the two
  DISH halves of this sign-off list close here, and only those two. The tonic recipe half and the
  fine-twin half are ruled separately and their wording above is left standing rather than
  rewritten. The eight plain dish foodHp and sellValue pairs and the live reagent bills are
  RATIFIED as shipped, so the flag they were opened against is retired at both of its source
  sites in the same change (src/sim/content/profession_items.ts and src/sim/content/recipes.ts);
  nothing in that block is proposed any more. Verified live at the close rather than taken from
  the record: every allowed curve point is backed against the ITEMS table rather than a hand
  list, so a dish-only point is refused by construction; every bill is gold-negative at the exact
  per-row input literal; and the beet braise sits deliberately at break-even on the raw sell
  basis (bog_beet 8 x3 plus fine_bog_beet 16 x1 is 40 against an output of 40), which is what
  keeps that arm's bound a tested edge instead of slack. Four of these bills are no longer the
  Phase 6 proposals this list was written against: 11e widened the bannock, the gourd soup, the
  sunmelon tart and the harvest platter under add-never-substitute and re-derived each input
  figure at the time, so the set has survived one real revision plus the packet economy sweeps
  with no finding against it. The D071 contingency is discharged rather than waived:
  qr-19-food-per-point-inversions landed ratifying both reads with nothing moving, so every
  crafted point these dishes reuse stands where it was. A re-set would need the maintainer's own
  numbers, and the packet records hold exactly one set, the shipped one.
