...
Status: Draft for owner review. Tuning numbers are PROPOSED with derivation shown,
not locked. All citations are against `origin/main` (and named feature branches)
as of this draft.
...
- **ilvl ladder** (source level + `QUALITY_ILVL_BONUS`/`RAID_ILVL_BONUS`):
  ...
  | Heroic boss set (`heroic_loot.ts`, 27 epics) | 25 | +6 | **31** |
  | Legendaries (2) | 20 | +10, raid +3 | 33 |
  | Incoming Heroic Nythraxis raid (`feature/heroic-nythraxis-tier`) | 27 | +6 / +10 | **33 / 37** |
...
Proposed allowance (primary rating per piece; 10 rating = 1%; tuning targets):
...
Budget policy (open question 9.1): v1 keeps ratings fully off the primary budget
...
| deathless_greatblade | mainhand | hit |
| gravewyrm_cleaver | mainhand | crit |

Result: a HEAVY set is ~5 hit + 3 crit, so a warrior fresh into Heroic claws back
~20% miss immediately, which is the whole point.
...
| scepter_of_the_deathless_court | mainhand | haste |
...
| stormcallers_focus | mainhand | haste |
...
These are auto-generated by `buildHeroicVariants` from the ilvl-29 base set pieces
and the two legendaries. Extend the variant builder so a heroic raid variant:
...
Optional parity (open question 9.3): give the caster T2 4pc (`set_soulblaze`, not a
bleed but milder than the T1 caster proc) a smaller `hitRating: 30` so casters are
not left out of the "4pc grants hit" buff. Not required for v1.
...
- `tests/item_level.test.ts`: primary-sum assertions stay green (ratings off-budget).
  Add a rating-allowance assertion: every ilvl-31 piece carries exactly one rating at
  its band, every ilvl-33/37 variant carries two, ilvl-26/28 armor carries none, and
  each vendor jewelry piece carries one. This is the guard that keeps the tier ladder
...
- Three-host parity: `hitRating`/`hitBonus` are pure derived stats recomputed by
  `recalcPlayerStats` on both hosts (like crit rating), so no wire change is expected;
  confirm with `cross-platform-sync`.
...
Open:
1. Ratings off the primary budget (v1, recommended) vs consuming some budget? Off-budget
   is what makes higher tiers feel stronger; on-budget would re-flatten them.
2. Exact allowance numbers (Section 5 table) and the per-piece assignments (Section 6)
   are tuning targets to lock.
3. Extend the "4pc grants hit" buff to the caster T2 4pc for archetype parity (7)?
4. Should a later expertise-style stat also cut mob dodge (untouched by hit)? At +3
   dodge is only 6.5%, low priority.
5. Do the ilvl-28 heroic dungeon variants stay rating-free (v1, to keep 31 the jump),
   or get a token rating?

## 10. Suggested PR breakdown

- PR A: hit-rating engine primitive (`hitRating` on `BaseItemDef` + `SetBonusEffect`,
  `HIT_RATING_PER_PCT`, entity fold, `swingMissChance`/spell-resist subtraction,
  resolver sum). Sim + parity tests. No content.
- PR B: rating allowance + validation test in `item_budget.ts`/`item_level.ts`.
- PR C: rebalance the 10 marks jewelry (6.1) + tooltip line + hit `t()` key.
- PR D: itemize the ilvl-31 heroic boss set (6.2) and the ilvl-29 raid seeds (6.3).
- PR E: weak 4pc bleed hit grants (7).
- PR F: raid variant builder dual-rating for ilvl-33/37 (6.4), landing with or after
  `feature/heroic-nythraxis-tier`.
- PR G: Azazel loot ratings (6.5), landing with `feature/azazel-heroic-loot`.
