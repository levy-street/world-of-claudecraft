...
- **Validation constants:** `COLLISION_EPSILON_CSS_PX = 0.5`; exact phone/tablet geometry and
  safe-area matrix from the product spec in both handedness variants; touching padded envelopes is
  valid. Defaults, load, runtime fallback, and Save share the strict validator with no baseline
  waiver.
