"""Art review renders for Vysska's cocoons (Blender, headless).

    blender --background --python docs/design/cocoon/preview.py

Writes silk_cocoon.png and brood_cocoon.png beside this file, each with a
figure-height post for scale (a wrapped player stands INSIDE the silk cocoon).
The web mark, the hanging strand, the rescue ring and her feeding are runtime
effects.
"""
import math
import os
...
if __name__ == "__main__":
    reset(); load("silk_cocoon_components.glb"); ground((0.1, 0.11, 0.08)); lights(3.0)
    figure(2.6, -0.6)
    camera((6.5, -8, 3.4), (0.4, 0, 1.8), lens=42); render("silk_cocoon.png")
    reset(); load("brood_cocoon_components.glb"); ground((0.1, 0.11, 0.08)); lights(3.0)
    figure(3.0, -0.6)
    camera((6.5, -8, 3.2), (0.4, 0, 1.3), lens=42); render("brood_cocoon.png")
