...
| # | Figure | Defect | Fix | Status |
|---|---|---|---|---|
| 2.1b | Ewald | Wanted a bigger, custom beard. Two attempts were rejected as "scuffed": both landed as a flat dark panel taped across his cheek with a hard rim, with the shipped moustache poking out from under it | **GROW IT FROM HIS OWN FACE.** `parts.grow_patch` copies the head's own front-lower faces and displaces them along their normals, with the displacement TAPERED TO ZERO at the patch rim, so the edges stay welded flush to the head and only the interior swells. No inner shell is needed: the head is the back face. Excludes faces already sampling the hair cell, so it grows off the SKIN and leaves the shipped moustache and chin tuft proud on top (growing them too swells them along their own wild normals and the whole mouth crumples). Subdivision is OFF by default: subdividing a triangle patch then displacing per-vertex makes the surface lumpy, which is worse than the coarse taper it was meant to fix. | **DONE** |
...
## 8b. Scope: MVP consistency

The maintainer's call, mid-Marsh: simplify where it helps, and an MVP that is
CONSISTENT beats chasing every numbered defect. So the bar for each remaining
figure is the one Ewald and Marsh now meet, and nothing finer:

- one silhouette prop that reads from every angle,
- three values (a pale field, a dark field, one accent),
- held props that read correctly in Idle and in the attack,
- no stray pale cells, no geometry poking through anything,
- and the authored detail actually visible at plate size.

Items below that bar (2.10's rank badge, 1.11's Cheer-pose clip, 2.22's parked
Nell details) are explicitly not blockers.
...
- **Grow organic additions OUT OF the host surface; never fit a shell over it.**
  Ewald's beard was attempted twice as generated geometry, first a revolved
  profile and then a lofted ray-cast shell, and both were rejected as scuffed.
  The failure was structural, not parametric: a shell is unrelated to the surface
  it sits on, so it lands as a slab with a hard rim however it is tuned, and no
  amount of retuning was going to fix it. The third attempt copies the host's own
  faces and swells them with the offset tapered to zero at the rim, which matches
  the face BY CONSTRUCTION. `parts.grow_patch` is the primitive; it is the route
  for Tam's hair (2.4) and the base of Edda's plait (2.5) too, and it should have
  been the first thing tried rather than the third.
- **Cells are shared across meshes; check before repainting one.** On the knight body
  a single palette cell covers the cuirass, arms, legs AND the helmet, which is why
  Marsh read as one flat monochrome mass and why no per-piece repaint was possible.
  The fix is to re-UV one MESH's faces onto a cell the others do not use, then repaint
  the shared cell freely. Pick a genuinely free cell when doing it: the first attempt
  moved the helmet onto `steel_b`, which the body also uses for a small chest patch,
  and produced the stray pale fleck of 2.9.
- **Align a colour boundary to an EDGE LOOP.** A boundary chosen by coordinate cuts
  across triangles and renders as a sawtooth that reads as tearing. Measure the host's
  vertex-z histogram, find the loops (the knight body has strong ones at z=0.958 and
  z=1.255), and land the boundary between two of them.
