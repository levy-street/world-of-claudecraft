...
| # | Figure | Defect | Fix | Status |
|---|---|---|---|---|
| 2.1b | Ewald | Wanted a bigger, custom beard. First attempt read as "scuffed": a blocky slab floating off the jaw with a hard straight top edge like a chin strap | `parts.beard` places every vertex by RAY CASTING from the head axis onto the face, so the mass matches the real chin. `hug_profile` cannot do this: it revolves one radius per angle, so the front bucket picks up whatever juts furthest inside a z band and applies it across the whole arc, which is what made the slab float. The OUTLINE carries the read: top edge low at the chin (below the mouth) rising to the sideburns, bottom hanging lowest at the front and tucking up under the jaw corners. Solid rather than a paper shell (inner surface stitched to outer around the perimeter), smooth-shaded because stacked rows flat-shaded read as horizontal ribbing, and its palette shade matched to the shipped moustache so it stops reading as a separate darker object. Three build errors on the way: rings with different vertex counts stitch across mismatched angles and make a spiked tangle; `front` is in TURNS, where the face is -0.25 (-pi/2) and -0.5 puts the beard on a cheek; and a top edge at one height across the face is a strap, not a beard. | **DONE** |
