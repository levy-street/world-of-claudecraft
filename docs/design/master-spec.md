...
**Junk (kind 'junk', quality 'poor'):** bogiron_nugget (12), soggy_moccasin (9), cracked_fetish (14), chipped_tusk (15), ogre_toe_ring (25), inert_storm_shard (28), frayed_prayer_beads (30), deepfen_pearl (600, Vael trophy). Since Masterwrought phase 11l (the trophy economy) cracked_wyrm_scale (35) is a quality 'common' crafting reagent consumed by `TROPHY_RECIPES` (src/sim/content/recipes.ts), never swept by Sell Junk; sellValue unchanged. chipped_tusk, bogiron_nugget and cracked_fetish stay poor: each is output-excluded under the record in the `TROPHY_RECIPES` header (no uncrafted output in its band survives beside the trainer's own rows).
