...
- **Mount running:** the same distance accumulator fires a mount-specific
  ...
  `mount_run_<key>` in the catalog (the Lanternback Troll) falls back to the plain
  `foot_<surface>` cue at running-step gain and pitch — hence the `surface`
  argument, which the custom-cue branch ignores.
