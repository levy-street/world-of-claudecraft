...
## Levels 21-60: the extended ladders (v0.20, the 60 cap)

Every ranked ability continues past 20 to a top rank in the 50-60 band
(guarded by `tests/spell_ranks.test.ts`). The 21-60 rows were generated by
continuing each ability's own 1-20 curve at the real classic growth schedule,
measured on the reference ladders (Fireball, Frostbolt, Healing Wave): value
growth per 6 levels is ~1.42 up to L30, ~1.33 to L42, then ~1.25, and mana
costs grow ~1.35/1.2/1.15 on the same bands (rage and energy costs stay flat,
as in classic). The generated Fireball ladder lands within a few percent of
the real classic table (L48 rank: 259-348 vs the real 255-353); that anchor is
asserted in the guard test. Cadence is one rank every 6 levels for the core
rotational kit, wider (8-12) for the late additions and the talent signature
strikes (Mortal Strike, Bloodthirst, Shield Slam, Whirlwind at 24/36/48/60).

Only the numeric fields that ranked in the authored 1-20 rows scale (the
generator diffs the last two authored rows), so CC durations, radii, tick
intervals, and stack caps never scale by accident. Conjure Food/Water gain
ranks 4-6 (L28/40/52) resolving the real classic consumable tiers
(874/1392/2148 food hp; 1344/2160/2934 drink mana).
